...
async function fixture(rootHtml, blogHtml) {
  ...
  await fs.writeFile(path.join(publishDir, 'index.html'), rootHtml);
...
describe('functional route heading verifier', () => {
  it('accepts exactly one H1 on the root and blog routes', async () => {
    const publishDir = await fixture('<h1>Docs</h1>', '<h1>Blog</h1>');
  ...
  it('fails closed when a heading is missing or duplicated', async () => {
    const missing = await fixture('<h1>Docs</h1>', '<main>Blog</main>');
    expect(() => verifyFunctionalHeadings(missing)).toThrow('/blog rendered 0 H1');

    const duplicate = await fixture('<h1>Docs</h1><h1>More docs</h1>', '<h1>Blog</h1>');
    expect(() => verifyFunctionalHeadings(duplicate)).toThrow('/ rendered 2 H1');
  });
});
