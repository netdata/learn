...
list of [simple patterns](/docs/agent/libnetdata/simple_pattern). For example,
...
list of [simple patterns](/docs/agent/libnetdata/simple_pattern). For
...
The value is a space-separated list of [simple patterns](/docs/agent/libnetdata/simple_pattern). For
...
-   `OPTIONS` is a space separated list of `percentage`, `absolute`, `min2max`, `unaligned`,
     `match-ids`, `match-names`. Check the [badges](/docs/agent/web/api/badges) documentation for more info.
...
### Example 8 - [Anomaly rate](/docs/agent/ml#anomaly-rate) based CPU dimensions alarm

Warning if 5 minute rolling [anomaly rate](/docs/agent/ml#anomaly-rate) for any CPU dimension is above 5%, critical if it goes above 20%:
...
### Example 9 - [Anomaly rate](/docs/agent/ml#anomaly-rate) based CPU chart alarm

Warning if 5 minute rolling [anomaly rate](/docs/agent/ml#anomaly-rate) averaged across all CPU dimensions is above 5%, critical if it goes above 20%:
...
### Example 10 - [Anomaly rate](/docs/agent/ml#anomaly-rate) based node level alarm

Warning if 5 minute rolling [anomaly rate](/docs/agent/ml#anomaly-rate) averaged across all ML enabled dimensions is above 5%, critical if it goes above 20%:
...
The `lookup` line will use the `anomaly_rate` dimension of the `anomaly_detection.anomaly_rate` ML chart to calculate the average [node level anomaly rate](/docs/agent/ml#node-anomaly-rate) over the last 5 minues.
