...
[dashboard](/docs/visualize/interact-dashboards-charts), run health checks to [trigger
alarms](/docs/monitor/view-active-alarms) and [send notifications](/docs/monitor/enable-notifications), and
[export](/docs/export/external-databases) all metrics to an external TSDB. When Netdata streams metrics to another
...
[distributed architecture](/docs/store/distributed-data-architecture) is ideal for speed and scale, streaming
...
between parent and child dashboards, all served by the parent's [web server](/docs/agent/web/server).
...
streaming](/docs/metrics-storage-management/enable-streaming) using a simple `parent-child` relationship. For all
the details, see the [streaming reference](/docs/metrics-storage-management/reference-streaming) doc.

Take your streaming setup even further by [exporting metrics](/docs/export/external-databases) to an external TSDB.

### Related information & further reading

- Streaming
  - **[How Netdata streams metrics](/docs/metrics-storage-management/how-streaming-works)**
  - [Enable streaming between nodes](/docs/metrics-storage-management/enable-streaming)
  - [Streaming reference](/docs/metrics-storage-management/reference-streaming)