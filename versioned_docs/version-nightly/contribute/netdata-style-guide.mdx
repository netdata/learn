...
For example, the [configuration](/docs/nightly/setup) doc first
...
| **Recommended** | Use `edit-config` to edit Netdata's configuration by first navigating to your [Netdata config directory](/docs/nightly/setup#the-netdata-config-directory), which is typically at `/etc/netdata`, then running `sudo edit-config netdata.conf`. |
...
to `/docs/nightly/setup`. To reference
