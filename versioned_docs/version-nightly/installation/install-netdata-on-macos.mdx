...
The number of charts displaying system metrics is limited, but you can use any of Netdata's [external plugins](/docs/nightly/developers/external-plugins-overview) to monitor any services you might have installed on your macOS system. 
You could also use a macOS system as the parent node in a [streaming configuration](/docs/nightly/references/streaming-and-replication).
...
> Community-maintained packages _may_ receive support from Netdata, but are only a best-effort affair. Learn more about [Netdata's platform support policy](/docs/nightly/getting-started/supported-platforms).
...
The `kickstart.sh` script accepts additional parameters to automatically [connect](/docs/nightly/setup/connect-agent-to-cloud) your node to Netdata
...
- `--claim-proxy`: Specify a proxy to use when connecting to the cloud in the form of `http://[user:pass@]host:ip` for an HTTP(S) proxy.
  See [connecting through a proxy](/docs/nightly/setup/connect-agent-to-cloud#connect-through-a-proxy) for details.
...
If you experience issues while claiming your node, follow the steps in our [Troubleshooting](/docs/nightly/setup/connect-agent-to-cloud#troubleshooting) documentation.
...
3. Use the same terminal session to install some of Netdata's prerequisites using Homebrew. If you don't want to use [Netdata Cloud](/docs/nightly/getting-started/netdata-cloud), you can omit `cmake`.
...
When you're finished with installation, check out our [single-node](/docs/nightly/possible-old,-not-wanted-documents/infrastructure-monitoring-with-netdata) or
[infrastructure](/docs/nightly/possible-old,-not-wanted-documents/infrastructure-monitoring-with-netdata) monitoring quickstart guides based on your use case.

Or, skip straight to [configuring the Netdata Agent](/docs/nightly/concepts/configure-the-netdata-agent).
