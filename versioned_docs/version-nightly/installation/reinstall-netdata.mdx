...
any [user configuration](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/configuration) in `netdata.conf` or other files, and will keep the same install
type that was used for the original install.

If you used any [optional
parameters](/docs/nightly/installation/installation-methods/one-line-installer-kickstart.sh#optional-parameters-to-alter-your-installation) during initial
...
best route is to [uninstall](/docs/nightly/installation/uninstall-netdata) and then try a fresh installation using the [one-line
installer](/docs/nightly/installation/installation-methods/one-line-installer-kickstart.sh).
