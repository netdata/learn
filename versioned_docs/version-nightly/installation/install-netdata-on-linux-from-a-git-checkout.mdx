...
-   `--disable-telemetry`: Opt-out of [anonymous statistics](/docs/nightly/references/anonymous-statistics) we use to make
    Netdata better.
...
-   `--local-files`: Used for [offline installations](/docs/nightly/installation/install-netdata-on-offline-systems). Pass four file paths: the Netdata
...
Unlike the [`kickstart.sh`](/docs/nightly/installation/install-netdata-with-kickstart.sh), the `netdata-installer.sh` script does
not allow you to automatically [connect](/docs/nightly/setup/connect-agent-to-cloud) your node to Netdata Cloud immediately after installation.

See the [connect to cloud](/docs/nightly/setup/connect-agent-to-cloud) doc for details on connecting a node with a manual installation of Netdata.
