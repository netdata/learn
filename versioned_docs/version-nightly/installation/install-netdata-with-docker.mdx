...
> :warning: Warning
>  
> The [edit-config](/docs/nightly/configuration#the-netdata-config-directory) script doesn't work when executed on
> the host system.
...
[HAProxy](/docs/nightly/setup/expose-local-dashboard-through-proxy/netdata-via-haproxy) so that it restricts connections to read-only access to the CONTAINERS
endpoint.
...
[Netdata daemon command line options](/docs/nightly/references/configuration/netdata-daemon#command-line-options) such as the IP address Netdata will be
...
instance](/docs/nightly/configuration/security-design). This section contains an example of how to install Netdata with an SSL
