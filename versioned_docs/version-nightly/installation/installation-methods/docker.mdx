...
> :warning: Warning
>  
> The [edit-config](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/configuration#the-netdata-config-directory) script doesn't work when executed on
> the host system.
...
[HAProxy](/docs/nightly/deployment-in-production/secure-your-nodes/reverse-proxies/haproxy) so that it restricts connections to read-only access to the CONTAINERS
endpoint.
...
[Netdata daemon command line options](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/netdata-daemon#command-line-options) such as the IP address Netdata will be
...
instance](/docs/nightly/deployment-in-production/secure-your-nodes). This section contains an example of how to install Netdata with an SSL
