...
The number of charts displaying system metrics is limited, but you can use any of Netdata's [external plugins](/docs/nightly/improving-netdata---developers/external-plugins) to monitor any services you might have installed on your macOS system. 
You could also use a macOS system as the parent node in a [streaming configuration](/docs/nightly/deployment-in-production/streaming-and-replication/streaming-configuration-reference).
...
> Community-maintained packages _may_ receive support from Netdata, but are only a best-effort affair. Learn more about [Netdata's platform support policy](/docs/nightly/installation/platform-support-policy).

## Install Netdata with our automatic one-line installation script

**Local Netdata Agent installation**
To install Netdata using our automatic [kickstart](/docs/nightly/getting-started/install-netdata#automatic-one-line-installation-script) open a new terminal and run:
...
The `kickstart.sh` script accepts additional parameters to automatically [connect](/docs/nightly/getting-started/monitor-your-infrastructure/connect-agent-to-cloud) your node to Netdata
...
area](/docs/nightly/getting-started/monitor-your-infrastructure/spaces).

- `--claim-token`: Specify a unique claiming token associated with your Space in Netdata Cloud to be used to connect to the node
  after the install.
- `--claim-rooms`: Specify a comma-separated list of tokens for each War Room this node should appear in.
- `--claim-proxy`: Specify a proxy to use when connecting to the cloud in the form of `http://[user:pass@]host:ip` for an HTTP(S) proxy.
  See [connecting through a proxy](/docs/nightly/getting-started/monitor-your-infrastructure/connect-agent-to-cloud#connect-through-a-proxy) for details.
...
If you experience issues while claiming your node, follow the steps in our [Troubleshooting](/docs/nightly/getting-started/monitor-your-infrastructure/connect-agent-to-cloud#troubleshooting) documentation.
...
3. Use the same terminal session to install some of Netdata's prerequisites using Homebrew. If you don't want to use [Netdata Cloud](/docs/nightly/getting-started/monitor-your-infrastructure), you can omit `cmake`.
...
When you're finished with installation, check out [how to monitor your infrastructure](/docs/nightly/getting-started/monitor-your-infrastructure), or skip straight to [configuring the Netdata Agent](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/configuration).
