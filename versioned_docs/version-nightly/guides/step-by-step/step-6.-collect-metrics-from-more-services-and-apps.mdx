...
restart netdata`, or the [appropriate method](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) for your system.
...
Check out the [collectors that come pre-installed with Netdata](/docs/nightly/references/collectors/supported-collectors-list) to find the module for the
...
These modules are primarily written in [Go](/docs/nightly/developers/external-plugis/go.d.plugin) (`go.d`) and
[Python](/docs/nightly/developers/collectors/python.d.plugin), although some use [Bash](/docs/nightly/developers/collectors/charts.d.plugin)
(`charts.d`).
