...
By default, Netdata stores metrics in a custom database we call the [database engine](/docs/nightly/concepts/database-engine), which
...
[**See our database engine calculator**](/docs/nightly/setup/change-how-long-netdata-stores-metrics) to help you correctly set `dbengine disk
...
As we said in [step 1](/docs/nightly/guides/step-by-step/step-1.-netdata-s-building-blocks), we have only complimentary systems, not competitors! We're
...
Next, Netdata needs to be [reinstalled](/docs/nightly/installation/reinstall-the-netdata-agent) in order to detect that the required
...
[Next: Set up a proxy &rarr;](/docs/nightly/guides/step-by-step/step-10.-set-up-a-proxy)
