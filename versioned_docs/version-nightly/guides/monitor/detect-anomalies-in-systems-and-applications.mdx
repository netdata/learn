...
you choose to sign up for Netdata Cloud and [connect your nodes](/docs/nightly/setup/connect-agent-to-cloud), you will have the ability to run
...
- A node running the Netdata Agent. If you don't yet have that, [install Netdata](/docs/nightly/getting-started/install-netdata).
- A Netdata Cloud account. [Sign up](https://app.netdata.cloud) if you don't have one already.
- Familiarity with configuring the Netdata Agent with [`edit-config`](/docs/nightly/setup).
...
Navigate to your [Netdata config directory](/docs/nightly/setup#the-netdata-config-directory) and use `edit-config`
...
[Restart the Agent](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) with `sudo systemctl restart netdata`, or the [appropriate
method](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) for your system, to start up the anomalies collector. By default, the
...
- `train_n_secs`: The number of historical observations to train each model on. The default is 4 hours, but if your node
  ...
  policy](/docs/nightly/setup/change-how-long-netdata-stores-metrics) or reducing this window.
...
[Nginx](/docs/nightly/integrations/monitor/webapps/nginx) and [web
log](/docs/nightly/integrations/monitor/quantify-logs-to-metrics/web-server-logs-apache-nginx-microsoft-iis) collectors.
...
[system](/docs/nightly/concepts/system-metrics), [container](/docs/nightly/concepts/container-metrics), or
[application](/docs/nightly/concepts/application-metrics) metrics you want to detect anomalies for.
...
what to monitor next, check out our list of [collectors](/docs/nightly/integrations/monitor) to see what kind of metrics Netdata
can collect from your systems, containers, and applications.

Keep on moving to [part 2](/docs/nightly/guides/monitor/monitor-and-visualize-anomalies-with-netdata-part-2), which covers the charts and alarms
Netdata creates for unsupervised anomaly detection.

For a different troubleshooting experience, try out the [Metric
Correlations](/docs/nightly/operations/metric-correlations) feature in Netdata Cloud. Metric
...
- [Netdata Agent · Anomalies collector](/docs/nightly/integrations/monitor/anything/anomalies)
- [Netdata Agent · Nginx collector](/docs/nightly/integrations/monitor/webapps/nginx)
...
- [Netdata Cloud · Metric Correlations](/docs/nightly/operations/metric-correlations)
