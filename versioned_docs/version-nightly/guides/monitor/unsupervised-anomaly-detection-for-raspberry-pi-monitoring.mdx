...
collector](/docs/nightly/integrations/monitor/anything/anomalies) on a Raspberry Pi. To our delight, it's actually quite
straightforward! 
...
- The [open-source Netdata](https://github.com/netdata/netdata) monitoring agent. If you don't have it installed on your
  node yet, [get started now](/docs/nightly/getting-started/install-netdata).

## Install dependencies

First make sure Netdata is using Python 3 when it runs Python-based data collectors. 

Next, open `netdata.conf` using [`edit-config`](/docs/nightly/setup#use-edit-config-to-edit-configuration-files)
from within the [Netdata config directory](/docs/nightly/setup#the-netdata-config-directory). Scroll down to the
...
Now you're ready to enable the collector and [restart Netdata](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent).
...
retraining](/docs/nightly/integrations/monitor/anything/anomalies#configuration) to happen less often if you wish.
...
- [_Detect anomalies in systems and applications_](/docs/nightly/guides/monitor/detect-anomalies-in-systems-and-applications)
- [_Monitor and visualize anomalies with Netdata_](/docs/nightly/guides/monitor/monitor-and-visualize-anomalies-with-netdata-part-2)

If you're using your Raspberry Pi for other purposes, like blocking ads/trackers with Pi-hole, check out our companions
Pi guide: [_Monitor Pi-hole (and a Raspberry Pi) with Netdata_](/docs/nightly/guides/monitor/monitor-pi-hole-and-a-raspberry-pi-with-netdata).
...
- [Netdata Agent · Get Netdata](/docs/nightly/getting-started/install-netdata)
