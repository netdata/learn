...
monitoring associated metrics, see [part 1](/docs/nightly/guides/monitor/detect-anomalies-in-systems-and-applications), which covers prerequisites and
...
`anomalies.conf`](/docs/nightly/guides/monitor/detect-anomalies-in-systems-and-applications#configure-the-anomalies-collector). With the `charts_regex`
and `charts_to_exclude` settings from [part 1](/docs/nightly/guides/monitor/detect-anomalies-in-systems-and-applications) of this guide series, the
...
In this case, [configure the alarm](/docs/nightly/setup/configure-health-alarms) with `sudo ./edit-config
health.d/anomalies.conf`. Take a look at the `lookup` line syntax in the [health
reference](/docs/nightly/operations/alerts/health#alarm-line-lookup) to understand how the anomalies collector automatically creates
...
**Anomalies** [section](/docs/nightly/references/local-agent-dashboard#sections) to see the pair of anomaly detection charts, which are
preconfigured to visualize per-second anomaly metrics based on your [configuration in
`anomalies.conf`](/docs/nightly/guides/monitor/detect-anomalies-in-systems-and-applications#configure-the-anomalies-collector).
...
`charts_regex` and `charts_to_exclude` in [part 1](/docs/nightly/guides/monitor/detect-anomalies-in-systems-and-applications).
...
editor](/docs/nightly/operations/netdata-cloud-visualizations/create-new-dashboards) that helps you create entirely new dashboards with charts targeted for
...
investigation using [Metric Correlations](/docs/nightly/operations/metric-correlations) to narrow
the dashboard into only the charts relevant to what you're seeing from the anomalies collector.

## What's next?

Between this guide and [part 1](/docs/nightly/guides/monitor/detect-anomalies-in-systems-and-applications), which covered setup and configuration, you
...
[support through collectors](/docs/nightly/integrations/monitor).
