...
only 512 MiB of RAM, or an IoT device like a [Raspberry Pi](/docs/nightly/guides/monitor/monitor-pi-hole-and-a-raspberry-pi-with-netdata). In these
...
doc](/docs/nightly/setup) before continuing with this guide. This guide assumes familiarity with the Netdata config
...
infrastructure](/docs/nightly/operations/netdata-cloud-visualizations/see-an-overview-of-your-infrastructure) in real-time with Netdata Cloud, or running [Metric
Correlations](/docs/nightly/operations/metric-correlations).

The Netdata Agent runs with the lowest possible [process scheduling
policy](/docs/nightly/references/configuration/netdata-daemon#netdata-process-scheduling-policy), which is `nice 19`, and uses the `idle` process scheduler.
...
every` for an individual collector is less than the global, the Netdata Agent uses the global setting. See the [collectors configuration reference](/docs/nightly/setup/collectors-configuration) for details.

To reduce the frequency of an [internal
plugin/collector](/docs/nightly/concepts/collectors-ref#collector-architecture-and-terminology), open `netdata.conf` and
...
To [configure an individual collector](/docs/nightly/setup/collectors-configuration#configure-a-collector), open its specific configuration file with
...
Reduce the disk space that the [database engine](/docs/nightly/concepts/database-engine) uses to retain metrics by editing
...
A dedicated web server like Nginx provides far more robustness than the Agent's internal [web server](/docs/nightly/developers/web/dashboards).
...
doc](/docs/nightly/setup/expose-local-dashboard-through-proxy/running-netdata-behind-nginx).
...
Now that your Agent is running smoothly, we recommend you [secure your nodes](/docs/nightly/setup) if you haven't
already.
...
-   [Interact with dashboards and charts](/docs/nightly/operations/netdata-cloud-visualizations/interact-with-dashboards-and-charts)
-   [Configure health alarms](/docs/nightly/setup/configure-health-alarms)
-   [Export metrics to external time-series databases](/docs/nightly/concepts/export-metrics-to-external-time-series-databases)
