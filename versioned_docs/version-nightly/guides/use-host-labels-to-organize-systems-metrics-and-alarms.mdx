...
to violate any of the [host label naming rules](/docs/nightly/setup/common-configuration-changes#organize-nodes-with-host-labels).
...
> documentation](/docs/nightly/references/streaming-and-replication#securing-streaming-communications) for details. You may also want to use
> [access lists](/docs/nightly/references/configuration/web-server#access-lists) or [expose the API only to LAN/localhost
> connections](/docs/nightly/setup/security-design#expose-netdata-only-in-a-private-lan).
...
documentation](/docs/nightly/operations/alerts/health#alarm-line-host-labels) for more details, and then get creative!

## Host labels in metrics exporting

If you have enabled any metrics exporting via our experimental [exporters](/docs/nightly/integrations/export), any new host
...
more about exporting, read the [documentation](/docs/nightly/integrations/export).
...
-   [Expose Netdata only in a private LAN](/docs/nightly/setup/security-design#expose-netdata-only-in-a-private-lan)
-   [Enable TLS/SSL for web/API requests](/docs/nightly/references/configuration/web-server#enabling-tls-support)
-   Put Netdata behind a proxy
    -   [Use an authenticating web server in proxy
        mode](/docs/nightly/setup/security-design#use-an-authenticating-web-server-in-proxy-mode)
    -   [Nginx proxy](/docs/nightly/setup/expose-local-dashboard-through-proxy/running-netdata-behind-nginx)
    -   [Apache proxy](/docs/nightly/setup/expose-local-dashboard-through-proxy/netdata-via-apache-s-mod_proxy)
    -   [Lighttpd](/docs/nightly/setup/expose-local-dashboard-through-proxy/netdata-via-lighttpd-v1.4.x)
    -   [Caddy](/docs/nightly/setup/expose-local-dashboard-through-proxy/netdata-via-caddy)
