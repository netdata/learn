...
The Netdata Agent uses [data collectors](/docs/nightly/concepts/collectors-ref) to 
fetch metrics from hundreds of system, container, and service endpoints. While the Netdata team and community has built 
[powerful collectors](/docs/nightly/integrations/monitor) for most system, container, 
...
[installed Netdata from a GitHub fork](/docs/nightly/installation/install-netdata-on-linux-from-a-git-checkout) you
...
[External Plugins Documentation](/docs/nightly/developers/external-plugins).
...
You can read more about `family` and `context` in the [web dashboard](/docs/nightly/developers/web/dashboards#families) doc.
...
[Restart Netdata](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) with `sudo systemctl restart netdata` to see the new humidity
chart:
...
[Restart Netdata](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) with `sudo systemctl restart netdata` to see the new
...
documentation](/docs/nightly/developers/external-plugins/python.d.plugin). 
