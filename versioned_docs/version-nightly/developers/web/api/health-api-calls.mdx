...
The `selection criteria` are key/value pairs, in the format `key : value`, where value is a Netdata [simple pattern](/docs/nightly/developers/libnetdata/simple-patterns). This means that you can create very powerful selectors (you will rarely need more than one or two).
