...
databases](/docs/nightly/exporting-data), or jump in to [enabling a connector](/docs/nightly/exporting-data/enable-an-exporting-connector).
...
restart its process_, not the entire [database of long-term metrics](/docs/nightly/deployment-in-production/metric-retention---database/change-how-long-netdata-stores-metrics).
...
[list of supported databases](/docs/nightly/exporting-data#supported-databases) for information on which
connector to enable and configure for your database of choice.

-   [**AWS Kinesis Data Streams**](/docs/nightly/exporting-data/aws-kinesis-data-streams): Metrics are sent to the service in `JSON`
    format.
-   [**Google Cloud Pub/Sub Service**](/docs/nightly/exporting-data/google-cloud-pub-sub-service): Metrics are sent to the service in `JSON`
    format.
-   [**Graphite**](/docs/nightly/exporting-data/graphite): A plaintext interface. Metrics are sent to the database server as
    ...
    Graphite](/docs/nightly/exporting-data/graphite).
-   [**JSON** document databases](/docs/nightly/exporting-data/json-document-databases)
-   [**OpenTSDB**](/docs/nightly/exporting-data/opentsdb): Use a plaintext or HTTP interfaces. Metrics are sent to
    OpenTSDB as `prefix.chart.dimension` with tag `host=hostname`.
-   [**MongoDB**](/docs/nightly/exporting-data/mongodb): Metrics are sent to the database in `JSON` format.
-   [**Prometheus**](/docs/nightly/exporting-data/using-netdata-with-prometheus): Use an existing Prometheus installation to scrape metrics
    from node using the Netdata API.
-   [**Prometheus remote write**](/docs/nightly/exporting-data/prometheus-remote-write). A binary snappy-compressed protocol
    buffer encoding over HTTP. Supports many [storage
    providers](https://prometheus.io/docs/operating/integrations/#remote-endpoints-and-storage).
-   [**TimescaleDB**](/docs/nightly/exporting-data/writing-metrics-to-timescaledb): Use a community-built connector that takes JSON streams from a
...
[Nginx](/docs/nightly/deployment-in-production/security/reverse-proxies/running-netdata-behind-nginx).
