...
visualize your nodes' metrics. When you [look at charts in Netdata Cloud](/docs/nightly/visualization,-charts-and-dashboards/interact-with-charts)
...
- **Performance**: Every query to a node's database takes only a few milliseconds to complete for responsiveness when
  ...
  like [Metric Correlations](/docs/nightly/troubleshooting-and-machine-learning/metric-correlations).
...
Read our document on changing [how long Netdata stores metrics](/docs/nightly/deployment-in-production/metric-retention---database/change-how-long-netdata-stores-metrics) on your nodes for
details.

You can also stream between nodes using [streaming](/docs/nightly/deployment-in-production/streaming-and-replication), allowing to replicate databases and create
...
TimescaleDB, and many others, see details on [integrating Netdata via exporting](/docs/nightly/exporting-data).
...
To enable certain features, such as [viewing active alarms](/docs/nightly/alerts-and-notifications/view-active-health-alarms)
...
[configuring the database engine](/docs/nightly/deployment-in-production/metric-retention---database/change-how-long-netdata-stores-metrics). Use our calculator to determine the system
