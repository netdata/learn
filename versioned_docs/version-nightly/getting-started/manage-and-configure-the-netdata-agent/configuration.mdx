...
- `netdata.conf` is the main configuration file. This is where you'll find most configuration options. Read descriptions
  for each in the [daemon config](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/daemon-configuration) doc.
...
- Various configuration files ending in `.conf` for [configuring plugins or
  collectors](/docs/nightly/data-collection/collectors-configuration) behave. Examples: `go.d.conf`,
...
- Various directories ending in `.d`, which contain other configuration files, each ending in `.conf`, for [configuring
  specific collectors](/docs/nightly/data-collection/collectors-configuration).
- `apps_groups.conf` is a configuration file for changing how applications/processes are grouped when viewing the
  **Application** charts from [`apps.plugin`](/docs/nightly/data-collection/monitor-anything/system-metrics/application-monitoring) or
  [`ebpf.plugin`](/docs/nightly/data-collection/monitor-anything/system-metrics/kernel-traces-metrics-ebpf).
- `health.d/` is a directory that contains [health configuration files](/docs/nightly/alerts-and-notifications/configure-alerts).
- `health_alarm_notify.conf` enables and configures [alarm notifications](/docs/nightly/alerts-and-notifications/notifications).
- `statsd.d/` is a directory for configuring Netdata's [statsd collector](/docs/nightly/data-collection/monitor-anything/generic-collectors/statsd).
- `stream.conf` configures [parent-child streaming](/docs/nightly/deployment-in-production/streaming-and-replication/streaming-configuration-reference) between separate nodes running the Agent.
- `.environment` is a hidden file that describes the environment in which the Netdata Agent is installed, including the
  `PATH` and any installation options. Useful for [reinstalling](/docs/nightly/installation/reinstall-netdata) or
...
After you make your changes, you need to [restart the Agent](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/start-stop-or-restart-the-netdata-agent) with `sudo systemctl
