...
the hostname/IP address of the remote node. You can also read more about [how the agent dashboard works](/docs/nightly/concepts/how-the-dashboard-works), or dive directly into the many ways
to [interact with charts](/docs/nightly/operations/interact-with-charts).

#### Configuration

Discover the recommended way to [configure Netdata's settings or behavior](/docs/nightly/configuration) using our built-in
`edit-config` script, then apply that knowledge to mission-critical tweaks, such as [changing how long Netdata stores
metrics](/docs/nightly/configuration/change-how-long-netdata-stores-metrics).

#### Data collection

If Netdata didn't autodetect all the hardware, containers, services, or applications running on your node, you should
learn more about [how data collectors work](/docs/nightly/concepts/collectors-ref). If there's a [supported
collector](/docs/nightly/integrations/monitor) for metrics you need, [configure the collector](/docs/nightly/configuration/collectors-configuration)
...
community, but you may want to [edit alarms](/docs/nightly/configuration/health) or 
[enable notifications](/docs/nightly/integrations/notify) to customize your Netdata experience.

#### Make your deployment production ready

Both [securing Netdata](/docs/nightly/configuration/secure-your-nodes) and [setting up replication](/docs/nightly/references/streaming-and-replication) are strongly recommended.
...
[kickstart script](/docs/nightly/installation/install-netdata-with-kickstart.sh).  
...
<InstallRegexLink>
  <InstallBoxRegexLink
    to="[](/docs/nightly/installation/install-netdata-with-docker)"
    os="Run with Docker" 
    svg="docker" />
  <InstallBoxRegexLink
    to="[](/docs/nightly/installation/install-netdata-on-kubernetes)"
    os="Deploy on Kubernetes" 
    svg="kubernetes" />
   <InstallBoxRegexLink
    to="[](/docs/nightly/installation/install-netdata-on-macos)"
    os="Install on macOS" 
    svg="macos" />
  <InstallBoxRegexLink
    to="[](/docs/nightly/installation/install-netdata-on-linux-from-a-git-checkout)"
    os="Linux from Git" 
    svg="linux" />
  <InstallBoxRegexLink
    to="[](/docs/nightly/installation/manually-build-netdata-from-source)"
    os="Linux from source"
    svg="linux" />
  <InstallBoxRegexLink
    to="[](/docs/nightly/installation/install-netdata-on-offline-systems)" 
    os="Linux for offline nodes"
    svg="linux" />
</InstallRegexLink>

- [Run with Docker](/docs/nightly/installation/install-netdata-with-docker)
- [Deploy on Kubernetes](/docs/nightly/installation/install-netdata-on-kubernetes)
- [Install on macOS](/docs/nightly/installation/install-netdata-on-macos)
- [Linux from Git](/docs/nightly/installation/install-netdata-on-linux-from-a-git-checkout)
- [Linux from source](/docs/nightly/installation/manually-build-netdata-from-source)
- [Linux for offline nodes](/docs/nightly/installation/install-netdata-on-offline-systems)
...
Starting with v1.30, Netdata collects anonymous usage information by default and sends it to a self-hosted PostHog instance within the Netdata infrastructure. Read about the information collected, and learn how to-opt, on our [anonymous statistics](/docs/nightly/references/anonymous-statistics) page.
...
PowerTools, to gather hard dependencies. See the [CentOS 6](/docs/nightly/installation/install-netdata-on-linux-from-a-git-checkout#centos--rhel-6x) and
[CentOS 8](/docs/nightly/installation/install-netdata-on-linux-from-a-git-checkout#centos--rhel-8x) sections for more information.
...
errors](/docs/nightly/installation/install-netdata-on-linux-from-a-git-checkout#nonrepresentable-section-on-output-errors) for a workaround.
