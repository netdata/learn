...
Together with [Netdata Cloud](/docs/nightly/getting-started/monitor-your-infrastructure), you can monitor your entire infrastructure in
...
Upon installation completing successfully, you should be able to see the node live in your Netdata Space and live charts in the Overview tab. [Read more about the cloud features](/docs/nightly/getting-started/monitor-your-infrastructure).
...
the hostname/IP address of the remote node. You can also read more about [how the agent dashboard works](/docs/nightly/getting-started/concepts/how-the-dashboard-works), or dive directly into the many ways
to [interact with charts](/docs/nightly/visualization,-charts-and-dashboards/interact-with-charts).

#### Configuration

Discover the recommended way to [configure Netdata's settings or behavior](/docs/nightly/getting-started/configuration) using our built-in
`edit-config` script, then apply that knowledge to mission-critical tweaks, such as [changing how long Netdata stores
metrics](/docs/nightly/deployment-in-production/metric-retention---database/change-how-long-netdata-stores-metrics).

#### Data collection

If Netdata didn't autodetect all the hardware, containers, services, or applications running on your node, you should
learn more about [how data collectors work](/docs/nightly/data-collection---importing-data). If there's a [supported
collector](/docs/nightly/data-collection---importing-data/monitor-anything) for metrics you need, [configure the collector](/docs/nightly/data-collection---importing-data/collectors-configuration)
...
community, but you may want to [edit alarms](/docs/nightly/alerts-and-notifications) or 
[enable notifications](/docs/nightly/alerts-and-notifications/notify) to customize your Netdata experience.

#### Make your deployment production ready

Both [securing Netdata](https://github.com/netdata/netdata/blob/master/docs/configure/secure-nodes.md) and [setting up replication](/docs/nightly/deployment-in-production/streaming-and-replication) are strongly recommended.
...
[kickstart script](/docs/nightly/installation/installation-methods/one-line-installer-kickstart.sh).  
...
<InstallRegexLink>
  <InstallBoxRegexLink
    to="[](/docs/nightly/installation/installation-methods/docker)"
    os="Run with Docker" 
    svg="docker" />
  <InstallBoxRegexLink
    to="[](/docs/nightly/installation/install-on-specific-environments/kubernetes)"
    os="Deploy on Kubernetes" 
    svg="kubernetes" />
   <InstallBoxRegexLink
    to="[](/docs/nightly/installation/install-on-specific-environments/macos)"
    os="Install on macOS" 
    svg="macos" />
  <InstallBoxRegexLink
    to="[](/docs/nightly/installation/installation-methods/from-a-git-checkout)"
    os="Linux from Git" 
    svg="linux" />
  <InstallBoxRegexLink
    to="[](/docs/nightly/installation/package-maintainers/manually-build-netdata-from-source)"
    os="Linux from source"
    svg="linux" />
  <InstallBoxRegexLink
    to="[](/docs/nightly/installation/installation-methods/offline-systems)" 
    os="Linux for offline nodes"
    svg="linux" />
</InstallRegexLink>

- [Run with Docker](/docs/nightly/installation/installation-methods/docker)
- [Deploy on Kubernetes](/docs/nightly/installation/install-on-specific-environments/kubernetes)
- [Install on macOS](/docs/nightly/installation/install-on-specific-environments/macos)
- [Linux from Git](/docs/nightly/installation/installation-methods/from-a-git-checkout)
- [Linux from source](/docs/nightly/installation/package-maintainers/manually-build-netdata-from-source)
- [Linux for offline nodes](/docs/nightly/installation/installation-methods/offline-systems)

The full list of all installation methods for various systems is available in [Netdata Learn](https://learn.netdata.cloud), 
under [Installation](/docs/nightly/installation).
...
Netdata](/docs/nightly/installation/update-the-netdata-agent).
...
PowerTools, to gather hard dependencies. See the [CentOS 6](/docs/nightly/installation/installation-methods/from-a-git-checkout#centos--rhel-6x) and
[CentOS 8](/docs/nightly/installation/installation-methods/from-a-git-checkout#centos--rhel-8x) sections for more information.
...
errors](/docs/nightly/installation/installation-methods/from-a-git-checkout#nonrepresentable-section-on-output-errors) for a workaround.
