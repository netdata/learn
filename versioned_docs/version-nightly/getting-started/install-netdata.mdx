...
Discover the recommended way to [configure Netdata's settings or behavior](/docs/nightly/setup) using our built-in
`edit-config` script, then apply that knowledge to mission-critical tweaks, such as [changing how long Netdata stores
metrics](/docs/nightly/setup/change-how-long-netdata-stores-metrics).
...
collector](/docs/nightly/integrations/monitor) for metrics you need, [configure the collector](/docs/nightly/setup/collectors-configuration)
...
community, but you may want to [edit alarms](/docs/nightly/setup/configure-health-alarms) or 
...
Both [securing Netdata](/docs/nightly/setup/secure-your-nodes) and [setting up replication](/docs/nightly/references/streaming-and-replication) are strongly recommended.
