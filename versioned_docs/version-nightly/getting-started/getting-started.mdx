...
sidebar_position: "1"
description: "Present netdata in a nutshell. The section includes everything needed to have a basic Netdata setup running, including overview of configuration, installing an agent, and cloud basics."
---
...
-   **300+ system, container, and application endpoints**: Collectors autodetect metrics from default endpoints and
    ...
    support](/docs/nightly/data-collection---importing-data/monitor-anything).
-   **20+ notification platforms**: Netdata's health watchdog sends warning and critical alarms to your [favorite
    platform](/docs/nightly/alerts-and-notifications/notify) to inform you of anomalies just seconds
...
-   **30+ external time-series databases**: Export resampled metrics as they're collected to other [local- and
    Cloud-based databases](/docs/nightly/exporting-data) for best-in-class
    interoperability.
...
But the best way to get under the hood or in the steering wheel of our highly efficient, low-latency system (supporting multiple readers and one writer on each metric) is to read the rest of our docs, or just to jump in and [get started](/docs/nightly/getting-started/install-netdata). But here's a good breakdown:
...
Prometheus collector](/docs/nightly/data-collection---importing-data/monitor-anything/generic-collectors/prometheus-endpoints). This even
...
Netdata is an inclusive open-source project and community. Please read our [Code of Conduct](/docs/nightly/improving-netdata---developers/community-code-of-conduct).
...
- Read our [Contributing Guide](/docs/nightly/improving-netdata---developers/contribute), which contains all the information you need to contribute to Netdata, such as improving our documentation, engaging in the community, and developing new features. We've made it as frictionless as possible, but if you need help, just ping us on our community forums!
...
Package maintainers should read the guide on [building Netdata from source](/docs/nightly/installation/package-maintainers/manually-build-netdata-from-source) for
...
[third party licenses](/docs/nightly/improving-netdata---developers/redistributed-software).
