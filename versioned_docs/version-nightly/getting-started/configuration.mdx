...
- `netdata.conf` is the main configuration file. This is where you'll find most configuration options. Read descriptions
  for each in the [daemon config](/docs/nightly/deployment-in-production/common-configuration-changes/daemon-configuration) doc.
...
- Various configuration files ending in `.conf` for [configuring plugins or
  collectors](/docs/nightly/data-collection---importing-data/collectors-configuration) behave. Examples: `go.d.conf`,
...
- Various directories ending in `.d`, which contain other configuration files, each ending in `.conf`, for [configuring
  specific collectors](/docs/nightly/data-collection---importing-data/collectors-configuration).
- `apps_groups.conf` is a configuration file for changing how applications/processes are grouped when viewing the
  **Application** charts from [`apps.plugin`](/docs/nightly/data-collection---importing-data/monitor-anything/system-metrics/application-monitoring) or
  [`ebpf.plugin`](/docs/nightly/data-collection---importing-data/monitor-anything/system-metrics/kernel-traces-metrics-ebpf).
- `health.d/` is a directory that contains [health configuration files](/docs/nightly/alerts-and-notifications).
- `health_alarm_notify.conf` enables and configures [alarm notifications](/docs/nightly/alerts-and-notifications/notify).
- `statsd.d/` is a directory for configuring Netdata's [statsd collector](/docs/nightly/data-collection---importing-data/monitor-anything/generic-collectors/statsd).
...
After you make your changes, you need to [restart the Agent](/docs/nightly/getting-started/start-stop-or-restart-the-netdata-agent) with `sudo systemctl
