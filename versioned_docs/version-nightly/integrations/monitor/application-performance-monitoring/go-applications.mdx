...
directory](/docs/nightly/setup), which is typically at `/etc/netdata`, to edit the `python.d.conf` file.
...
directory](/docs/nightly/setup), which is typically at `/etc/netdata`.
