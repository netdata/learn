...
Edit the `netdata.conf` configuration file using [`edit-config`](/docs/nightly/setup#use-edit-config-to-edit-configuration-files) from the [Netdata config directory](/docs/nightly/setup#the-netdata-config-directory), which is typically at `/etc/netdata`.
