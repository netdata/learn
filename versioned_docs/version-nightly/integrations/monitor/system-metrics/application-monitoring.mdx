...
In addition, if the [eBPF collector](/docs/nightly/integrations/monitor/system-metrics/kernel-traces-metrics-ebpf) is running, your dashboard will also show an
additional [list of charts](/docs/nightly/integrations/monitor/system-metrics/kernel-traces-metrics-ebpf#integration-with-appsplugin) using low-level Linux
metrics.
...
[`ebpf.d.conf`](/docs/nightly/integrations/monitor/system-metrics/kernel-traces-metrics-ebpf#configure-the-ebpf-collector) file to ensure the eBPF program is enabled.

Also see our [guide on troubleshooting apps with eBPF
metrics](/docs/nightly/guides/troubleshoot/monitor-troubleshoot-and-debug-applications-with-ebpf-metrics) for ideas on how to interpret these charts in a
few scenarios.
...
For more information about badges check [Generating Badges](/docs/nightly/developers/web/api/netdata-badges)
