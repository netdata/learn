...
- [K6 load testing tool](https://k6.io)
  ...
  - [Documentation](/docs/nightly/integrations/monitor/apps/k6-load-testing)
...
- [Asterisk](https://www.asterisk.org/)
  ...
  - [Documentation](/docs/nightly/integrations/monitor/voip/asterisk)
...
Private charts are controlled with `create private charts for metrics matching = *`. This setting accepts a space-separated list of [simple patterns](/docs/nightly/developers/libnetdata/simple-patterns). Netdata will create private charts for all metrics **by default**.
...
You can specify Netdata StatsD to have a different `memory mode` than the rest of the Netdata Agent. You can read more about `memory mode` in the [documentation](/docs/nightly/configuration/database).
...
> You can read more about how the Netdata Agent organizes information in the relevant [documentation](/docs/nightly/developers/web/dashboards)
...
-   `metrics` is a Netdata [simple pattern](/docs/nightly/developers/libnetdata/simple-patterns). This pattern should match all the possible StatsD metrics that will be participating in the application `myapp`.
...
`pattern` is a keyword. When set, `METRIC` is expected to be a Netdata [simple pattern](/docs/nightly/developers/libnetdata/simple-patterns) that will be used to match all the StatsD metrics to be added to the chart. So, `pattern` automatically matches any number of StatsD metrics, all of which will be added as separate chart dimensions.
...
- Create a configuration file for your app using [edit-config](/docs/nightly/configuration): `sudo ./edit-config
  statsd.d/myapp.conf`
...
[Netdata config directory](/docs/nightly/configuration#the-netdata-config-directory). 
Use [`edit-config`](/docs/nightly/configuration#use-edit-config-to-edit-configuration-files) 
...
Find more details about family and context in our [documentation](/docs/nightly/developers/web/dashboards#families).
...
- **pattern**: A keyword that tells the StatsD server the `METRIC` string is actually a 
  [simple pattern](/docs/nightly/developers/libnetdata/simple-patterns). 
  ...
  patterns in [dimension patterns](/docs/nightly/integrations/monitor/anything/statsd#dimension-patterns).
...
To enable this StatsD configuration, [restart Netdata](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent).
...
- Make sure you follow the [contributing guide](/docs/nightly/contribute) 
