...
Read more about [Starting, Stopping and Restarting the Agent](/docs/nightly/getting-started/start-stop-or-restart-the-netdata-agent).
...
With the introduction of our new architecture, Agents running versions lower than `v1.32.0` can face claiming problems, so we recommend you [update the Netdata Agent](/docs/nightly/installation/update-the-netdata-agent) to the latest stable version.
