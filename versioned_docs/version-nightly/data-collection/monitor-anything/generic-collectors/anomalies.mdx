...
**Note**: Check out the [Netdata Anomaly Advisor](/docs/nightly/troubleshooting-and-machine-learning/anomaly-advisor) for a more native anomaly detection experience within Netdata.
...
directory](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/configuration), which is usually at `/etc/netdata`.
...
- If you would like to enable this on a Raspberry Pi, then check out [this guide](/docs/nightly/miscellaneous/unsupervised-anomaly-detection-for-raspberry-pi-monitoring) which will guide you through first installing LLVM.
