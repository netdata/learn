...
when you [install Netdata](/docs/nightly/getting-started/install-netdata).
...
For example, the [Nginx collector](/docs/nightly/references/collectors-references/webapps/nginx) searches
...
To quickly find your answer, see our [list of supported collectors](/docs/nightly/references/collectors/supported-collectors-list).

Generally, Netdata's collectors can be grouped into three types:

-   [Systems](/docs/nightly/concepts/system-metrics): Monitor CPU, memory, disk, networking, systemd, eBPF, and much more.
    Every metric exposed by `/proc`, `/sys`, and other Linux kernel sources.
-   [Containers](/docs/nightly/concepts/container-metrics): Gather metrics from container agents, like `dockerd` or `kubectl`,
    along with the resource usage of containers and the applications they run.
-   [Applications](/docs/nightly/concepts/application-metrics): Collect per-second metrics from web servers, databases, logs,
...
-   **Orchestrators** are external plugins that run and manage one or more modules. They run as independent processes.
    ...
    -   [go.d.plugin](/docs/nightly/developers/collectors/go.d.plugin): An orchestrator for data
        collection modules written in `go`.
    -   [python.d.plugin](/docs/nightly/developers/collectors/python.d.plugin): An orchestrator for data collection modules written in
        `python` v2/v3.
    -   [charts.d.plugin](/docs/nightly/developers/collectors/charts.d.plugin): An orchestrator for data collection modules written in
        `bash` v4+.
...
[Enable or configure a collector](/docs/nightly/setup/enable-or-configure-a-collector) if the default settings are not compatible with
your infrastructure.

See our [collectors reference](/docs/nightly/setup/collectors-configuration) for detailed information on Netdata's collector architecture,
