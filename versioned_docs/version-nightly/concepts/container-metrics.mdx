...
Netdata uses [cgroups.plugin](/docs/nightly/references/collectors-references/virtualized-environments/containers/monitor-cgroups) to poll `/sys/fs/cgroup` and convert the raw data
...
[Helmchart](/docs/nightly/installation/deploy-kubernetes-monitoring-with-netdata) to automate deployment, collectors for k8s agent services, and
...
[dockerd](/docs/nightly/references/collectors-references/virtualized-environments/containers/docker) or [Docker
...
[containers/VMs](/docs/nightly/references/collectors/supported-collectors-list#containers-and-vms) and
[Kubernetes](/docs/nightly/references/collectors/supported-collectors-list#containers-and-vms) headings.

## Collect Docker metrics

Netdata has robust Docker monitoring thanks to the aforementioned
[cgroups.plugin](/docs/nightly/references/collectors-references/virtualized-environments/containers/monitor-cgroups). By polling cgroups every second, Netdata can produce meaningful
...
-   A [Kubelet collector](/docs/nightly/references/collectors-references/container-orchestrators/kubernetes/kubelet), which runs
    on each node in a k8s cluster to monitor the number of pods/containers, the volume of operations on each container,
    and more.
-   A [kube-proxy collector](/docs/nightly/references/collectors-references/container-orchestrators/kubernetes/kube-proxy), which
    also runs on each node and monitors latency and the volume of HTTP requests to the proxy.
-   A [cgroups collector](/docs/nightly/references/collectors-references/virtualized-environments/containers/monitor-cgroups), which collects CPU, memory, and bandwidth metrics for
