...
Netdata uses [cgroups.plugin](/docs/nightly/integrations/monitor/virtualized-environments/containers/monitor-cgroups) to poll `/sys/fs/cgroup` and convert the raw data
...
[Helmchart](/docs/nightly/installation/install-netdata-on-kubernetes) to automate deployment, collectors for k8s agent services, and
...
[dockerd](/docs/nightly/integrations/monitor/virtualized-environments/containers/docker) or [Docker
Engine](/docs/nightly/integrations/monitor/virtualized-environments/containers/docker-engine). You can find all
container collectors in our supported collectors list under the
[containers/VMs](/docs/nightly/integrations/monitor#containers-and-vms) and
[Kubernetes](/docs/nightly/integrations/monitor#containers-and-vms) headings.

## Collect Docker metrics

Netdata has robust Docker monitoring thanks to the aforementioned
[cgroups.plugin](/docs/nightly/integrations/monitor/virtualized-environments/containers/monitor-cgroups). By polling cgroups every second, Netdata can produce meaningful
...
collector](/docs/nightly/integrations/monitor/databases/mysql) to look at `127.0.0.0:3306` for
MySQL metrics:
...
metrics](/docs/nightly/integrations/monitor/databases/mysql#charts) as well.

### Collect metrics from applications running in Docker containers

You could use this technique to monitor an entire infrastructure of Docker containers. The same [enable and configure](/docs/nightly/configuration/collectors-configuration) procedures apply whether an application runs on the host system or inside
a container. You may need to configure the target endpoint if it's not the application's default.

Netdata can even [run in a Docker container](/docs/nightly/installation/install-netdata-with-docker) itself, and then collect metrics about the
host system, its own container with cgroups, and any applications you want to monitor.

See our [application metrics doc](/docs/nightly/concepts/application-metrics) for details about Netdata's application metrics
...
-   A [service discovery plugin](https://github.com/netdata/agent-service-discovery), which discovers and creates
    ...
    collector](/docs/nightly/integrations/monitor/extended-metric-collection/prometheus-endpoints). With these
...
-   A [Kubelet collector](/docs/nightly/integrations/monitor/container-orchestrators/kubernetes/kubelet), which runs
    on each node in a k8s cluster to monitor the number of pods/containers, the volume of operations on each container,
    and more.
-   A [kube-proxy collector](/docs/nightly/integrations/monitor/container-orchestrators/kubernetes/kube-proxy), which
    also runs on each node and monitors latency and the volume of HTTP requests to the proxy.
-   A [cgroups collector](/docs/nightly/integrations/monitor/virtualized-environments/containers/monitor-cgroups), which collects CPU, memory, and bandwidth metrics for
...
brokers, and more. See more in the [application metrics doc](/docs/nightly/concepts/application-metrics).

If you already have all the information you need about collecting metrics, move into Netdata's meaningful visualizations
with [seeing an overview of your infrastructure](/docs/nightly/operations/netdata-cloud-visualizations/see-an-overview-of-your-infrastructure) using Netdata Cloud.
