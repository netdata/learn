...
visualize your nodes' metrics. When you [look at charts in Netdata Cloud](/docs/nightly/operations/netdata-cloud-visualizations/interact-with-dashboards-and-charts)
...
- **Performance**: Every query to a node's database takes only a few milliseconds to complete for responsiveness when
  ...
  like [Metric Correlations](/docs/nightly/operations/metric-correlations).
...
Read our document on changing [how long Netdata stores metrics](/docs/nightly/configuration/change-how-long-netdata-stores-metrics) on your nodes for
details.

You can also stream between nodes using [streaming](/docs/nightly/configuration/streaming-and-replication), allowing to replicate databases and create
...
TimescaleDB, and many others, see details on [integrating Netdata via exporting](/docs/nightly/concepts/export-metrics-to-external-time-series-databases).
...
To enable certain features, such as [viewing active alarms](/docs/nightly/operations/alerts/view-active-health-alarms)
...
[configuring the database engine](/docs/nightly/configuration/change-how-long-netdata-stores-metrics). Use our calculator to determine the system
