...
devices, and [containers](/docs/nightly/concepts/container-metrics) with zero configuration.
...
[**proc.plugin**](/docs/nightly/integrations/monitor/system-metrics/os-provided-metrics-proc.plugin) gathers metrics from the `/proc` and `/sys` folders in Linux
...
[**cgroups.plugin**](/docs/nightly/integrations/monitor/virtualized-environments/containers/monitor-cgroups) collects rich metrics about containers and virtual machines
...
[collecting container metrics](/docs/nightly/concepts/container-metrics) doc.

[**ebpf.plugin**](/docs/nightly/integrations/monitor/system-metrics/kernel-traces-metrics-ebpf): Netdata's extended Berkeley Packet Filter (eBPF) collector
...
can find all system collectors in our [supported collectors list](/docs/nightly/integrations/monitor#system-collectors).

## Collect Windows system metrics

Netdata is also capable of monitoring Windows systems. The [WMI
collector](/docs/nightly/integrations/monitor/system-metrics/windows-machines) integrates with
...
details see [the requirements](/docs/nightly/integrations/monitor/system-metrics/windows-machines#requirements).

Next, [configure the WMI
collector](/docs/nightly/integrations/monitor/system-metrics/windows-machines#configuration) to point to the URL
and port of your exposed endpoint. Restart Netdata with `sudo systemctl restart netdata`, or the [appropriate
method](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) for your system. You'll start seeing Windows system metrics, such as CPU
...
doc](/docs/nightly/concepts/application-metrics#collect-metrics-from-applications-running-on-windows).

## What's next?

Because there's some overlap between system metrics and [container metrics](/docs/nightly/concepts/container-metrics), you
should investigate Netdata's container compatibility if you use them heavily in your infrastructure.

If you don't use containers, skip ahead to collecting [application metrics](/docs/nightly/concepts/application-metrics) with
Netdata.
