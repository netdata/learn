...
[dashboard](/docs/nightly/operations/netdata-cloud-visualizations/interact-with-dashboards-and-charts), run health checks to [trigger
alarms](/docs/nightly/operations/alerts/view-active-health-alarms) and [send notifications](/docs/nightly/integrations/notify), and
[export](/docs/nightly/concepts/export-metrics-to-external-time-series-databases) all metrics to an external TSDB. When Netdata streams metrics to another
...
[distributed architecture](/docs/nightly/concepts/distributed-data-architecture) is ideal for speed and scale, streaming
...
between parent and child dashboards, all served by the parent's [web server](/docs/nightly/references/configuration/web-server).
...
streaming](/docs/nightly/configuration/enable-streaming-between-nodes) using a simple `parent-child` relationship. For all
the details, see the [streaming reference](/docs/nightly/references/configuration/streaming-reference) doc.

Take your streaming setup even further by [exporting metrics](/docs/nightly/concepts/export-metrics-to-external-time-series-databases) to an external TSDB.

### Related information & further reading

- Streaming
  - **[How Netdata streams metrics](/docs/nightly/concepts/how-metrics-streaming-works)**
  - [Enable streaming between nodes](/docs/nightly/configuration/enable-streaming-between-nodes)
  - [Streaming reference](/docs/nightly/references/configuration/streaming-reference)
