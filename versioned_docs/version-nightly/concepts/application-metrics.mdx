...
[**apps.plugin**](/docs/nightly/integrations/monitor/system-metrics/application-monitoring) looks at the Linux process tree every second, much like `top` or
...
- [Prometheus endpoints](/docs/nightly/integrations/monitor/extended-metric-collection/prometheus-endpoints): Gathers
  ...
  endpoints.
- [Web server logs (Apache, NGINX)](/docs/nightly/integrations/monitor/quantify-logs-to-metrics/web-server-logs-apache-nginx-microsoft-iis):
...
- [Redis](/docs/nightly/integrations/monitor/databases/redis): Monitor database status by
  reading the server's response to the `INFO` command.
- [Apache](/docs/nightly/integrations/monitor/webapps/apache): Collect Apache web server
  performance metrics via the `server-status?auto` endpoint.
- [Nginx](/docs/nightly/integrations/monitor/webapps/nginx): Monitor web server status
...
- [ElasticSearch](/docs/nightly/integrations/monitor/apps/elasticsearch): Collect search
  engine performance and health statistics. Optionally collects per-index metrics.
- [PHP-FPM](/docs/nightly/integrations/monitor/webapps/php-fpm): Collect application summary
  and processes health metrics by scraping the status page (`/status?full`).

Our [supported collectors list](/docs/nightly/integrations/monitor#service-and-application-collectors) shows all Netdata's
...
Once you have Netdata running on that separate system, you can follow the [collectors configuration reference](/docs/nightly/configuration/collectors-configuration) documentation to tell the collector to look for exposed metrics on the Windows system's IP
...
collector](/docs/nightly/integrations/monitor/databases/mysql) to look at `203.0.113.0:3306`:
...
list](/docs/nightly/integrations/monitor#service-and-application-collectors) that can run on Windows.

## What's next?

If you haven't yet seen the [supported collectors list](/docs/nightly/integrations/monitor) give it a once-over for any
additional applications you may want to monitor using Netdata's native collectors, or the [generic Prometheus
collector](/docs/nightly/integrations/monitor/extended-metric-collection/prometheus-endpoints).
...
infrastructure](/docs/nightly/operations/netdata-cloud-visualizations/see-an-overview-of-your-infrastructure) using Netdata Cloud.
