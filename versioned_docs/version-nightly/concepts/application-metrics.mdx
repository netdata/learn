...
[**apps.plugin**](/docs/nightly/references/collectors-references/system-metrics/application-monitoring) looks at the Linux process tree every second, much like `top` or
...
- [Prometheus endpoints](/docs/nightly/references/collectors-references/extended-metric-collection/prometheus-endpoints): Gathers
  ...
  endpoints.
- [Web server logs (Apache, NGINX)](/docs/nightly/references/collectors-references/quantify-logs-to-metrics/web-server-logs-apache-nginx-microsoft-iis):
...
- [Redis](/docs/nightly/references/collectors-references/databases/redis): Monitor database status by
  reading the server's response to the `INFO` command.
- [Apache](/docs/nightly/references/collectors-references/webapps/apache): Collect Apache web server
  performance metrics via the `server-status?auto` endpoint.
- [Nginx](/docs/nightly/references/collectors-references/webapps/nginx): Monitor web server status
...
- [ElasticSearch](/docs/nightly/references/collectors-references/apps/elasticsearch): Collect search
  engine performance and health statistics. Optionally collects per-index metrics.
- [PHP-FPM](/docs/nightly/references/collectors-references/webapps/php-fpm): Collect application summary
  and processes health metrics by scraping the status page (`/status?full`).

Our [supported collectors list](/docs/nightly/references/collectors/supported-collectors-list#service-and-application-collectors) shows all Netdata's
...
If you haven't yet seen the [supported collectors list](/docs/nightly/references/collectors/supported-collectors-list) give it a once-over for any
