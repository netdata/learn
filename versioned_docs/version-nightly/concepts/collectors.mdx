...
[list of supported collectors](/docs/nightly/integrations/monitor/monitor).
...
[database engine for storage](/docs/nightly/setup/change-how-long-netdata-stores-metrics)
, and immediately 
[visualizes them meaningfully](/docs/nightly/operations/netdata-cloud-visualizations/interact-with-dashboards-and-charts) 
on dashboards.
...
-   **Plugins** help manage all the independent data collection processes in a variety of programming languages, based on 
    ...
    -   **External** plugins organize collectors that gather metrics from external processes, such as a MySQL database or
        ...
        [plugins.d](/docs/nightly/developers/external-plugins/external-plugins), which provides additional management options.

-   **Orchestrators** are external plugins that run and manage one or more modules. They run as independent processes.
    The Go orchestrator is in active development.

    -   [go.d.plugin](/docs/nightly/developers/external-plugins/go.d.plugin/go.d.plugin): An orchestrator for data
        collection modules written in `go`.

    -   [python.d.plugin](/docs/nightly/developers/external-plugins/python.d.plugin/python.d.plugin): 
        An orchestrator for data collection modules written in `python` v2/v3.

    -   [charts.d.plugin](/docs/nightly/developers/external-plugins/charts.d.plugin): 
