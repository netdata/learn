...
While [panning through time and zooming in/out](/docs/nightly/operations/interact-with-charts) from charts it is helpful when
...
At any time, [configure the internal TSDB's storage capacity](/docs/nightly/configuration/change-how-long-netdata-stores-metrics) to expand your
depth of historical metrics.

## What's next?

One useful next step after selecting a timeframe is [exporting the
metrics](/docs/nightly/operations/import-export-and-print-a-snapshot) into a snapshot file, which can then be shared and imported
into any other Netdata dashboard.

There are also many ways to [customize](/docs/nightly/operations/customize-the-standard-dashboard) the standard dashboard experience, from changing
