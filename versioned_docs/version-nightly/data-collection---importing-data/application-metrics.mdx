...
[**apps.plugin**](/docs/nightly/data-collection---importing-data/monitor-anything/system-metrics/application-monitoring) looks at the Linux process tree every second, much like `top` or
...
- [Prometheus endpoints](/docs/nightly/data-collection---importing-data/monitor-anything/generic-collectors/prometheus-endpoints): Gathers
  ...
  endpoints.
- [Web server logs (Apache, NGINX)](/docs/nightly/data-collection---importing-data/monitor-anything/quantify-logs-to-metrics/web-server-logs-apache-nginx-microsoft-iis):
...
- [Redis](/docs/nightly/data-collection---importing-data/monitor-anything/databases/redis): Monitor database status by
  reading the server's response to the `INFO` command.
- [Apache](/docs/nightly/data-collection---importing-data/monitor-anything/webapps/apache): Collect Apache web server
  performance metrics via the `server-status?auto` endpoint.
- [Nginx](/docs/nightly/data-collection---importing-data/monitor-anything/webapps/nginx): Monitor web server status
...
- [ElasticSearch](/docs/nightly/data-collection---importing-data/monitor-anything/apps/elasticsearch): Collect search
  engine performance and health statistics. Optionally collects per-index metrics.
- [PHP-FPM](/docs/nightly/data-collection---importing-data/monitor-anything/webapps/php-fpm): Collect application summary
  and processes health metrics by scraping the status page (`/status?full`).

Our [supported collectors list](/docs/nightly/data-collection---importing-data/monitor-anything#service-and-application-collectors) shows all Netdata's
...
Once you have Netdata running on that separate system, you can follow the [collectors configuration reference](/docs/nightly/data-collection---importing-data/collectors-configuration) documentation to tell the collector to look for exposed metrics on the Windows system's IP
...
collector](/docs/nightly/data-collection---importing-data/monitor-anything/databases/mysql) to look at `203.0.113.0:3306`:
...
list](/docs/nightly/data-collection---importing-data/monitor-anything#service-and-application-collectors) that can run on Windows.

## What's next?

If you haven't yet seen the [supported collectors list](/docs/nightly/data-collection---importing-data/monitor-anything) give it a once-over for any
additional applications you may want to monitor using Netdata's native collectors, or the [generic Prometheus
collector](/docs/nightly/data-collection---importing-data/monitor-anything/generic-collectors/prometheus-endpoints).
...
infrastructure](/docs/nightly/visualization,-charts-and-dashboards/see-an-overview-of-your-infrastructure) using Netdata Cloud.
