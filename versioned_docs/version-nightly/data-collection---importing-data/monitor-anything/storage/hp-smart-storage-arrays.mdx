...
learn_rel_path: "Data collection - importing data/Monitor Anything/Storage"
...
Netdata [config directory](/docs/nightly/getting-started/configuration), which is typically at `/etc/netdata`, to edit the `python.d.conf`
file.
...
restart netdata`, or the [appropriate method](/docs/nightly/getting-started/start-stop-or-restart-the-netdata-agent) for your system.

## Configuration

Edit the `python.d/hpssa.conf` configuration file using `edit-config` from the
Netdata [config directory](/docs/nightly/getting-started/configuration), which is typically at `/etc/netdata`.
