...
collector](/docs/nightly/data-collection---importing-data/monitor-anything/generic-collectors/anomalies) on a Raspberry Pi. To our delight, it's actually quite
straightforward! 
...
Next, open `netdata.conf` using [`edit-config`](/docs/nightly/getting-started/configuration#use-edit-config-to-edit-configuration-files)
from within the [Netdata config directory](/docs/nightly/getting-started/configuration#the-netdata-config-directory). Scroll down to the
...
Now you're ready to enable the collector and [restart Netdata](/docs/nightly/getting-started/start-stop-or-restart-the-netdata-agent).
...
retraining](/docs/nightly/data-collection---importing-data/monitor-anything/generic-collectors/anomalies#configuration) to happen less often if you wish.
