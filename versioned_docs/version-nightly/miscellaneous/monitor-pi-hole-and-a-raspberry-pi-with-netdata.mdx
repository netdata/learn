...
collector](/docs/nightly/data-collection/monitor-anything/apps/pi-hole) will autodetect the new service
...
If you're completely new to Netdata, look at the [Introduction](/docs/nightly/getting-started) section for a walkthrough of all its features. For a more expedited tour, see the [get started documentation](/docs/nightly/getting-started/install-netdata).

### Enable temperature sensor monitoring

You need to manually enable Netdata's built-in [temperature sensor
collector](/docs/nightly/data-collection/monitor-anything/devices/lm-sensors) to start collecting metrics.

> Netdata uses a few plugins to manage its [collectors](/docs/nightly/data-collection/collectors-configuration), each using a different language: Go,
...
engine](/docs/nightly/deployment-in-production/metric-retention---database/database-engine). On the Raspberry Pi used for this guide, Netdata collects 1,500 metrics every
...
and the [Database configuration documentation](/docs/nightly/deployment-in-production/metric-retention---database) to help you determine the right
...
[300+ integrations](/docs/nightly/data-collection/monitor-anything).

-   [Optimize performance](/docs/nightly/deployment-in-production/how-to-optimize-the-netdata-agent-s-performance) using tweaks developed for IoT devices.
-   [Stream Raspberry Pi metrics](/docs/nightly/deployment-in-production/streaming-and-replication/streaming-configuration-reference) to a parent host for easy access or longer-term storage.
-   [Tweak alarms](/docs/nightly/alerts-and-notifications/configure-alerts) for either Pi-hole or the health of your Raspberry Pi.
-   [Export metrics to external databases](/docs/nightly/exporting-data/exporting-reference) with the exporting engine.
