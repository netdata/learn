...
collector](/docs/nightly/integrations/monitor/apps/pi-hole) will autodetect the new service
...
collector](/docs/nightly/integrations/monitor/devices/lm-sensors) to start collecting metrics.

> Netdata uses a few plugins to manage its [collectors](/docs/nightly/configuration/collectors-configuration), each using a different language: Go,
...
engine](/docs/nightly/concepts/database-engine). On the Raspberry Pi used for this guide, Netdata collects 1,500 metrics every
...
and the [Database configuration documentation](/docs/nightly/configuration/database) to help you determine the right
...
[300+ integrations](/docs/nightly/integrations/monitor).

-   [Optimize performance](/docs/nightly/configuration/how-to-optimize-the-netdata-agent-s-performance) using tweaks developed for IoT devices.
-   [Stream Raspberry Pi metrics](/docs/nightly/configuration/streaming-and-replication) to a parent host for easy access or longer-term storage.
-   [Tweak alarms](/docs/nightly/configuration/health) for either Pi-hole or the health of your Raspberry Pi.
-   [Export metrics to external databases](/docs/nightly/integrations/export) with the exporting engine.
