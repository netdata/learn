...
Use the [Overview](/docs/nightly/operations/visualizations/home-overview-and-single-node-view) for monitoring an infrastructure in real time using
composite charts and Netdata's familiar dashboard UI.

Check the [War Room docs](/docs/nightly/operations/war-rooms) for details on the utility bar, which contains the [node
filter](/docs/nightly/operations/war-rooms#node-filter) and the [timeframe
selector](/docs/nightly/operations/war-rooms#play-pause-force-play-and-timeframe-selector).
...
need to configure those nodes to collect from additional data sources. See our [collectors configuration reference](/docs/nightly/configuration/collectors-configuration) 
...
If you want to see up to 30 days of historical metrics in Cloud (and more on individual node dashboards), read about [changing how long Netdata stores metrics](/docs/nightly/configuration/change-how-long-netdata-stores-metrics). Also, see our
...
alarms](/docs/nightly/integrations/notify/cloud-alert-notifications/view-active-alerts) with the Alerts Smartboard.
