...
When you install the Netdata Agent, the [daemon](/docs/nightly/operations/netdata-daemon) is configured to start at boot and stop and
restart/shutdown.
...
The Netdata Agent also comes with a [CLI tool](/docs/nightly/operations/agent-cli) capable of performing shutdowns. Start the Agent back up
