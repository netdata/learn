...
To learn the basics of locating and editing health configuration files, see the [Configure health alarms](/docs/nightly/setup/configure-health-alarms) documentation.
...
-   The `[health]` section in `netdata.conf`. By editing the daemon's behavior, you can disable health monitoring
    ...
    configuration](/docs/nightly/references/configuration/daemon#health-section-options) for a table of all the available settings, their
...
Defines the list of labels present on a host. See our [host labels guide](/docs/nightly/guides/use-host-labels-to-organize-systems-metrics-and-alarms) for
...
api](/docs/nightly/developers/web/api/health-api-calls).
