...
Netdata uses an [internal, static-threaded web server](/docs/nightly/configuration/web-server) to host the HTML, CSS, and JavaScript
files that make up the local Agent dashboard. You don't have to configure anything to access it, although you can adjust
[your settings](/docs/nightly/configuration/web-server#other-netdataconf-web-section-options) in the `netdata.conf` file, or run Netdata
