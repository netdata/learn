...
doc](/docs/nightly/getting-started/configuration) before continuing with this guide. This guide assumes familiarity with the Netdata config
...
infrastructure](/docs/nightly/visualization,-charts-and-dashboards/see-an-overview-of-your-infrastructure) in real-time with Netdata Cloud, or running [Metric
...
policy](/docs/nightly/deployment-in-production/netdata-daemon#netdata-process-scheduling-policy), which is `nice 19`, and uses the `idle` process scheduler.
...
every` for an individual collector is less than the global, the Netdata Agent uses the global setting. See the [collectors configuration reference](/docs/nightly/data-collection---importing-data/collectors-configuration) for details.

To reduce the frequency of an [internal
plugin/collector](/docs/nightly/data-collection---importing-data#collector-architecture-and-terminology), open `netdata.conf` and
...
To [configure an individual collector](/docs/nightly/data-collection---importing-data/collectors-configuration#configure-a-collector), open its specific configuration file with
...
collector](/docs/nightly/data-collection---importing-data#collector-architecture-and-terminology), you can disable any of them.
...
doc](/docs/nightly/deployment-in-production/security/reverse-proxies/running-netdata-behind-nginx).
