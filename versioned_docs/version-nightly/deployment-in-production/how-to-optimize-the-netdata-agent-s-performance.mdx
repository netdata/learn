...
doc](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/configuration) before continuing with this guide. This guide assumes familiarity with the Netdata config
...
infrastructure](/docs/nightly/visualization,-charts-and-dashboards/cloud-visualizations/infrastructure-overview) in real-time with Netdata Cloud, or running [Metric
Correlations](/docs/nightly/troubleshooting-and-machine-learning/metric-correlations).

The Netdata Agent runs with the lowest possible [process scheduling
policy](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/netdata-daemon#netdata-process-scheduling-policy), which is `nice 19`, and uses the `idle` process scheduler.
...
every` for an individual collector is less than the global, the Netdata Agent uses the global setting. See the [collectors configuration reference](/docs/nightly/data-collection/collectors-configuration) for details.

To reduce the frequency of an [internal
plugin/collector](/docs/nightly/data-collection#collector-architecture-and-terminology), open `netdata.conf` and
...
To [configure an individual collector](/docs/nightly/data-collection/collectors-configuration#configure-a-collector), open its specific configuration file with
...
collector](/docs/nightly/data-collection#collector-architecture-and-terminology), you can disable any of them.
...
See how to [change how long Netdata stores metrics](/docs/nightly/deployment-in-production/metric-retention---database/change-how-long-netdata-stores-metrics).
...
A dedicated web server like Nginx provides far more robustness than the Agent's internal [web server](/docs/nightly/improving-netdata---developers/web/dashboards).
...
doc](/docs/nightly/deployment-in-production/secure-your-nodes/reverse-proxies/nginx).
