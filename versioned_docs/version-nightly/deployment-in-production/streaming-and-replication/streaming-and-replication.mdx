...
[data archiving to third party time-series databases](/docs/nightly/exporting-data/exporting-reference).
...
[quick introduction to streaming](/docs/nightly/deployment-in-production/streaming-and-replication)
...
possible, refer to the [performance optimization guide](/docs/nightly/deployment-in-production/how-to-optimize-the-netdata-agent-s-performance).
...
You can [use](/docs/nightly/exporting-data/exporting-reference#configuration) the exporting engine to configure data archiving to an external database (it archives all databases maintained on
this host).
...
`allow from` settings are [Netdata simple patterns](/docs/nightly/improving-netdata---developers/libnetdata/simple-patterns): string matches
...
1. On the parent node (receiving node), [enable TLS support](/docs/nightly/deployment-in-production/secure-your-nodes/web-server#enabling-tls-support). 
...
data consistent, read our [memory mode documentation](/docs/nightly/deployment-in-production/metric-retention---database).
