...
to violate any of the [host label naming rules](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/common-configuration-changes#organize-nodes-with-host-labels).
...
> documentation](/docs/nightly/deployment-in-production/streaming-and-replication/streaming-configuration-reference#securing-streaming-communications) for details. You may also want to use
> [access lists](/docs/nightly/deployment-in-production/secure-your-nodes/web-server#access-lists) or [expose the API only to LAN/localhost
> connections](/docs/nightly/deployment-in-production/secure-your-nodes#expose-netdata-only-in-a-private-lan).
...
documentation](/docs/nightly/alerts-and-notifications/configure-alerts#alarm-line-host-labels) for more details, and then get creative!

## Host labels in metrics exporting

If you have enabled any metrics exporting via our experimental [exporters](/docs/nightly/exporting-data/exporting-reference), any new host
...
more about exporting, read the [documentation](/docs/nightly/exporting-data/exporting-reference).
