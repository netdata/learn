...
learn_rel_path: "Deployment in production/Common configuration changes"
sidebar_position: "44"
...
the [web server access lists](/docs/nightly/deployment-in-production/security/web-server#access-lists).

`netdata.conf` has sections stated with `[section]`. You will see the following sections:

1. `[global]` to [configure](#global-section-options) the [Netdata daemon](/docs/nightly/deployment-in-production/netdata-daemon).
...
5. `[environment variables]` to [configure](#environment-variables-section-options) the environment variables used
   Netdata.
6. `[sqlite]` to [configure](#sqlite-section-options) the [Netdata daemon](/docs/nightly/deployment-in-production/netdata-daemon) SQLite settings.
...
8. `[health]` to [configure](#health-section-options) general settings for [health monitoring](/docs/nightly/getting-started/concepts/health-monitoring).
9. `[web]` to [configure the web server](/docs/nightly/deployment-in-production/security/web-server).
10. `[registry]` for the [Netdata registry](/docs/nightly/deployment-in-production/security/registry).
11. `[global statistics]` for the [Netdata registry](/docs/nightly/deployment-in-production/security/registry).
12. `[statsd]` for the general settings of the [stats.d.plugin](/docs/nightly/data-collection---importing-data/monitor-anything/generic-collectors/statsd).
13. `[plugins]` to [configure](#plugins-section-options) which [collectors](/docs/nightly/data-collection---importing-data) to use and PATH
    settings.
...
After `netdata.conf` has been modified, Netdata needs to be [restarted](/docs/nightly/getting-started/start-stop-or-restart-the-netdata-agent) for
changes to apply:
...
|:-------------------------------------:|:-------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       process scheduling policy       |    `keep`     | See [Netdata process scheduling policy](/docs/nightly/deployment-in-production/netdata-daemon#netdata-process-scheduling-policy)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|               OOM score               |      `0`      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
|  glibc malloc arena max for plugins   |      `1`      | See [Virtual memory](/docs/nightly/deployment-in-production/netdata-daemon#virtual-memory).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|  glibc malloc arena max for Netdata   |      `1`      | See [Virtual memory](/docs/nightly/deployment-in-production/netdata-daemon#virtual-memory).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
|      cleanup obsolete charts after secs       |   `3600`   | See [monitoring ephemeral containers](/docs/nightly/data-collection---importing-data/monitor-anything/virtualized-environments/containers/monitor-cgroups#monitoring-ephemeral-containers), also sets the timeout for cleaning up obsolete dimensions                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
|         delete obsolete charts files          |   `yes`    | See [monitoring ephemeral containers](/docs/nightly/data-collection---importing-data/monitor-anything/virtualized-environments/containers/monitor-cgroups#monitoring-ephemeral-containers), also affects the deletion of files for obsolete dimensions                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
|         log         |                         `/var/log/netdata`                         | The directory in which the [log files](/docs/nightly/deployment-in-production/netdata-daemon#log-files) are kept.                                                                                                                                                     |
...
|      registry       |              `/opt/netdata/var/lib/netdata/registry`               | Contains the [registry](/docs/nightly/deployment-in-production/security/registry) database and GUID that uniquely identifies each Netdata Agent                                                                                                                        |

### [logs] section options

|              setting               |            default            | info                                                                                                                                                                                                                                                                               |
|:----------------------------------:|:-----------------------------:|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|            debug flags             |     `0x0000000000000000`      | Bitmap of debug options to enable. For more information check [Tracing Options](/docs/nightly/deployment-in-production/netdata-daemon#debugging).                                                                                                                                                                      |
|               debug                | `/var/log/netdata/debug.log`  | The filename to save debug information. This file will not be created if debugging is not enabled. You can also set it to `syslog` to send the debug messages to syslog, or `none` to disable this log. For more information check [Tracing Options](/docs/nightly/deployment-in-production/netdata-daemon#debugging). |
...
monitoring](/docs/nightly/getting-started/concepts/health-monitoring).

[Alarm notifications](/docs/nightly/alerts-and-notifications/agent-alert-notifications) are configured in `health_alarm_notify.conf`.
...
Refer to the [web server documentation](/docs/nightly/deployment-in-production/security/web-server)
...
the [registry documentation](/docs/nightly/deployment-in-production/security/registry).
...
Most internal plugins will provide additional options. Check [Internal Plugins](/docs/nightly/data-collection---importing-data) for more
information.
