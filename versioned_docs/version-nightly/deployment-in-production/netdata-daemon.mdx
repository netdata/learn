...
-   You can stop Netdata by killing it with `killall netdata`. You can stop and start Netdata at any point. When
    exiting, the [database engine](/docs/nightly/deployment-in-production/metric-retention---database/database-engine) saves metrics to `/var/cache/netdata/dbengine/` so that
...
You can send commands during runtime via [netdatacli](/docs/nightly/getting-started/agent-cli).
...
The `collector.log` is the `stderr` of all [collectors](/docs/nightly/data-collection---importing-data/monitor-anything)
 run by `netdata`.
...
then execute this to [restart Netdata](/docs/nightly/getting-started/start-stop-or-restart-the-netdata-agent):
