...
Save and close the editor, then [restart your Agent](/docs/nightly/getting-started/start-stop-or-restart-the-netdata-agent) using `sudo systemctl
...
> See the [configuration basics doc](/docs/nightly/getting-started/configuration) for details on how to find `netdata.conf` and use
> `edit-config`.
...
[access lists](/docs/nightly/deployment-in-production/security/web-server#access-lists). This method also fully retains the ability to stream metrics
...
See the [web server](/docs/nightly/deployment-in-production/security/web-server#access-lists) docs for additional details about access lists. You can take
access lists one step further by [enabling SSL](/docs/nightly/deployment-in-production/security/web-server#enabling-tls-support) to encrypt data from local
...
The default configuration uses a public registry under registry.my-netdata.io (more information about the registry here: [mynetdata-menu-item](/docs/nightly/deployment-in-production/security/registry) ). Please be aware that if you use that public registry, you submit the following information to a third party server: 

-   The url where you open the web-ui in the browser (via http request referrer)
-   The hostnames of the Netdata servers

If sending this information to the central Netdata registry violates your security policies, you can configure Netdata to [run your own registry](/docs/nightly/deployment-in-production/security/registry#run-your-own-registry).
