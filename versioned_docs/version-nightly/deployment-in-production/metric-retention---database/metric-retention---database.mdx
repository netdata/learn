...
[Parent - Child](/docs/nightly/deployment-in-production/streaming-and-replication) setups at the children side and only when the
resource constraints are very strict.

So,

- On a single node setup, use `[db].mode = dbengine`.
- On a [Parent - Child](/docs/nightly/deployment-in-production/streaming-and-replication) setup, use `[db].mode = dbengine` on the
...
When having a [**parent - child**](/docs/nightly/deployment-in-production/streaming-and-replication) setup, the child (the
...
You can also disable [data collection plugins](/docs/nightly/data-collection) that you don't need. Disabling such plugins will also
