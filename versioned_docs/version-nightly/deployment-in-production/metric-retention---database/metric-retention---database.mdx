...
[Parent - Child](/docs/nightly/getting-started/how-metrics-streaming-works) setups at the children side and only when the
resource constraints are very strict.

So,

- On a single node setup, use `[db].mode = dbengine`.
- On a [Parent - Child](/docs/nightly/getting-started/how-metrics-streaming-works) setup, use `[db].mode = dbengine` on the
...
When having a [**parent - child**](/docs/nightly/getting-started/how-metrics-streaming-works) setup, the child (the
