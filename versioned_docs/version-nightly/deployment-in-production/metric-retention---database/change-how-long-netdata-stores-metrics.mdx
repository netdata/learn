...
[`dbengine`](/docs/nightly/deployment-in-production/metric-retention---database/database-engine), to store metrics.
...
[dbengine memory requirements](/docs/nightly/deployment-in-production/metric-retention---database/database-engine#memory-requirements).
...
[`edit-config`](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/configuration#use-edit-config-to-edit-configuration-files) 
and make your changes in the `[db]` subsection. 

Save the file and restart the Agent with `sudo systemctl restart netdata`, or
the [appropriate method](/docs/nightly/getting-started/manage-and-configure-the-netdata-agent/start-stop-or-restart-the-netdata-agent) 
