...
[`dbengine`](/docs/nightly/concepts/database-engine), to store metrics.
...
[dbengine memory requirements](/docs/nightly/concepts/database-engine#memory-requirements).
...
[`edit-config`](/docs/nightly/configuration#use-edit-config-to-edit-configuration-files) 
and make your changes in the `[db]` subsection. 

Save the file and restart the Agent with `sudo systemctl restart netdata`, or
the [appropriate method](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) 
