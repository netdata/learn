...
only 512 MiB of RAM, or an IoT device like a [Raspberry Pi](/docs/nightly/miscellaneous/monitor-pi-hole-and-a-raspberry-pi-with-netdata). In these
...
doc](/docs/nightly/configuration) before continuing with this guide. This guide assumes familiarity with the Netdata config
...
every` for an individual collector is less than the global, the Netdata Agent uses the global setting. See the [collectors configuration reference](/docs/nightly/configuration/collectors-configuration) for details.
...
To [configure an individual collector](/docs/nightly/configuration/collectors-configuration#configure-a-collector), open its specific configuration file with
...
Now that your Agent is running smoothly, we recommend you [secure your nodes](/docs/nightly/configuration) if you haven't
already.
