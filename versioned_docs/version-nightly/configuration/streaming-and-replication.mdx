...
[data archiving to third party time-series databases](/docs/nightly/integrations/export).
...
possible, refer to the [performance optimization guide](/docs/nightly/configuration/how-to-optimize-the-netdata-agent-s-performance).
...
You can [use](/docs/nightly/integrations/export#configuration) the exporting engine to configure data archiving to an external database (it archives all databases maintained on
this host).
...
`allow from` settings are [Netdata simple patterns](/docs/nightly/developers/libnetdata/simple-patterns): string matches
...
1. On the parent node (receiving node), [enable TLS support](/docs/nightly/configuration/web-server#enabling-tls-support). 
...
Check the Netdata [exporting documentation](/docs/nightly/concepts/export-metrics-to-external-time-series-databases) for configuring this.
...
data consistent, read our [memory mode documentation](/docs/nightly/configuration/database).
