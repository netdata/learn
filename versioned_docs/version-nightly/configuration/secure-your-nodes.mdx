...
Despite this design decision, your [data](/docs/nightly/configuration/security-design#your-data-is-safe-with-netdata) and your
[systems](/docs/nightly/configuration/security-design#your-systems-are-safe-with-netdata) are safe with Netdata. Netdata is read-only,
...
nodes](/docs/nightly/configuration/security-design#why-netdata-should-be-protected). If left accessible to the internet at large, the
...
You can disable the local dashboard (and API) but retain the encrypted Agent-Cloud link ([ACLK](/docs/nightly/configuration/agent-cloud-link-aclk)) that
...
Save and close the editor, then [restart your Agent](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) using `sudo systemctl
...
> See the [configuration basics doc](/docs/nightly/configuration) for details on how to find `netdata.conf` and use
> `edit-config`.
...
[access lists](/docs/nightly/references/configuration/web-server#access-lists). This method also fully retains the ability to stream metrics
...
patterns](/docs/nightly/developers/libnetdata/simple-patterns).
...
See the [web server](/docs/nightly/references/configuration/web-server#access-lists) docs for additional details about access lists. You can take
access lists one step further by [enabling SSL](/docs/nightly/references/configuration/web-server#enabling-tls-support) to encrypt data from local
...
dedicated to [running Netdata behind Nginx](/docs/nightly/setup/expose-local-dashboard-through-proxy/running-netdata-behind-nginx).

We also have guides for [Apache](/docs/nightly/setup/expose-local-dashboard-through-proxy/netdata-via-apache-s-mod_proxy), [Lighttpd](/docs/nightly/setup/expose-local-dashboard-through-proxy/netdata-via-lighttpd-v1.4.x),
[HAProxy](/docs/nightly/setup/expose-local-dashboard-through-proxy/netdata-via-haproxy), and [Caddy](/docs/nightly/setup/expose-local-dashboard-through-proxy/netdata-via-caddy).

## What's next?

Read about [Netdata's security design](/docs/nightly/configuration/security-design) and our [blog
post](https://www.netdata.cloud/blog/netdata-agent-dashboard/) about why the local Agent dashboard is both open and
secure by design.

Next up, learn about [collectors](/docs/nightly/concepts/collectors-ref) to ensure you're gathering every essential
