...
- `netdata.conf` is the main configuration file. This is where you'll find most configuration options. Read descriptions
  for each in the [daemon config](/docs/nightly/configuration/daemon) doc.
...
- `stream.conf` configures [parent-child streaming](/docs/nightly/configuration/streaming-and-replication) between separate nodes running the Agent.
...
See [configure agent containers](/docs/nightly/installation/installation-methods/docker#configure-agent-containers).
...
- [Netdata Agent · Daemon](/docs/nightly/operations/netdata-daemon)
