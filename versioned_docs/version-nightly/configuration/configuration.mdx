...
installation](/docs/nightly/getting-started/install-netdata#install-on-linux-with-one-line-installer), the **Netdata config
...
- `netdata.conf` is the main configuration file. This is where you'll find most configuration options. Read descriptions
  for each in the [daemon config](/docs/nightly/references/configuration/daemon) doc.
...
- Various configuration files ending in `.conf` for [configuring plugins or
  collectors](/docs/nightly/configuration/collectors-configuration) behave. Examples: `go.d.conf`,
...
- Various directories ending in `.d`, which contain other configuration files, each ending in `.conf`, for [configuring
  specific collectors](/docs/nightly/configuration/collectors-configuration).
- `apps_groups.conf` is a configuration file for changing how applications/processes are grouped when viewing the
  **Application** charts from [`apps.plugin`](/docs/nightly/integrations/monitor/system-metrics/application-monitoring) or
  [`ebpf.plugin`](/docs/nightly/integrations/monitor/system-metrics/kernel-traces-metrics-ebpf).
- `health.d/` is a directory that contains [health configuration files](/docs/nightly/configuration/health).
- `health_alarm_notify.conf` enables and configures [alarm notifications](/docs/nightly/integrations/notify).
- `statsd.d/` is a directory for configuring Netdata's [statsd collector](/docs/nightly/integrations/monitor/anything/statsd).
- `stream.conf` configures [parent-child streaming](/docs/nightly/references/streaming-and-replication) between separate nodes running the Agent.
- `.environment` is a hidden file that describes the environment in which the Netdata Agent is installed, including the
  `PATH` and any installation options. Useful for [reinstalling](/docs/nightly/installation/reinstall-the-netdata-agent) or
  [uninstalling](/docs/nightly/installation/uninstall-netdata) the Agent.
...
See [configure agent containers](/docs/nightly/installation/install-netdata-with-docker#configure-agent-containers).
...
After you make your changes, you need to [restart the Agent](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) with `sudo systemctl
...
Learn more about [starting, stopping, or restarting](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) the Netdata daemon to apply
configuration changes.

Apply some [common configuration changes](/docs/nightly/configuration/common-configuration-changes) to quickly tweak the Agent's behavior.

[Add security to your node](/docs/nightly/configuration/secure-your-nodes) with what you've learned about the Netdata config directory
...
-   [Enable new collectors](/docs/nightly/configuration/collectors-configuration) or tweak their behavior.
-   [Configure existing health alarms](/docs/nightly/configuration/health) or create new ones.
-   [Enable notifications](/docs/nightly/integrations/notify) to receive updates about the health of your
    infrastructure.
-   Change [the long-term metrics retention period](/docs/nightly/configuration/change-how-long-netdata-stores-metrics) using the database engine.

### Related reference documentation

- [Netdata Agent · Daemon](/docs/nightly/references/configuration/netdata-daemon)
- [Netdata Agent · Health monitoring](/docs/nightly/concepts/health-monitoring)
