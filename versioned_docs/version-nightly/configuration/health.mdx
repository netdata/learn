...
-   The `[health]` section in `netdata.conf`. By editing the daemon's behavior, you can disable health monitoring
    ...
    [daemon configuration](/docs/nightly/configuration/daemon#health-section-options) for a table of 
...
You can compile Netdata with [debugging](/docs/nightly/operations/netdata-daemon#debugging) and then set in `netdata.conf`:
