...
You can use health alarms in conjunction with any of Netdata's [collectors](/docs/nightly/concepts/collectors-ref) (see
the [supported collector list](/docs/nightly/integrations/monitor)) to monitor the health of your systems, containers, and
...
-   The `[health]` section in `netdata.conf`. By editing the daemon's behavior, you can disable health monitoring
    ...
    [daemon configuration](/docs/nightly/references/configuration/daemon#health-section-options) for a table of 
...
Navigate to your [Netdata config directory](/docs/nightly/configuration) and
...
To write a new health entity into a new file, navigate to your [Netdata config directory](/docs/nightly/configuration),
...
-   A few lines use space-separated lists to define how the entity behaves. You can use `*` as a wildcard or prefix with
    `!` for a negative match. Order is important, too! See our [simple patterns docs](/docs/nightly/developers/libnetdata/simple-patterns) for
    more examples.
...
list of [simple patterns](/docs/nightly/developers/libnetdata/simple-patterns). For example,
...
list of [simple patterns](/docs/nightly/developers/libnetdata/simple-patterns). For
...
The value is a space-separated list of [simple patterns](/docs/nightly/developers/libnetdata/simple-patterns). For
...
The value is a space-separate list of simple patterns. See our [simple patterns docs](/docs/nightly/developers/libnetdata/simple-patterns) for
some examples.
...
Everything is the same with [badges](/docs/nightly/developers/web/api/netdata-badges). In short:
...
-   `OPTIONS` is a space separated list of `percentage`, `absolute`, `min2max`, `unaligned`,
     `match-ids`, `match-names`. Check the [badges](/docs/nightly/developers/web/api/netdata-badges) documentation for more info.
...
Defines the list of labels present on a host. See our [host labels guide](/docs/nightly/operations/use-host-labels-to-organize-systems-metrics-and-alarms) for
...
See our [simple patterns docs](/docs/nightly/developers/libnetdata/simple-patterns) for more examples.
...
> If you don't know how to find the CHART_NAME, you can read about it [here](/docs/nightly/developers/web/dashboards#charts).
...
templates for charts belonging to a given [context](/docs/nightly/developers/web/dashboards#contexts). The reason is that all charts of a given
context are essentially identical, with the only difference being the [family](/docs/nightly/developers/web/dashboards#families) that
...
You can compile Netdata with [debugging](/docs/nightly/references/configuration/netdata-daemon#debugging) and then set in `netdata.conf`:
...
[health management API](/docs/nightly/developers/web/api/health-api-calls).
...
a [Netdata simple pattern](/docs/nightly/developers/libnetdata/simple-patterns) to create 
...
Use a [simple pattern](/docs/nightly/developers/libnetdata/simple-patterns)! One example of a simple pattern is a single wildcard
(`*`).
...
our [simple patterns documentation](/docs/nightly/developers/libnetdata/simple-patterns).
