...
Netdata uses an [internal, static-threaded web server](/docs/nightly/deployment-in-production/security/web-server) to host the HTML, CSS, and JavaScript
files that make up the local Agent dashboard. You don't have to configure anything to access it, although you can adjust
[your settings](/docs/nightly/deployment-in-production/security/web-server#other-netdataconf-web-section-options) in the `netdata.conf` file, or run Netdata
behind an [Nginx proxy](/docs/nightly/deployment-in-production/security/reverse-proxies/running-netdata-behind-nginx), and so on.
...
For information on creating custom dashboards from scratch, see the [custom dashboards](/docs/nightly/visualization,-charts-and-dashboards/custom-dashboards) or
[Atlassian Confluence dashboards](/docs/nightly/improving-netdata---developers/web/atlassian-confluence-dashboards) guides.
