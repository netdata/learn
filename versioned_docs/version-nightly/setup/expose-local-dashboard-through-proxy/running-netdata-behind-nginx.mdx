...
In case Netdata's web server has been [configured to use TLS](/docs/nightly/references/configuration/web-server#enabling-tls-support), it is
...
`allow connections from` accepts [Netdata simple patterns](/docs/nightly/developers/libnetdata-libraries/simple-patterns) to match against the
