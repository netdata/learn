...
The Netdata Agent uses a custom made time-series database (TSDB), named the [`dbengine`](/docs/nightly/concepts/database-engine), to store metrics.
...
We advise you to visit the [tiering mechanism](/docs/nightly/concepts/database-engine#tiering) reference. This will help you
...
[`edit-config`](/docs/nightly/concepts/configure-the-netdata-agent#use-edit-config-to-edit-configuration-files) and look for the `[db]`
...
the [appropriate method](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) for your system, to change the database engine's size.

## What's next?

If you have multiple nodes with the Netdata Agent installed, you
can [stream metrics](/docs/nightly/concepts/how-metrics-streaming-works) from any number of _child_ nodes to a _
...
with [exporting to other time-series databases](/docs/nightly/concepts/export-metrics-to-external-time-series-databases). With exporting, you can use the
node's resources to surface metrics when [viewing dashboards](/docs/nightly/operations/netdata-cloud-visualizations/interact-with-dashboards-and-charts), while also
archiving metrics elsewhere for further analysis, visualization, or correlation with other tools.

### Related reference documentation

- [Netdata Agent · Database engine](/docs/nightly/concepts/database-engine)
- [Netdata Agent · Database engine configuration option](/docs/nightly/references/configuration/daemon#[db]-section-options)
