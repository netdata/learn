...
You can use health alarms in conjunction with any of Netdata's [collectors](/docs/nightly/concepts/how-netdata-s-metrics-collectors-work) (see
the [supported collector list](/docs/nightly/references/collectors/supported-collectors-list)) to monitor the health of your systems, containers, and
...
[infrastructure](/docs/nightly/possible-old,-not-wanted-documents/infrastructure-monitoring-with-netdata), you must configure each node with the same health configuration
files.

## Edit health configuration files

All of Netdata's [health configuration files](/docs/nightly/operations/alerts/health#health-configuration-files) are in Netdata's config
directory, inside the `health.d/` directory. Navigate to your [Netdata config directory](/docs/nightly/concepts/configure-the-netdata-agent) and
...
Read Netdata's [health reference](/docs/nightly/operations/alerts/health#health-entity-reference) for a full listing of the format,
syntax, and functionality of health entities.

To write a new health entity into a new file, navigate to your [Netdata config directory](/docs/nightly/concepts/configure-the-netdata-agent),
...
-   `warn`/`crit`: The value at which Netdata should trigger a warning or critical alarm. This example uses simple
    ...
    [hysteresis](/docs/nightly/operations/alerts/health#special-use-of-the-conditional-operator) to avoid superfluous notifications.
...
To build complex, dynamic alarms, read our guide on [dimension templates](/docs/nightly/guides/monitor/use-dimension-templates-to-create-dynamic-alarms).
