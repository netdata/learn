...
1. `dbengine`, (the default) data are in database files. The [Database Engine](/docs/nightly/concepts/database-engine) works like a
   ...
   details see [here](/docs/nightly/concepts/database-engine).
...
[Parent - Child](/docs/nightly/concepts/how-metrics-streaming-works) setups at the children side and only when the
resource constraints are very strict.

So,

- On a single node setup, use `[db].mode = dbengine`.
- On a [Parent - Child](/docs/nightly/concepts/how-metrics-streaming-works) setup, use `[db].mode = dbengine` on the
...
When having a [**parent - child**](/docs/nightly/concepts/how-metrics-streaming-works) setup, the child (the
...
You can also disable [data collection plugins](/docs/nightly/references/collectors/plugins-reference) that you don't need. Disabling such plugins will also
