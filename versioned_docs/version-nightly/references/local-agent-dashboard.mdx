...
Netdata uses an [internal, static-threaded web server](/docs/nightly/references/configuration/web-server) to host the HTML, CSS, and JavaScript
files that make up the local Agent dashboard. You don't have to configure anything to access it, although you can adjust
[your settings](/docs/nightly/references/configuration/web-server#other-netdataconf-web-section-options) in the `netdata.conf` file, or run Netdata
behind an [Nginx proxy](/docs/nightly/setup/expose-local-dashboard-through-proxy/running-netdata-behind-nginx), and so on.
...
broken down into [contexts](/docs/nightly/developers/web/dashboards#contexts) and
[families](/docs/nightly/developers/web/dashboards#families).
...
[Increase the metrics retention policy](/docs/nightly/setup/change-how-long-netdata-stores-metrics) for your node to see more historical
timeframes.
...
[families](/docs/nightly/developers/web/dashboards#families) from that section. Netdata automatically
...
document on [using the Agent with Netdata Cloud](/docs/nightly/possible-old,-not-wanted-documents/use-the-agent-with-netdata-cloud).
...
For information on creating custom dashboards from scratch, see the [custom dashboards](/docs/nightly/operations/custom-dashboards) or
[Atlassian Confluence dashboards](/docs/nightly/developers/web/atlassian-confluence-dashboards) guides.
