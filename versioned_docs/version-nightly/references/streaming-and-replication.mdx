...
You can [use](/docs/nightly/references/configuration/exporting-reference#configuration) the exporting engine to configure data archiving to an external database (it archives all databases maintained on
this host).
...
`allow from` settings are [Netdata simple patterns](/docs/nightly/developers/libnetdata-libraries/simple-patterns): string matches
