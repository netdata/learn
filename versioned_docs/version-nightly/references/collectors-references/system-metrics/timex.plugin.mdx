...
Edit the `netdata.conf` configuration file using [`edit-config`](/docs/nightly/concepts/configure-the-netdata-agent#use-edit-config-to-edit-configuration-files) from the [Netdata config directory](/docs/nightly/concepts/configure-the-netdata-agent#the-netdata-config-directory), which is typically at `/etc/netdata`.
