...
In addition, if the [eBPF collector](/docs/nightly/references/collectors-references/system-metrics/kernel-traces-metrics-ebpf) is running, your dashboard will also show an
additional [list of charts](/docs/nightly/references/collectors-references/system-metrics/kernel-traces-metrics-ebpf#integration-with-appsplugin) using low-level Linux
metrics.
...
[`ebpf.d.conf`](/docs/nightly/references/collectors-references/system-metrics/kernel-traces-metrics-ebpf#configure-the-ebpf-collector) file to ensure the eBPF program is enabled.
