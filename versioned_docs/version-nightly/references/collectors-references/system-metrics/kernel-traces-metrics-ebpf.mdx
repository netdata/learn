...
For hands-on configuration and troubleshooting tips see our [tutorial on troubleshooting apps with eBPF metrics](/docs/nightly/guides/troubleshoot/monitor-troubleshoot-and-debug-applications-with-ebpf-metrics).
...
1.  Navigate to the [Netdata config directory](/docs/nightly/concepts/configure-the-netdata-agent#the-netdata-config-directory).
    ```bash
    cd /etc/netdata
    ```

2.  Use the [`edit-config`](/docs/nightly/concepts/configure-the-netdata-agent#use-edit-config-to-edit-configuration-files) script to edit `netdata.conf`.
...
1.  Navigate to the [Netdata config directory](/docs/nightly/concepts/configure-the-netdata-agent#the-netdata-config-directory).
    ...
    cd /etc/netdata
    ```
2.  Use the [`edit-config`](/docs/nightly/concepts/configure-the-netdata-agent#use-edit-config-to-edit-configuration-files) script to edit [`ebpf.d.conf`](https://github.com/netdata/netdata/blob/master/collectors/ebpf.plugin/ebpf.d.conf).
...
[`apps.plugin`](/docs/nightly/references/collectors-references/system-metrics/application-monitoring). This integration helps you understand how specific applications
...
[`cgroups.plugin`](/docs/nightly/references/collectors-references/virtualized-environments/containers/monitor-cgroups). This integration helps you understand how a specific cgroup
...
-   `cachestat`: Netdata's eBPF data collector creates charts about the memory page cache. When the integration with
    [`apps.plugin`](/docs/nightly/references/collectors-references/system-metrics/application-monitoring) is enabled, this collector creates charts for the whole host _and_
...
1.  Navigate to the [Netdata config directory](/docs/nightly/concepts/configure-the-netdata-agent#the-netdata-config-directory).
    ...
    cd /etc/netdata
    ```
2.  Use the [`edit-config`](/docs/nightly/concepts/configure-the-netdata-agent#use-edit-config-to-edit-configuration-files) script to edit a thread configuration file. The following configuration files are available:
...
[simple patterns](/docs/nightly/developers/libnetdata-libraries/simple-patterns). The `ports`, and `ips` settings accept negation (`!`) to deny
...
[restart Netdata](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) with `sudo systemctl restart netdata` to see if system memory
