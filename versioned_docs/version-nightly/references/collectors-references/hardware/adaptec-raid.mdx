...
Netdata [config directory](/docs/nightly/concepts/configure-the-netdata-agent), which is typically at `/etc/netdata`, to edit the `python.d.conf`
file.
...
systemctl restart netdata`, or the [appropriate method](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent) for your system.

## Configuration

Edit the `python.d/adaptec_raid.conf` configuration file using `edit-config` from the
Netdata [config directory](/docs/nightly/concepts/configure-the-netdata-agent), which is typically at `/etc/netdata`.
