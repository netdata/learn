...
-   You can stop Netdata by killing it with `killall netdata`. You can stop and start Netdata at any point. When
    exiting, the [database engine](/docs/nightly/concepts/database-engine) saves metrics to `/var/cache/netdata/dbengine/` so that
...
You can send commands during runtime via [netdatacli](/docs/nightly/references/agent-cli).
...
The `collector.log` is the `stderr` of all [collectors](/docs/nightly/references/collectors/supported-collectors-list)
 run by `netdata`.
...
then execute this to [restart Netdata](/docs/nightly/operations/start-stop-or-restart-the-netdata-agent):
