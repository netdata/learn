...
databases](https://github.com/netdata/netdata/blob/master/docs/export/external-databases.md), or jump in to [enabling a connector](/docs/nightly/setup/enable-an-exporting-connector).
...
restart its process_, not the entire [database of long-term metrics](/docs/nightly/setup/change-how-long-netdata-stores-metrics).
...
[list of supported databases](/docs/nightly/concepts/export-metrics-to-external-time-series-databases#supported-databases) for information on which
connector to enable and configure for your database of choice.

-   [**AWS Kinesis Data Streams**](/docs/nightly/setup/exporting-connectors/aws-kinesis-data-streams): Metrics are sent to the service in `JSON`
    format.
-   [**Google Cloud Pub/Sub Service**](/docs/nightly/setup/exporting-connectors/google-cloud-pub-sub-service): Metrics are sent to the service in `JSON`
    format.
-   [**Graphite**](/docs/nightly/setup/exporting-connectors/graphite): A plaintext interface. Metrics are sent to the database server as
...
-   [**JSON** document databases](/docs/nightly/setup/exporting-connectors/json-document-databases)
-   [**OpenTSDB**](/docs/nightly/setup/exporting-connectors/opentsdb): Use a plaintext or HTTP interfaces. Metrics are sent to
    OpenTSDB as `prefix.chart.dimension` with tag `host=hostname`.
-   [**MongoDB**](/docs/nightly/setup/exporting-connectors/mongodb): Metrics are sent to the database in `JSON` format.
-   [**Prometheus**](/docs/nightly/setup/exporting-connectors/using-netdata-with-prometheus): Use an existing Prometheus installation to scrape metrics
    from node using the Netdata API.
-   [**Prometheus remote write**](/docs/nightly/setup/exporting-connectors/prometheus-remote-write). A binary snappy-compressed protocol
    buffer encoding over HTTP. Supports many [storage
    providers](https://prometheus.io/docs/operating/integrations/#remote-endpoints-and-storage).
-   [**TimescaleDB**](/docs/nightly/setup/exporting-connectors/writing-metrics-to-timescaledb): Use a community-built connector that takes JSON streams from a
...
[Nginx](/docs/nightly/setup/expose-local-dashboard-through-proxy/running-netdata-behind-nginx).
