...
13. `[plugins]` to [configure](#plugins-section-options) which [collectors](/docs/nightly/concepts/collectors) to use and PATH
    settings.
...
[Alarm notifications](/docs/nightly/integrations/notify/agent-alert-notifications/agent-alert-notifications) are configured in `health_alarm_notify.conf`.
...
Most internal plugins will provide additional options. Check [Internal Plugins](/docs/nightly/concepts/collectors) for more
information.
