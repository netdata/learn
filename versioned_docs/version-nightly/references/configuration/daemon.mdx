...
7. `[ml]` to configure settings for [machine learning](/docs/nightly/setup/configure-machine-learning-ml-powered-anomaly-detection).
...
12. `[statsd]` for the general settings of the [stats.d.plugin](/docs/nightly/references/collectors-references/apm/statsd.plugin).
13. `[plugins]` to [configure](#plugins-section-options) which [collectors](/docs/nightly/references/collectors/plugins-reference) to use and PATH
    settings.
...
|                   retention                   |   `3600`   | Used with `mode = save/map/ram/alloc`, not the default `mode = dbengine`. This number reflects the number of entries the `netdata` daemon will by default keep in memory for each chart dimension. Check [Memory Requirements](/docs/nightly/setup/database) for more information.                                                                                                                                                                                                                                                                                                                                                           |
...
|          memory deduplication (ksm)           |   `yes`    | When set to `yes`, Netdata will offer its in-memory round robin database and the dbengine page cache to kernel same page merging (KSM) for deduplication. For more information check [Memory Deduplication - Kernel Same Page Merging - KSM](/docs/nightly/setup/database#ksm)                                                                                                                                                                                                                                                                                                                                                               |
|      cleanup obsolete charts after secs       |   `3600`   | See [monitoring ephemeral containers](/docs/nightly/references/collectors-references/virtualized-environments/containers/monitor-cgroups#monitoring-ephemeral-containers), also sets the timeout for cleaning up obsolete dimensions                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
|         delete obsolete charts files          |   `yes`    | See [monitoring ephemeral containers](/docs/nightly/references/collectors-references/virtualized-environments/containers/monitor-cgroups#monitoring-ephemeral-containers), also affects the deletion of files for obsolete dimensions                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
[Alarm notifications](/docs/nightly/operations/alerts/notifications-reference) are configured in `health_alarm_notify.conf`.
...
|                enabled alarms                  |                       *                          | Defines which alarms to load from both user and stock directories. This is a [simple pattern](/docs/nightly/developers/libnetdata-libraries/simple-patterns) list of alarm or template names. Can be used to disable specific alarms. For example, `enabled alarms =  !oom_kill *` will load all alarms except `oom_kill`. |
...
Most internal plugins will provide additional options. Check [Internal Plugins](/docs/nightly/references/collectors/plugins-reference) for more
information.
...
|     setting     |                   default                    | info                                                                                                                                                  |
|:---------------:|:--------------------------------------------:|:------------------------------------------------------------------------------------------------------------------------------------------------------|
|  update every   | the value of `[global].update every` setting | The frequency in seconds the plugin should collect values. For more information check the [performance guide](/docs/nightly/guides/how-to-optimize-the-netdata-agent-s-performance). |
