...
To do both, you will need to create (or modify) alarm configuration files. Our [documentation](/docs/monitor/configure-alarms) details the process. To develop and test the alarm, you will only need an active installation of the Netdata Agent, so the developer container described above is not required.
...
We expect all contributors to abide by the [Contributor Covenant Code of Conduct](/contribute/code-of-conduct). For a pull request to be accepted, you will also need to accept the [Netdata contributors license agreement](/docs/agent/contribute/contributors), as part of the PR process.
