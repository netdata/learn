    ...
    for file in file_array:
        if Path(file).is_file():
            ...
            if "most_popular: True" in content:
                most_popular.append([str(file).lower().rsplit("/",1)[1], file, "#00ab44"])
            elif "maintained%20by-Netdata-" in content:
                rest_netdata_integrations.append([str(file).lower().rsplit("/",1)[1], file, "#00ab44"])
            else:
                community_integrations.append([str(file).lower().rsplit("/",1)[1], file, ""])

    sorted_array = sorted(most_popular) + sorted(rest_netdata_integrations) + sorted(community_integrations)
...
def get_dir_make_file_and_recurse(directory):
    ...
    if len(sorted(Path(directory).glob("**/**/*"))) > 1:
        ...
        try:
            sidebar_position = re.search(r'sidebar_position:.*', Path(sorted_list[0][1]).read_text())[0]
        except TypeError:
            sidebar_position = ""
...
# print(Path('data-collection').glob("*"))
...
get_dir_make_file_and_recurse('docs/alerting/notifications')
