...
"""
        ...
        for file_array_entry in sorted_list:
            ...
            if Path(file).is_file():
                ...
                if "DO NOT EDIT THIS FILE DIRECTLY" in whole_file and (str(directory).split("/")[-1] != str(file).replace(".mdx", "").split("/")[-1]):
                    print(str(directory).split("/")
                          [-1], str(directory).split("/")[-2], file)
                    meta_dict = ingest_func.read_metadata(whole_file)

                    if meta_dict["learn_link"].split("/")[-1] == meta_dict["learn_link"].split("/")[-1]:
