...
"""
        ...
        for file_array_entry in sorted_list:
            ...
            if Path(file).is_file():
                ...
                if "DO NOT EDIT THIS FILE DIRECTLY" in whole_file and (str(file).split("/")[-2] != str(file).replace(".mdx", "").split("/")[-1]):
                    # print(str(file).split("/")[-2] != str(file).replace(".mdx", "").split("/")[-1], file)
                    meta_dict = ingest_func.read_metadata(whole_file)

                    if meta_dict["learn_link"].split("/")[-1] == meta_dict["learn_link"].split("/")[-2]:
                        meta_dict["learn_link"] = meta_dict["learn_link"].rsplit(
                            "/", 1)[0]
                        # print(type(meta_dict["learn_link"]))
