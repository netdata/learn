...
def populate_integrations(markdownFiles):
    ...
    for file in markdownFiles:
        ...
        if whole_file not in ignore_dup:
            ...
            if "DO NOT EDIT THIS FILE DIRECTLY" in whole_file:
                ...
                if os.path.islink(file):
                    ...
                    if not file.replace("ingest-temp-folder/", "").split('/',1)[1] in metadata_dictionary['custom_edit_url']:
                        ...
                        proper_edit_url = "https://github.com/netdata/" + proper_edit_url.split('/',1)[0] + "/edit/master/" + proper_edit_url.split('/',1)[1]
    ...
    map_file = pd.concat([upper, collectors_entries.sort_values(by=['sidebar_label'], key=lambda col: col.str.lower()), lower], ignore_index=True)
    ...
    map_file = pd.concat([upper, alerting_agent.sort_values(by=['sidebar_label'], key=lambda col: col.str.lower()), lower], ignore_index=True)

    replace_index = map_file.loc[map_file['custom_edit_url'] == "cloud_notifications_integrations"].index
    upper = map_file.iloc[:replace_index[0]]
    lower = map_file.iloc[replace_index[0]+1:]

    map_file = pd.concat([upper, alerting_cloud.sort_values(by=['sidebar_label'], key=lambda col: col.str.lower()), lower], ignore_index=True)
    ...
    map_file = pd.concat([upper, exporting_entries.sort_values(by=['sidebar_label'], key=lambda col: col.str.lower()), lower], ignore_index=True)
