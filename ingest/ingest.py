...
def load_map_yaml(map_path):
    ...
    def add_meta(meta):
        if not isinstance(meta, dict):
            return
        rows.append({
            "custom_edit_url": meta.get("edit_url"),
            "sidebar_label": meta.get("label"),
            "learn_status": meta.get("status"),
            "learn_rel_path": meta.get("path"),
...
def populate_integrations(markdownFiles):
    ...
    with open("ingest/generated_map.yaml", "w", encoding="utf-8") as fh:
        yaml.dump(generated_map_data, fh, default_flow_style=False, allow_unicode=True, sort_keys=False)
...
def update_metadata_of_file(path_to_file, dictionary):
    ...
    for field in dictionary:
        ...
        if field in ['description', 'keywords']:
            if val is None or val == '' or val == 'None' or (isinstance(val, list) and len(val) == 0):
                continue
...
def get_dir_make_file_and_recurse(directory):
    ...
    - Integrations outnumber non-integration files (or all integrations are in subdirs)
                ...
                if INTEGRATION_MARKER in whole_file:
                    ...
                    if direct_child and direct_integrations == 1:
                        ...
                        if is_published or has_custom_url:
                            ...
                            if len(direct_files) == 1 and direct_files[0].stem == Path(directory).name:
                    ...
                    try:
                        box_to = meta_dict["learn_link"].replace("https://learn.netdata.cloud", "")
...
if __name__ == '__main__':
            ...
            if repo_str.startswith('/') or ':/' in repo_str:
                ...
                if ':' in repo_str and not repo_str.startswith('/'):
                    ...
                    try:
                        ...
                        print(f"ERROR: Invalid format '{repo_str}'. Expected repo_name:/path/to/repo")
                        exit(-1)
                else:
                    ...
                    if dir_name in default_repos:
                        ...
                        for known_repo in default_repos.keys():
                            if known_repo.lower() in dir_name.lower() or dir_name.lower() in known_repo.lower():
                                matched_repo = known_repo
                                break
                    
                    if not matched_repo:
                        print(f"ERROR: Could not map directory '{dir_name}' to a known repository.")
                        print(f"Known repos: {list(default_repos.keys())}")
                        print(f"Either use explicit format 'repo_name:{local_path}' or name your directory after a known repo.")
                        exit(-1)
                    
                    LOCAL_REPOS[matched_repo] = local_path
                    print(f"Using local directory for {matched_repo}: {LOCAL_REPOS[matched_repo]}")
            else:
    ...
    with open("./ingest/one_commit_back_file-dict.yaml", "w", encoding="utf-8") as fh:
        yaml.dump(redirect_data, fh, default_flow_style=False, allow_unicode=True, sort_keys=False)
