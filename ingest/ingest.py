...
REDIRECT_GATE_EXIT_CODE = 3
SIDEBAR_ORDER_STATE_PATH = Path(__file__).resolve().with_name(
    "generated_sidebar_order.json"
)
KICKSTART_CHECKSUM_PLACEHOLDER = "@KICKSTART_CHECKSUM@"
KICKSTART_CHECKSUM_DOC = Path(
    "Netdata Agent/Installation/Linux/Linux.mdx"
)
KICKSTART_SCRIPT_PATH = Path("packaging/installer/kickstart.sh")
...
def apply_kickstart_checksum(docs_root, checksum):
    """Write the supplied kickstart checksum into the ingested document."""
    if (
        not isinstance(checksum, str)
        or re.fullmatch(r"[0-9a-fA-F]{32}", checksum) is None
    ):
        raise ValueError(
            "Kickstart checksum must be exactly 32 hexadecimal characters"
        )

    docs_root = _require_regular_directory(docs_root)
    document = _validate_path_under_root(
        docs_root, docs_root / KICKSTART_CHECKSUM_DOC, expected="file"
    )
    content = _read_regular_text(document)
    occurrences = content.count(KICKSTART_CHECKSUM_PLACEHOLDER)
    if occurrences != 1:
        raise ValueError(
            "Expected exactly one kickstart checksum placeholder in "
            f"{document}, found {occurrences}"
        )

    updated = content.replace(
        KICKSTART_CHECKSUM_PLACEHOLDER, checksum.lower(), 1
    )
    _atomic_write_text(document, updated, docs_root=docs_root)


def resolve_kickstart_checksum(checksum, regenerate_grids_only, local_repos):
    """Resolve the checksum without changing the selected source checkout."""
    if regenerate_grids_only:
        return checksum
    ...
    local_netdata = local_repos.get("netdata")
    if local_netdata is None:
        raise ValueError(
            "Full ingest requires --kickstart-checksum with exactly 32 hexadecimal "
            "characters unless an explicit local netdata checkout is selected"
        )

    repository_root = _require_regular_directory(local_netdata)
    script = _validate_path_under_root(
        repository_root,
        repository_root / KICKSTART_SCRIPT_PATH,
        expected="file",
    )
    return hashlib.md5(
        _read_regular_bytes(script), usedforsecurity=False
    ).hexdigest()
...
def write_sidebar_order_state(
    ...
    entries = [
        ...
        for (parent_path, child_name), position in sorted(
            order_map.items(), key=lambda item: (item[0][0], item[1], item[0][1])
        )
    ]
    payload = {
        "schema_version": 1,
...
def _integration_source_identity(custom_edit_url):
...
def _integration_source_suffix(custom_edit_url):
    source = _integration_source_identity(custom_edit_url)
    return re.sub(r"[^a-z0-9]+", "-", source.lower()).strip("-")


def _generated_integration_seo_title(metadata):
    label = " ".join(str(metadata.get("sidebar_label", "")).split())
    source = _integration_source_identity(metadata.get("custom_edit_url", ""))
...
def resolve_publish_path_collisions(publish_map):
    """
    Keep existing routes for non-colliding pages and disambiguate integration duplicates.

    Collision members get deterministic routes and explicit SEO titles based on
    source identity. Any collision that cannot be made unambiguous fails ingest.
    """
    by_path = {}
    by_slug = {}
    ...
    for md_files in collision_groups:
        ...
        for md_file in md_files:
            ...
            seo_title = _generated_integration_seo_title(metadata)
            if not seo_title:
                raise ValueError(
                    f"Cannot derive generated integration SEO title for {md_file}: "
                    f"{custom_edit_url}"
                )
            title_identity = seo_title.casefold()
            if title_identity in seo_titles:
                raise ValueError(
                    "Duplicate generated integration SEO title after source "
                    f"disambiguation: {seo_title}\n"
                    f"  {seo_titles[title_identity]}\n"
                    f"  {md_file}"
                )
            seo_titles[title_identity] = md_file
...
def _relative_luminance(rgb):
    r, g, b = rgb
    rl = _srgb_to_linear(max(0.0, min(1.0, r)))
    gl = _srgb_to_linear(max(0.0, min(1.0, g)))
    bl = _srgb_to_linear(max(0.0, min(1.0, b)))
...
def _blend_rgb_with_background(rgb, alpha, bg_rgb):
    a = max(0.0, min(1.0, alpha))
    return (
        rgb[0] * a + bg_rgb[0] * (1.0 - a),
        rgb[1] * a + bg_rgb[1] * (1.0 - a),
        rgb[2] * a + bg_rgb[2] * (1.0 - a),
    )
        ...
        try:
            ...
            if len(parts) >= 4:
                ...
                if alpha_token.endswith("%"):
                    a = max(0.0, min(1.0, float(alpha_token[:-1]) / 100.0))
                else:
                    a = max(0.0, min(1.0, float(alpha_token)))
            return (r, g, b, a)
        except ValueError:
            return None
...
def _analyze_svg_luminance(content):
    try:
        ...
        if style_attr:
            for declaration in style_attr.split(";"):
                if ":" in declaration:
                    ...
                    style_map[key.strip().lower()] = val.strip()

        opacity = element.attrib.get("opacity", style_map.get("opacity", "1"))
        opacity_value = max(0.0, min(1.0, _safe_float(opacity, 1.0)))

        for paint_key, default_weight in (("fill", 1.0), ("stroke", 0.65)):
            paint = element.attrib.get(paint_key, style_map.get(paint_key, ""))
            parsed = _parse_css_color(paint)
            if not parsed:
                continue

            local_opacity_attr = f"{paint_key}-opacity"
            local_opacity = element.attrib.get(
                local_opacity_attr,
                style_map.get(local_opacity_attr, "1"),
            )
            local_opacity_value = max(0.0, min(1.0, _safe_float(local_opacity, 1.0)))

            r, g, b, color_alpha = parsed
            final_alpha = opacity_value * local_opacity_value * color_alpha
            if final_alpha <= 0.01:
                continue

            rgb = (r, g, b)
            light_luminance = _relative_luminance(
                _blend_rgb_with_background(rgb, final_alpha, LIGHT_THEME_BG_RGB)
            )
            dark_luminance = _relative_luminance(
                _blend_rgb_with_background(rgb, final_alpha, DARK_THEME_BG_RGB)
            )

            weight = max(0.05, final_alpha) * default_weight
            weighted_light += light_luminance * weight
            weighted_dark += dark_luminance * weight
            total_weight += weight
    ...
    return {
        "light": weighted_light / total_weight,
        "dark": weighted_dark / total_weight,
        "confidence": "medium",
    }


def _analyze_raster_luminance(content):
    if Image is None:
        return None
    ...
    img.thumbnail((LOGO_RASTER_SAMPLE_SIZE, LOGO_RASTER_SAMPLE_SIZE))
    pixels = img.getdata()
    ...
    for r, g, b, alpha in pixels:
        if alpha < LOGO_ALPHA_MIN:
            continue

        rgb = (r / 255.0, g / 255.0, b / 255.0)
        a = alpha / 255.0
        light_luminance = _relative_luminance(
            _blend_rgb_with_background(rgb, a, LIGHT_THEME_BG_RGB)
        )
        dark_luminance = _relative_luminance(
            _blend_rgb_with_background(rgb, a, DARK_THEME_BG_RGB)
        )
...
def _classify_logo_by_luminance(luminance_info):
    if not luminance_info:
        return {"light": "unknown", "dark": "unknown", "confidence": "low"}

    bg_light_l = _relative_luminance(LIGHT_THEME_BG_RGB)
    bg_dark_l = _relative_luminance(DARK_THEME_BG_RGB)
    light_contrast = _contrast_ratio(luminance_info["light"], bg_light_l)
    dark_contrast = _contrast_ratio(luminance_info["dark"], bg_dark_l)

    return {
        "light": "low" if light_contrast < LOGO_CONTRAST_THRESHOLD else "ok",
        "dark": "low" if dark_contrast < LOGO_CONTRAST_THRESHOLD else "ok",
        "confidence": luminance_info.get("confidence", "medium"),
    }


def _analyze_remote_logo(url):
    if url in LOGO_CACHE:
        return LOGO_CACHE[url]

    result = {"light": "unknown", "dark": "unknown", "confidence": "low"}

    try:
        response = requests.get(
            url,
            timeout=LOGO_ANALYSIS_TIMEOUT,
            headers={"User-Agent": "netdata-learn-ingest-logo-analyzer/1.0"},
        )
        response.raise_for_status()
        content_type = (response.headers.get("Content-Type") or "").lower()
        is_svg = url.lower().split("?", 1)[0].endswith(".svg") or "svg" in content_type

        if is_svg:
            svg_info = _analyze_svg_luminance(response.text)
            result = _classify_logo_by_luminance(svg_info)
        else:
            raster_info = _analyze_raster_luminance(response.content)
            result = _classify_logo_by_luminance(raster_info)
    except Exception:
        result = {"light": "unknown", "dark": "unknown", "confidence": "low"}

    LOGO_CACHE[url] = result
    LOGO_ANALYSIS_SUMMARY["analyzed"] += 1
    if result["light"] == "low":
        LOGO_ANALYSIS_SUMMARY["low_light"] += 1
    if result["dark"] == "low":
        LOGO_ANALYSIS_SUMMARY["low_dark"] += 1
    if result["light"] == "unknown" and result["dark"] == "unknown":
        LOGO_ANALYSIS_SUMMARY["unknown"] += 1
...
def _set_html_attr(tag, attr_name, attr_value):
    ...
    if attr_regex.search(tag):
        return attr_regex.sub(f' {attr_name}="{attr_value}"', tag, count=1)

    closing = "/>" if tag.rstrip().endswith("/>") else ">"
    insert_idx = tag.rfind(closing)
    if insert_idx == -1:
        return tag
    return tag[:insert_idx] + f' {attr_name}="{attr_value}"' + tag[insert_idx:]


def _annotate_integration_logo_tags(body):
    ...
    def _repl(match):
        tag = match.group(0)
        url = match.group(1)
        classification = _analyze_remote_logo(url)
        updated = _set_html_attr(tag, "data-integration-logo", "true")
        updated = _set_html_attr(
            updated, "data-logo-contrast-light", classification["light"]
        )
        updated = _set_html_attr(
            updated, "data-logo-contrast-dark", classification["dark"]
        )
        updated = _set_html_attr(
            updated, "data-logo-contrast-confidence", classification["confidence"]
        )
        return updated
...
def _render_grid_page(
    ...
    page_description = (
        description
        if page_number == 1
        else f"{description} Page {page_number} of {page_count}."
    )
    cards_markup = "\n".join(cards)
...
slug: "/{page_slug}"
learn_link: "https://learn.netdata.cloud/docs/{page_slug}"
description: "{page_description}"
---

# {sidebar_label}{page_suffix}
...
<GridPagination basePath="/docs/{slug_path}" currentPage={{{page_number}}} pageCount={{{page_count}}} />
'''
...
def get_dir_make_file_and_recurse(
    ...
    if any(Path(directory).glob("**/*")):
        ...
        try:
            relative_dir = dir_name.relative_to(docs_root)
...
""".strip())
        ...
        existing_generated_grid = _is_generated_grid_page(filename)
        should_create_grid = (
            integrations > 0
            and not has_published_single_content
            and (
                not Path(filename).is_file()
                or (overwrite_generated and existing_generated_grid)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--regenerate-grids-only",
        action="store_true",
        help="Regenerate existing AUTOGENERATED integration grids without ingesting upstream content.",
    )

    parser.add_argument(
        "--kickstart-checksum",
        help=(
            "Replace the ingested kickstart checksum placeholder before generated "
            "artifact identities are written. Required unless --local-repo or "
            "--repos selects a local netdata checkout."
        ),
    )
    ...
    try:
        args.kickstart_checksum = resolve_kickstart_checksum(
            args.kickstart_checksum,
            args.regenerate_grids_only,
            LOCAL_REPOS,
        )
    ...
    map_schema_path = "ingest-temp-folder/netdata/docs/.map/map.schema.json"
    if not os.path.isfile(map_schema_path):
        print(f"ERROR: Required map schema not found: {map_schema_path}")
        exit(MAP_SCHEMA_EXIT_CODE)

    try:
        schema_valid, schema_errors = validate_map_schema("map.yaml", map_schema_path)
    except Exception as exc:
        print(f"ERROR: Failed to validate map.yaml schema: {exc}")
        exit(MAP_SCHEMA_EXIT_CODE)
    ...
    apply_kickstart_checksum(DOCS_PREFIX, args.kickstart_checksum)
    ...
    if LOGO_ANALYSIS_SUMMARY["analyzed"] > 0:
        ...
        print(f"Low contrast in dark theme: {LOGO_ANALYSIS_SUMMARY['low_dark']}")
        print(f"Low contrast in light theme: {LOGO_ANALYSIS_SUMMARY['low_light']}")
        print(f"Unknown classifications: {LOGO_ANALYSIS_SUMMARY['unknown']}")
