...
- Create a configuration file for your app using [edit-config](/guides/step-by-step/step-04): `sudo ./edit-config statsd.d/myapp.conf`
...
- A node with the [Netdata Agent](/docs/get#install-the-netdata-agent) installed.
...
An important thing to notice is that StatsD has different types of metrics, as illustrated in the [Netdata documentation](/docs/agent/collectors/statsd.plugin#metrics-supported-by-netdata). Histograms and timers support mathematical operations to be performed on top of the baseline metric, like reporting the `average` of the value. 
...
Start by creating a new configuration file under the `statsd.d/` folder in the [Netdata config directory](/docs/configure/nodes#the-netdata-config-directory). Use [`edit-config`](/docs/configure/nodes#use-edit-config-to-edit-configuration-files) to create a new file called `k6.conf`.
...
To enable this StatsD configuration, [restart Netdata](/docs/configure/start-stop-restart).
...
To edit the section, please follow the Netdata [documentation](/docs/agent/web/gui#customizing-the-local-dashboard).
