...
The `lookup` line will use the `anomaly_rate` dimension of the `anomaly_detection.anomaly_rate` ML chart to calculate the average [node level anomaly rate](/docs/machine-learning-and-anomaly-detection/ml-models-and-anomaly-detection#anomaly-rate) over the last 5 minutes.
