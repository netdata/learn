...
- The `[health]` section in `netdata.conf`. By editing the daemon's behavior, you can disable health monitoring
    ...
    [daemon configuration](/docs/netdata-agent/configuration/daemon#health-section-options) for a table of
...
You can compile Netdata with [debugging](/docs/netdata-agent/daemon#debugging) and then set in `netdata.conf`:
