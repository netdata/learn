...
Before you begin, complete the prerequisites and local exporter setup in [Ingest OpenTelemetry Metrics and Logs](/docs/collecting-metrics/opentelemetry/otlp-ingestion), including the Netdata Cloud sign-in required to verify `otel-logs`.
...
To alert on the derived chart, use the standard [Netdata alert configuration reference](/docs/alerts-&-notifications/alert-configuration-reference). For the shared endpoint, security, and basic metrics/logs workflows, see [Ingest OpenTelemetry Metrics and Logs](/docs/collecting-metrics/opentelemetry/otlp-ingestion).
