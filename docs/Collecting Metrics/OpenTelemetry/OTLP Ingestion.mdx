...
|:------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------|
| Netdata is the only consumer of host or application metrics                         | Use Netdata's [native collectors](/docs/welcome-to-netdata/monitor-anything)                             |
...
| Network devices send syslog                                                         | Use the dedicated [OpenTelemetry Collector syslog setup](/docs/network-performance-monitoring/syslog-from-network-devices/opentelemetry-collector-setup) |
...
For production pipelines beyond these smoke tests, continue with [Metrics Collection](/docs/collecting-metrics/opentelemetry/metrics-collection), [Logs Collection](/docs/collecting-metrics/opentelemetry/logs-collection), and [Transformations](/docs/collecting-metrics/opentelemetry/transformations). Each page links its pinned examples to the complete upstream Collector documentation.
...
1. Open `otel.yaml` with [`edit-config`](/docs/netdata-agent/configuration#edit-configuration-files) and bind `endpoint.path` to an address reachable by the sender.
...
Do not expose a plaintext `0.0.0.0:4317` listener. The optional `auth.enabled` setting requires an `X-Scope-OrgID` header for log tenant selection, but it does not authenticate the sender. See the [OpenTelemetry plugin reference](/docs/collecting-metrics/opentelemetry) for the full public endpoint, tenant, retention, mapping, and remote-storage configuration.
...
For plugin-specific diagnosis, use the [OpenTelemetry plugin reference](/docs/collecting-metrics/opentelemetry). To derive a metric from matching log records, continue with [Create metrics from OpenTelemetry logs](/docs/collecting-metrics/opentelemetry/logs-to-metrics).
