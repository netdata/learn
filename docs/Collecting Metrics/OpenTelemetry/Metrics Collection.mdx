...
Before you begin, complete [Ingest OpenTelemetry Metrics and Logs](/docs/collecting-metrics/opentelemetry/otlp-ingestion). The examples on this page use OpenTelemetry Collector Contrib `0.157.0` and the local, plaintext loopback endpoint from that guide. Use TLS when the Collector and Netdata Agent are on different hosts.
...
Use the smaller [host metrics smoke test](/docs/collecting-metrics/opentelemetry/otlp-ingestion#smoke-test-host-metrics) to verify the OTLP path. For a broader existing OpenTelemetry host pipeline, add the shared exporter and this receiver and pipeline:
...
- **Netdata shows one dimension per chart:** add or adjust an OTel metric mapping. See the mapping section of the [OpenTelemetry plugin reference](/docs/collecting-metrics/opentelemetry).
