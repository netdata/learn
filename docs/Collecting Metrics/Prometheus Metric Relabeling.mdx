---
custom_edit_url: "https://github.com/netdata/netdata/edit/master/src/go/pkg/relabel/README.md"
...
- **Job relabeling** is operator policy. It runs after the job's `selector` and before
  [chart profiles](/docs/collecting-metrics/prometheus-profile-format) are selected, so it can bring an
...
- It is matched against the **full metric name, including** the `_bucket`/`_sum`/`_count` suffixes of histograms and
  ...
  [`match`](/docs/collecting-metrics/prometheus-profile-format#match) works on family base names instead.)
...
format's [`relabeling` section](/docs/collecting-metrics/prometheus-profile-format#relabeling) for authoring
...
[`fallback_type` section](/docs/collecting-metrics/prometheus-profile-format#fallback_type) for the full order.
