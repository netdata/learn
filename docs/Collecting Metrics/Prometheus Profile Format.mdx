...
provide [metric relabeling](/docs/collecting-metrics/prometheus-metric-relabeling) that normalizes its metrics
...
[set up the Prometheus collector job](/docs/collecting-metrics/collectors/applications/prometheus-endpoint)
first.
...
[Chart Template Format](/docs/collecting-metrics/chart-template-format); inline comments explain each field.
...
1. **Scrape** -- the job scrapes the endpoint. The `selector` job option filters unwanted series
   ([selector syntax](https://github.com/netdata/netdata/blob/master/src/go/pkg/prometheus/selector/README.md)).
2. **Job normalization** -- the job's
   [relabeling](/docs/collecting-metrics/prometheus-metric-relabeling) rewrites names and labels. Histogram and
...
|:----------------|:--------:|:------------------------------------------------------------------------------------------------------------------------------------------------------|
| `match`         |   yes    | [Netdata simple pattern](/docs/developer-and-contributor-corner/libnetdata/simple-patterns) tested against post-job metric family names. One hit makes the profile applicable. |
...
- It is tested against the **metric family names** of the scraped metrics -- the series name with the
  ...
  [Chart template rules](#chart-template-rules) and [the relabeling block `match`](/docs/collecting-metrics/prometheus-metric-relabeling#match).)
...
- Syntax is a Netdata [simple pattern](/docs/developer-and-contributor-corner/libnetdata/simple-patterns): a space-separated list of globs where
...
rule format as job-level [metric relabeling](/docs/collecting-metrics/prometheus-metric-relabeling), including the
...
- This is chart suppression, not ingestion filtering. Use the job `selector` or a
  [relabeling](/docs/collecting-metrics/prometheus-metric-relabeling) `drop` rule when the
...
[Chart Template Format](/docs/collecting-metrics/chart-template-format) for every group, chart, instance, label, lifecycle,
...
2. Start from the [Complete example](#complete-example) above or from a stock profile (for example
   [`haproxy.yaml`](https://github.com/netdata/netdata/blob/master/src/go/plugin/go.d/config/go.d/prometheus.profiles/default/haproxy.yaml), installed under
