...
keywords: [prometheus, profiles, relabeling, collectors, charts]
description: "Define curated chart profiles for Prometheus endpoints and reshape scraped metrics with Prometheus-compatible relabeling rules."
sidebar_position: "90"
...
1. **Scrape** -- the job scrapes the endpoint. The `selector` job option filters unwanted series
   ([selector syntax](https://github.com/netdata/netdata/blob/master/src/go/pkg/prometheus/selector/README.md)), and [relabeling](#metric-relabeling) rewrites metric
