...
[metric relabeling](/docs/collecting-metrics/prometheus-metric-relabeling) option, which reshapes scraped metrics
...
1. **Scrape** -- the job scrapes the endpoint. The `selector` job option filters unwanted series
   ([selector syntax](https://github.com/netdata/netdata/blob/master/src/go/pkg/prometheus/selector/README.md)), and
   [relabeling](/docs/collecting-metrics/prometheus-metric-relabeling) rewrites metric names and labels. Profiles
...
|:-----------|:--------:|:--------------------------------------------------------------------------------------------------------------------------------------------------------|
| `match`    |   yes    | [Netdata simple pattern](/docs/developer-and-contributor-corner/libnetdata/simple-patterns) tested against scraped metric family names. One hit makes the profile applicable.  |
