...
learn_link: "https://learn.netdata.cloud/docs/deployment-guides/deployment-strategies"
---
...
The Agent will act as a _stand-alone_ Agent by default, and this is great to start out with for small setups and home labs. By [connecting each Agent to Cloud](/docs/netdata-cloud/connect-agent-to-cloud), you can see an overview of all your nodes, with aggregated charts and centralized alerting, without setting up a Parent.
...
The stand-alone setup is configured out of the box with reasonable defaults, but please consult our [configuration documentation](/docs/netdata-agent/configuration/cheatsheet) for details, including the overview of [common configuration changes](/docs/netdata-agent/configuration/common-configuration-changes).
...
As mentioned above, the recommendation is to not claim the Child to Cloud directly during your setup, avoiding establishing an [ACLK](/docs/netdata-cloud/agent-cloud-link-aclk) connection.
...
For the Parent, besides setting up streaming, the example will also provide an example configuration of multiple [tiers](/docs/developer-and-contributor-corner/database-engine#tiering) of metrics [storage](/docs/netdata-agent/configuration/optimizing-metrics-database/change-how-long-netdata-stores-metrics), for 10 children, with about 2k metrics each.
...
1. Understand Netdata's [security and privacy design](/docs/security-and-privacy-design) and 
   [secure your nodes](/docs/netdata-agent/securing-netdata-agents)
...
3. [Optimize the Netdata Agents system utilization and performance](/docs/netdata-agent/configuration/how-to-optimize-the-netdata-agent-s-performance)

   To save valuable system resources, especially when running on weak IoT devices.

We also suggest that you:

1. [Use Netdata Cloud to access the dashboards](/docs/netdata-cloud/monitor-your-infrastructure)
...
3. [Use host labels](/docs/netdata-agent/configuration/organize-systems-metrics-and-alerts)
