...
databases](/docs/exporting), or jump in to [enabling a connector](/docs/exporting/enable-an-exporting-connector).
...
restart its process_, not the entire [database of long-term metrics](/docs/configuring/optimizing-metrics-database/change-how-long-netdata-stores-metrics).
...
[list of supported databases](/docs/exporting#supported-databases) for information on which
connector to enable and configure for your database of choice.

-   [**AWS Kinesis Data Streams**](/docs/exporting/aws-kinesis-data-streams): Metrics are sent to the service in `JSON`
    format.
-   [**Google Cloud Pub/Sub Service**](/docs/exporting/google-cloud-pub-sub-service): Metrics are sent to the service in `JSON`
    format.
...
-   [**JSON** document databases](/docs/exporting/json-document-databases)
...
-   [**TimescaleDB**](/docs/exporting/timescaledb): Use a community-built connector that takes JSON streams from a
