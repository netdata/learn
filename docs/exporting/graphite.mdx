...
learn_link: "https://learn.netdata.cloud/docs/exporting/graphite"
---
# Export metrics to Graphite providers

You can use the Graphite connector for
the [exporting engine](/docs/exporting/exporting-reference) to archive your agent's
metrics to Graphite providers for long-term storage, further analysis, or correlation with data from other sources.

## Prerequisites

You have already [installed Netdata](/docs/installing) and 
Graphite.
...
We'll not worry about the [rest of the settings](/docs/exporting/exporting-reference#options)
 for now. Restart the Agent using `sudo systemctl restart netdata`, or the
[appropriate method](/docs/maintaining/starting-and-stopping-netdata-agents) for your
...
the [exporting reference guide](/docs/exporting/exporting-reference).
