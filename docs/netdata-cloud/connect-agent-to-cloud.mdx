...
This section guides you through installing and securely connecting a new Agent to Netdata Cloud via the encrypted Agent-Cloud Link ([ACLK](/docs/netdata-cloud/agent-cloud-link-aclk)). Connecting your Agent to Netdata Cloud unlocks additional features like centralized monitoring and easier collaboration.
...
| Method              | Description                                                                                                                                                                                                      |
|---------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Linux/FreeBSD/macOS | Install directly using the [kickstart.sh](/docs/netdata-agent/installation/linux) script.                                                                                                                     |
| Docker              | Install as a container using the provided docker run command or YAML files (Docker Compose/Swarm).                                                                                                               |
| Kubernetes          | Install inside the cluster using `helm`. **Important**: refer to the [Kubernetes installation](/docs/netdata-agent/installation/kubernetes#deploy-netdata-on-your-kubernetes-cluster) for detailed  instructions. |
...
2. Look above this command, near the [Updates channel](/docs/netdata-agent/versions-&-platforms). You should see a button that says "Regenerate token."
...
the [ACLK](/docs/netdata-cloud/agent-cloud-link-aclk) cannot connect to Cloud.
...
>
our [recommended kickstart script](/docs/netdata-agent/installation/linux).
...
We recommend you reinstall Netdata with a [static build](/docs/netdata-agent/installation/linux#install-type), which uses an up-to-date version of OpenSSL with hostname validation enabled.
