...
Netdata's [Machine Learning](/docs/machine-learning-and-anomaly-detection/ml-models-and-anomaly-detection) capabilities are enabled by default if the [Database mode](/docs/netdata-agent/configuration/database) is set to `db = dbengine`
...
2. In the `[ml]` section, set `enabled` to `yes` to enable ML, `no` to disable it, or leave it at the default `auto` to enable ML only when [Database mode](/docs/netdata-agent/configuration/database) is set to `dbengine`
