...
Netdata's [Machine Learning](/docs/machine-learning-and-anomaly-detection/ml-models-and-anomaly-detection) capabilities are enabled by default if the [Database mode](/docs/netdata-agent/configuration/optimizing-metrics-database/database-modes-for-parent-child-setups) is set to `db = dbengine`
...
2. In the `[ml]` section, set `enabled` to `yes` to enable ML, `no` to disable it, or leave it at the default `auto` to enable ML only when [Database mode](/docs/netdata-agent/configuration/optimizing-metrics-database/database-modes-for-parent-child-setups) is set to `dbengine`
3. [Restart Netdata](/docs/netdata-agent/maintenance/starting-and-stopping-netdata-agents)
