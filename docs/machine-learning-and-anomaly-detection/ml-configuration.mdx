...
learn_link: "https://learn.netdata.cloud/docs/machine-learning-and-anomaly-detection/ml-configuration"
---

# ML Configuration

You can use Netdata's [Machine Learning](/docs/machine-learning-and-anomaly-detection/ml-models-and-anomaly-detection) capabilities to detect anomalies in your infrastructure metrics. This feature is enabled by default if your [Database mode](/docs/netdata-agent/database) is set to `db = dbengine`.

## Enabling or Disabling Machine Learning

To enable or disable Machine Learning capabilities on your node:

1. [Edit `netdata.conf`](/docs/netdata-agent/configuration#edit-a-configuration-file-using-edit-config).
2. In the `[ml]` section:
    - Set `enabled` to `yes` to enable ML.
    - Set `enabled` to `no` to disable ML.
    - Leave it at the default `auto` to enable ML only when [Database mode](/docs/netdata-agent/database) is set to `dbengine`.
3. [Restart Netdata](/docs/netdata-agent/maintenance/service-control).
...
This example assumes three child nodes [streaming](/docs/observability-centralization-points/metrics-centralization-points) to one parent node. It shows different ways to configure ML:
