...
When viewing the [Metrics tab or a single-node dashboard](/docs/dashboards-and-charts/tabs/metrics), you'll find the **Metric Correlations** button in the top-right corner.

To start:

1. Click **Metric Correlations**.
2. [Highlight a selection of metrics](/docs/dashboards-and-charts/charts#highlight) on a single chart. The selected timeframe must be at least 15 seconds.
...
- When running Metric Correlations from the [Metrics tab](/docs/dashboards-and-charts/tabs/metrics) across multiple nodes, refine your results by grouping by node:
