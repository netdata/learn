...
- Run it in the debug mode `bin/godplugin -d -m <MODULE_NAME>`. This will output the `STDOUT` of the collector, the same
  ...
  our [documentation](/docs/improving-netdata---developers/external-plugins#external-plugins-api).
- If you want to test the collector with the actual Netdata Agent, you need to replace the `go.d.plugin` binary that
  ...
  you [restart](/docs/getting-started/manage-and-configure/start-stop-or-restart-the-netdata-agent) the Netdata Agent, it will detect and run
...
:exclamation: Netdata module produces [`charts`](/docs/improving-netdata---developers/external-plugins#chart), not
raw metrics.
...
- `Charts` returns the [charts](/docs/improving-netdata---developers/external-plugins#chart) (`*module.Charts`).
