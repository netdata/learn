...
To better understand the guidelines and the API behind our External plugins, please have a look at the [Introduction to External plugins](/docs/improving-netdata---developers/external-plugins) prior to reading this page.
...
`charts.d.plugin` itself can be [configured](/docs/getting-started/manage-and-configure/configuration#use-edit-config-to-edit-configuration-files) using the configuration file `/etc/netdata/charts.d.conf`. This file is also a BASH script.
