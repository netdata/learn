...
For example, the [configuration](/docs/nightly/getting-started/manage-and-configure/configuration) doc first
...
| **Recommended** | Use `edit-config` to edit Netdata's configuration by first navigating to your [Netdata config directory](/docs/nightly/getting-started/manage-and-configure/configuration#the-netdata-config-directory), which is typically at `/etc/netdata`, then running `sudo edit-config netdata.conf`. |
