...
learn_link: "https://learn.netdata.cloud/docs/maintaining/reinstall-netdata-agents"
---
...
any [user configuration](/docs/netdata-agent/configuration) in `netdata.conf` or other files, and will keep the same install
type that was used for the original install.

If you used any [optional
parameters](/docs/netdata-agent/installation/one-line-installer-for-all-linux-systems#optional-parameters-to-alter-your-installation) during initial
...
best route is to [uninstall](/docs/maintaining/uninstall-netdata-agents) and then try a fresh installation using the [one-line
installer](/docs/netdata-agent/installation/one-line-installer-for-all-linux-systems).
