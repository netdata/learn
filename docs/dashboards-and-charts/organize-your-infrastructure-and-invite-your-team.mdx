...
3. _Connect nodes*_, click on **Nodes** tab. Copy the claiming script to your node and run it. See the
   [connect to Cloud doc](/docs/netdata-agent/configuration/connect-agent-to-cloud) for details.
...
Next, pick a role for the invited user. You can read more about [which roles are available](/docs/netdata-cloud/role-based-access-model#what-roles-are-available) based on your [subscription plan](/docs/netdata-cloud/netdata-plans).

Click the **Send** button to send an email invitation, which will prompt them
to [sign up](/docs/dashboards-and-charts/sign-in-with-email-google-or-github) and join your Space.
