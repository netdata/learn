...
sidebar_position: "1132000"
learn_link: "https://learn.netdata.cloud/docs/dashboards-and-charts/organize-your-infrastructure-and-invite-your-team"
---
...
3. _Connect nodes*_, click on **Nodes** tab. Copy the claiming script to your node and run it. See the
   [connect to Cloud doc](/docs/netdata-cloud/connect-agent-to-cloud) for details.
...
- **Incident response**  
  You can also create new War Rooms as one of the first steps in your incident response process.
   For example, you have a user-facing web app that relies on Apache Pulsar for a message queue, and one of your nodes using the [Pulsar collector](/docs/collecting-metrics/message-brokers/apache-pulsar) begins reporting a suspiciously low messages rate.
...
Next, pick a role for the invited user. You can read more about [which roles are available](/docs/netdata-cloud/authentication-&-authorization/role-based-access-model#what-roles-are-available) based on your [subscription plan](/docs/netdata-cloud/netdata-plans).

Click the **Send** button to send an email invitation, which will prompt them
to [sign up](/docs/netdata-cloud/authentication-&-authorization/sign-in-to-netdata) and join your Space.
