...
As of `v1.35.0-22-nightly` Metric Correlations has been enabled by default on all Agents. Due to the latest optimizations to the metric correlation algorithm of the Agent, now the resource impact of this feature enabled by default is less than the previous default of having the agent fetch and send all the raw data to do the actual Metric Correlation in the Cloud side. As such running MC on the agent is less impactful on local resources than running via cloud.
