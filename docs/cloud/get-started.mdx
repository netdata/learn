...
users can select from existing nodes or select an admin from a provided list to request a node connection. You can connect any 
node running Netdata, whether it's a physical or virtual machine, a Docker container, IoT device, and more. 
