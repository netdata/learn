...
From within the created War Rooms, Netdata Cloud prompts you to [connect](/docs/agent/claim) your nodes to Netdata Cloud. You
can connect any node running Netdata, whether it's a physical or virtual machine, a Docker container, IoT device, and
more. Non-admin users can select from existing nodes or select an admin from a provided list to request a node connection.
