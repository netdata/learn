...
| **[Alerts Configuration Manager](/docs/alerts-&-notifications/creating-alerts-with-the-alerts-configuration-manager)** | Visual UI wizard for creating and editing alerts | Manual control over every parameter via a guided interface |
| **[Manual config files](/docs/alerts-&-notifications/alert-configuration-reference)** | Edit `health.d/*.conf` files directly | Advanced users, automation pipelines, version-controlled configurations |
...
Both. Alerts Automation provides a fully automated, AI-powered path where you describe what you want in plain English and the AI generates, tests, and helps you deploy the alert configuration. You can also configure alerts manually using the [Alerts Configuration Manager](/docs/alerts-&-notifications/creating-alerts-with-the-alerts-configuration-manager) UI or by [editing config files directly](/docs/alerts-&-notifications/alert-configuration-reference).
...
1. **ML-based anomaly detection** — Every Netdata Agent runs unsupervised machine learning models that learn normal metric behavior and score anomalies in real time. You can create alerts based on anomaly rates. See [ML Anomaly Detection](/docs/netdata-ai/anomaly-detection).
...
The [Alerts Configuration Manager](/docs/alerts-&-notifications/creating-alerts-with-the-alerts-configuration-manager) is a visual UI wizard where you manually select metrics, set thresholds, and configure parameters field by field. Alerts Automation uses AI to generate the entire configuration from a natural language description and lets you validate it against historical data before deployment. Both produce the same alert definitions.

## See also

- [Alert Configuration Reference](/docs/alerts-&-notifications/alert-configuration-reference) — full manual configuration syntax
- [Alerts Configuration Manager](/docs/alerts-&-notifications/creating-alerts-with-the-alerts-configuration-manager) — visual UI wizard for alert configuration
- [ML Anomaly Detection](/docs/netdata-ai/anomaly-detection) — machine learning based anomaly scoring
- [Alert Troubleshooting](/docs/netdata-ai/troubleshooting/alert-troubleshooting) — AI-powered alert investigation
- [Netdata AI Overview](/docs/netdata-ai) — complete AI capabilities guide
