...
1. [Edit `netdata.conf`](/docs/netdata agent/configuration#edit-configuration-files).
2. In the `[ml]` section:
...
This example assumes three child nodes [streaming](/docs/netdata parents/metrics centralization points/metrics-centralization-points) to one parent node. It shows different ways to configure ML:
