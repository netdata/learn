...
learn_rel_path: "Netdata AI/MCP/MCP Connections"
sidebar_position: "30"
...
- A reachable **MCP server** to connect to. Netdata ships built-in integrations for popular providers (GitHub, [PagerDuty](/docs/netdata-ai/mcp/mcp-connections/pagerduty), Atlassian Cloud for Jira/Confluence/Bitbucket) and a **Custom MCP Server** option for any HTTPS MCP endpoint.

## Provider guides

- [PagerDuty](/docs/netdata-ai/mcp/mcp-connections/pagerduty): connect Netdata AI to PagerDuty incidents, on-call schedules and change events.
...
1. An **organization owner** installs [Netdata MCP](https://github.com/apps/netdata-mcp) on the organization, from [https://github.com/apps/netdata-mcp/installations/new](https://github.com/apps/netdata-mcp/installations/new).
...
See [Conversations](/docs/netdata-ai/conversations) for more on live, interactive troubleshooting.
...
See [Investigations](/docs/netdata-ai/investigations) and [Scheduled Reports](/docs/netdata-ai/insights/scheduled-reports).
...
An investigation or scheduled report costs **1 AI credit when it completes**, no matter how many tools it calls. In conversations, tool results become part of the exchange, so a very verbose MCP server makes a conversation cost slightly more. See [Conversations](/docs/netdata-ai/conversations#ai-credits-consumption) and [Investigations](/docs/netdata-ai/investigations#availability-and-credits).
...
[Netdata Cloud Outbound IP Addresses](/docs/netdata-cloud/netdata-cloud-outbound-ip-addresses) for the published list and how
to consume it.
