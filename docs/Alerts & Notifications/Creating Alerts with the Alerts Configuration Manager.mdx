...
**Want AI to handle the configuration for you?** [Alerts Automation](/docs/netdata-ai/alerts-automation) uses AI to suggest alerts, generate the configuration, and test it against historical data — so you can validate thresholds before deployment without writing any configuration manually.
...
For details on how anomalies are detected, see [ML Anomaly Detection](/docs/netdata-ai/anomaly-detection). For the underlying alert configuration syntax with `anomaly-bit` lookups, see the [Alert Configuration Reference](/docs/alerts-&-notifications/alert-configuration-reference) (Examples 6 and 7).
...
- If you need help writing custom alerts, check the [full alert reference](/docs/alerts-&-notifications/alert-configuration-reference)
- To create alerts using natural language instead of this manual wizard, see [Alerts Automation](/docs/netdata-ai/alerts-automation)
