...
You can use health alerts with any of Netdata's [collectors](/docs/collecting metrics/collecting-metrics) (see the [supported collector list](/docs/welcome to netdata/monitor-anything)) to monitor your systems, containers, and applications in real time.

While you can view active alerts on both the local dashboard and Netdata Cloud, you configure all health alerts _per node_ via individual Netdata Agents. If you want to deploy a new alert across your [infrastructure](/docs/netdata cloud/spaces and rooms/spaces-and-rooms), you must configure each node with the same health configuration files.
