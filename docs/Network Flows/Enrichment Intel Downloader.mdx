...
sidebar_position: "50"
learn_link: "https://learn.netdata.cloud/docs/network-flows/enrichment-intel-downloader"
slug: "/network-flows/enrichment-intel-downloader"
---
...
You can still pull *any* MMDB build (including a custom one) into the resolver by configuring `enrichment.geoip.asn_database` / `geo_database` directly — the downloader is one of several producers; the plugin doesn't care who wrote the MMDB. See the [Custom MMDB Database](/docs/network-flows/enrichment-methods/custom-mmdb-database) card. If you prefer MaxMind's own updater, run [`geoipupdate`](/docs/network-flows/enrichment-methods/maxmind-geoip-geolite2) and point `enrichment.geoip.asn_database` / `enrichment.geoip.geo_database` at the MMDB files it produces.
...
When the plugin auto-detects MMDBs this way it forces `optional: true` on the geoip stanza — a missing or transiently-unreadable file does not crash the plugin. If you instead set `asn_database` / `geo_database` explicitly in `netflow.yaml`, you control the `optional` flag yourself; see [Configuration](/docs/network-flows/configuration#enrichment).
...
- Per-provider details (refresh cadence, license, schema, attribution requirements):
  ...
  - [CAIDA RouteViews Prefix-to-AS](/docs/network-flows/enrichment-methods/caida-routeviews-prefix-to-as) — prefix-to-AS TSV feed; ASN-only.
  - [MaxMind GeoIP / GeoLite2](/docs/network-flows/enrichment-methods/maxmind-geoip-geolite2) — authenticated MaxMind downloads or MMDB files managed by `geoipupdate`.
  - [IP2Location LITE IP-Country](/docs/network-flows/enrichment-methods/ip2location-lite-ip-country) — public country-only CSV ZIP feed.
  - [IPDeny Country Zones](/docs/network-flows/enrichment-methods/ipdeny-country-zones) — country CIDR archive.
  - [IPIP Country Database](/docs/network-flows/enrichment-methods/ipip-country-database) — country text ZIP feed.
  - [Custom MMDB Database](/docs/network-flows/enrichment-methods/custom-mmdb-database) — your own MMDB build.
- The enrichment mechanism that consumes these files: [Enrichment](/docs/network-flows/enrichment) (the MMDB shared mechanism section).
- The plugin knobs that point at the files: [Configuration › `enrichment.geoip`](/docs/network-flows/configuration#enrichment).
