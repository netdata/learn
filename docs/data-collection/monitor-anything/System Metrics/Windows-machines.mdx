...
- (Optional) [Disable all plugins](/docs/getting-started/manage-and-configure/common-configuration-changes#disable-a-collector-or-plugin) except for go.d in `netdata.conf`, so that you only see Windows metrics.
- (Optional) Set up [replication](/docs/deployment-in-production/streaming-and-replication/streaming-configuration-reference), for high availability.
...
Netdata [config directory](/docs/getting-started/manage-and-configure/configuration#the-netdata-config-directory), which is typically at `/etc/netdata`.
