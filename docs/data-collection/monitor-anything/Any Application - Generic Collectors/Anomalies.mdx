...
sidebar_position: "314043"
slug: "/data-collection/monitor-anything/any-application---generic-collectors/anomalies"
---


# Anomaly detection with Netdata

**Note**: Check out the [Netdata Anomaly Advisor](/docs/troubleshooting-and-machine-learning/anomaly-advisor) for a more native anomaly detection experience within Netdata.
...
directory](/docs/getting-started/manage-and-configure/configuration), which is usually at `/etc/netdata`.
...
- Python 3 is required as the [`netdata-pandas`](https://github.com/netdata/netdata-pandas) package uses Python async libraries ([asks](https://pypi.org/project/asks/) and [trio](https://pypi.org/project/trio/)) to make asynchronous calls to the [Netdata REST API](/docs/api) to get the required data for each chart.
...
- If you would like to enable this on a Raspberry Pi, then check out [this guide](/docs/miscellaneous/unsupervised-anomaly-detection-for-raspberry-pi-monitoring) which will guide you through first installing LLVM.
