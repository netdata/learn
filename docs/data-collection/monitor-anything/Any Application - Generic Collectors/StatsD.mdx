...
learn_rel_path: "Data collection/Monitor Anything/Any application - Generic Collectors"
...
sidebar_position: "314041"
slug: "/data-collection/monitor-anything/any-application---generic-collectors/statsd"
---
...
- [K6 load testing tool](https://k6.io)
  ...
  - [Documentation](/docs/data-collection/monitor-anything/applications/k6-load-testing)
...
- [Asterisk](https://www.asterisk.org/)
  ...
  - [Documentation](/docs/data-collection/monitor-anything/applications/asterisk)
...
Private charts are controlled with `create private charts for metrics matching = *`. This setting accepts a space-separated list of [simple patterns](/docs/improving-netdata---developers/libnetdata/simple-patterns). Netdata will create private charts for all metrics **by default**.
...
You can specify Netdata StatsD to have a different `memory mode` than the rest of the Netdata Agent. You can read more about `memory mode` in the [documentation](/docs/deployment-in-production/streaming-and-replication/database-modes-for-parent-child-setups).
...
> You can read more about how the Netdata Agent organizes information in the relevant [documentation](/docs/improving-netdata---developers/web)
...
-   `metrics` is a Netdata [simple pattern](/docs/improving-netdata---developers/libnetdata/simple-patterns). This pattern should match all the possible StatsD metrics that will be participating in the application `myapp`.
...
`pattern` is a keyword. When set, `METRIC` is expected to be a Netdata [simple pattern](/docs/improving-netdata---developers/libnetdata/simple-patterns) that will be used to match all the StatsD metrics to be added to the chart. So, `pattern` automatically matches any number of StatsD metrics, all of which will be added as separate chart dimensions.
...
- Create a configuration file for your app using [edit-config](/docs/getting-started/manage-and-configure/configuration): `sudo ./edit-config
  statsd.d/myapp.conf`
...
- A node with the [Netdata](/docs/getting-started/install-netdata) installed.
...
Use [`edit-config`](/docs/getting-started/manage-and-configure/configuration#use-edit-config-to-edit-configuration-files) 
...
Find more details about family and context in our [documentation](/docs/improving-netdata---developers/web#families).
...
- **pattern**: A keyword that tells the StatsD server the `METRIC` string is actually a 
  [simple pattern](/docs/improving-netdata---developers/libnetdata/simple-patterns). 
  ...
  patterns in [dimension patterns](/docs/data-collection/monitor-anything/any-application---generic-collectors/statsd#dimension-patterns).
...
To enable this StatsD configuration, [restart Netdata](/docs/getting-started/manage-and-configure/start-stop-or-restart-the-netdata-agent).
...
To edit the section, please follow the Netdata [documentation](/docs/visualizations/agent-dashboard#customizing-the-local-dashboard).
...
- Make sure you follow the [contributing guide](/docs/improving-netdata---developers) 
