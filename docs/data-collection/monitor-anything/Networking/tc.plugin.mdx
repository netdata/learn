...
5.  **Protect the servers from DDoS attacks.**
    ...
    Using QoS together with a [SYNPROXY](/docs/data-collection/monitor-anything/System%20Metrics/OS-provided-metrics-proc.plugin) will provide a great degree of protection against most DDoS attacks. Actually when I wrote that article, a few folks tried to DDoS the Netdata demo site to see in real-time the SYNPROXY operation. They did not do it right, but anyway a great deal of requests reached the Netdata server. What saved Netdata was QoS. The Netdata demo server has QoS installed, so the requests were throttled and the server did not even reach the point of resource starvation. Read about it [here](/docs/data-collection/monitor-anything/System%20Metrics/OS-provided-metrics-proc.plugin).
