...
[**apps.plugin**](/docs/data-collection/apm/application-monitoring) looks at the Linux process tree every second, much like `top` or
...
- [Prometheus endpoints](/docs/data-collection/apm/openmetrics-endpoints): Gathers
  ...
  endpoints.
- [Web server logs (Apache, NGINX)](/docs/data-collection/web-servers-and-web-proxies/web-logs/apache-nginx-microsoft-iis):
...
- [Redis](/docs/data-collection/database-servers/redis): Monitor database status by
...
caveat is that you must [install Netdata](/docs/install-the-netdata-agent) on a separate system or a compatible VM because there
...
collector](/docs/data-collection/database-servers/mysql-and-mariadb) to look at `203.0.113.0:3306`:
...
collector](/docs/data-collection/apm/openmetrics-endpoints).
...
infrastructure](/docs/netdata-dashboards-and-visualizations/infrastructure-overview) using Netdata Cloud.
