...
This collector uses the [Netdata rest api](/docs/netdata-apis/api) to get the `mean` and `stddev`
...
agent's [config directory](/docs/typical-netdata-agent-configurations/configuration#the-netdata-config-directory), which is
