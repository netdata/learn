...
Netdata uses [cgroups.plugin](/docs/data-collection/containers-and-vms/cgroups) to poll `/sys/fs/cgroup` and convert the raw data
...
[Helmchart](/docs/install-the-netdata-agent/kubernetes) to automate deployment, collectors for k8s agent services, and
...
[cgroups.plugin](/docs/data-collection/containers-and-vms/cgroups). By polling cgroups every second, Netdata can produce meaningful
...
collector](/docs/data-collection/database-servers/mysql-and-mariadb) to look at `127.0.0.0:3306` for
MySQL metrics:
...
metrics](/docs/data-collection/database-servers/mysql-and-mariadb#charts) as well.
...
Netdata can even [run in a Docker container](/docs/install-the-netdata-agent/docker) itself, and then collect metrics about the
...
-   A [service discovery plugin](https://github.com/netdata/agent-service-discovery), which discovers and creates
    ...
    collector](/docs/data-collection/apm/openmetrics-endpoints). With these
...
-   A [Kubelet collector](/docs/data-collection/kubernetes/kubelet), which runs
    ...
    and more.
-   A [kube-proxy collector](/docs/data-collection/kubernetes/kube-proxy), which
    also runs on each node and monitors latency and the volume of HTTP requests to the proxy.
-   A [cgroups collector](/docs/data-collection/containers-and-vms/cgroups), which collects CPU, memory, and bandwidth metrics for
...
with [seeing an overview of your infrastructure](/docs/netdata-dashboards-and-visualizations/infrastructure-overview) using Netdata Cloud.
