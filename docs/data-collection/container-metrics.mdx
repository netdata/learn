...
Netdata uses [cgroups.plugin](/docs/data-collection/monitor-anything/Virtualized Environments/Containers/Monitor-Cgroups) to poll `/sys/fs/cgroup` and convert the raw data
...
[dockerd](/docs/data-collection/monitor-anything/Virtualized Environments/Containers/Docker) or [Docker
Engine](/docs/data-collection/monitor-anything/Virtualized Environments/Containers/Docker-Engine). You can find all
...
[cgroups.plugin](/docs/data-collection/monitor-anything/Virtualized Environments/Containers/Monitor-Cgroups). By polling cgroups every second, Netdata can produce meaningful
...
-   A [service discovery plugin](https://github.com/netdata/agent-service-discovery), which discovers and creates
    ...
    collector](/docs/data-collection/monitor-anything/Generic Collectors/Prometheus-endpoints). With these
...
-   A [Kubelet collector](/docs/data-collection/monitor-anything/Container Orchestrators/Kubernetes/Kubelet), which runs
    on each node in a k8s cluster to monitor the number of pods/containers, the volume of operations on each container,
    and more.
-   A [kube-proxy collector](/docs/data-collection/monitor-anything/Container Orchestrators/Kubernetes/Kube-proxy), which
    also runs on each node and monitors latency and the volume of HTTP requests to the proxy.
-   A [cgroups collector](/docs/data-collection/monitor-anything/Virtualized Environments/Containers/Monitor-Cgroups), which collects CPU, memory, and bandwidth metrics for
