...
Private charts are controlled with `create private charts for metrics matching = *`. This setting accepts a space-separated list of [simple patterns](/docs/contributing-to-netdata/libnetdata/simple-patterns). Netdata will create private charts for all metrics **by default**.
...
You can specify Netdata StatsD to have a different `memory mode` than the rest of the Netdata Agent. You can read more about `memory mode` in the [documentation](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/database-modes-for-parent-child-setups).
...
> You can read more about how the Netdata Agent organizes information in the relevant [documentation](/docs/contributing-to-netdata/web)
...
-   `metrics` is a Netdata [simple pattern](/docs/contributing-to-netdata/libnetdata/simple-patterns). This pattern should match all the possible StatsD metrics that will be participating in the application `myapp`.
...
`pattern` is a keyword. When set, `METRIC` is expected to be a Netdata [simple pattern](/docs/contributing-to-netdata/libnetdata/simple-patterns) that will be used to match all the StatsD metrics to be added to the chart. So, `pattern` automatically matches any number of StatsD metrics, all of which will be added as separate chart dimensions.
...
- Create a configuration file for your app using [edit-config](/docs/typical-netdata-agent-configurations/configuration): `sudo ./edit-config
  statsd.d/myapp.conf`
...
- A node with the [Netdata](/docs/install-the-netdata-agent) installed.
...
[Netdata config directory](/docs/typical-netdata-agent-configurations/configuration#the-netdata-config-directory). 
Use [`edit-config`](/docs/typical-netdata-agent-configurations/configuration#use-edit-config-to-edit-configuration-files) 
...
Find more details about family and context in our [documentation](/docs/contributing-to-netdata/web#families).
...
- **pattern**: A keyword that tells the StatsD server the `METRIC` string is actually a 
  [simple pattern](/docs/contributing-to-netdata/libnetdata/simple-patterns). 
...
To enable this StatsD configuration, [restart Netdata](/docs/maintenance-operations-on-netdata-agents/starting-and-stopping-netdata-agents).
...
To edit the section, please follow the Netdata [documentation](/docs/netdata-dashboards-and-visualizations/accessing-netdata-dashboards/netdata-agent-dashboards#customizing-the-local-dashboard).
...
- Make sure you follow the [contributing guide](/docs/contributing-to-netdata) 
