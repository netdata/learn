...
when you [install Netdata](/docs/install-the-netdata-agent).
...
[database engine for storage](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/change-how-long-netdata-stores-metrics)
, and immediately 
[visualizes them meaningfully](/docs/netdata-dashboards-and-visualizations/netdata-charts) 
on dashboards.
...
-   **Plugins** help manage all the independent data collection processes in a variety of programming languages, based on 
    ...
    -   **External** plugins organize collectors that gather metrics from external processes, such as a MySQL database or
        ...
        [plugins.d](/docs/contributing-to-netdata/external-plugins), which provides additional management options.

-   **Orchestrators** are external plugins that run and manage one or more modules. They run as independent processes.
    The Go orchestrator is in active development.

    -   [go.d.plugin](/docs/contributing-to-netdata/external-plugins/go.d.plugin): An orchestrator for data
        collection modules written in `go`.

    -   [python.d.plugin](/docs/contributing-to-netdata/external-plugins/python.d.plugin): 
        An orchestrator for data collection modules written in `python` v2/v3.

    -   [charts.d.plugin](/docs/contributing-to-netdata/external-plugins/charts.d.plugin): 
