...
when you [install Netdata](/docs/getting-started/install-netdata).

All collectors are **installed by default** with every installation of Netdata. You do not need to install
collectors manually to collect metrics from new sources. See how you can  
[monitor anything with Netdata](/docs/data-collection/monitor-anything).
...
our [collectors' configuration reference](/docs/data-collection/collectors-configuration).
...
[database engine for storage](/docs/deployment-in-production/metric-retention---database/change-how-long-netdata-stores-metrics)
, and immediately 
[visualizes them meaningfully](/docs/getting-started/monitor-your-infrastructure/interact-with-charts) 
on dashboards.
...
-   **Plugins** help manage all the independent data collection processes in a variety of programming languages, based on 
    ...
    -   **External** plugins organize collectors that gather metrics from external processes, such as a MySQL database or
        ...
        [plugins.d](/docs/improving-netdata---developers/external-plugins), which provides additional management options.

-   **Orchestrators** are external plugins that run and manage one or more modules. They run as independent processes.
    The Go orchestrator is in active development.

    -   [go.d.plugin](/docs/improving-netdata---developers/external-plugins/go.d.plugin): An orchestrator for data
        collection modules written in `go`.

    -   [python.d.plugin](/docs/improving-netdata---developers/external-plugins/python.d.plugin): 
        An orchestrator for data collection modules written in `python` v2/v3.

    -   [charts.d.plugin](/docs/improving-netdata---developers/external-plugins/charts.d.plugin): 
