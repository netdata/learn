...
learn_rel_path: "Data collection/External-plugins"
sidebar_position: "614120"
learn_link: "https://learn.netdata.cloud/docs/data-collection/external-plugins/charts.d.plugin"
---
# charts.d.plugin
...
To better understand the guidelines and the API behind our External plugins, please have a look at the [Introduction to External plugins](/docs/data-collection/external-plugins) prior to reading this page.
...
By default, `charts.d.plugin` is not included as part of the install when using [our official native DEB/RPM packages](/docs/installing/native-linux-distribution-packages). You can install it by installing the `netdata-plugin-chartsd` package.

## Configuration

`charts.d.plugin` itself can be [configured](/docs/configuring/configuration#use-edit-config-to-edit-configuration-files) using the configuration file `/etc/netdata/charts.d.conf`. This file is also a BASH script.
