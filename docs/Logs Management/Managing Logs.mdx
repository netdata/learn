...
sidebar_label: "Managing Logs"
learn_status: "Published"
learn_rel_path: "Logs Management"
keywords: [logs, management, full-text search, facets, histogram, live tail, sampling]
description: "How logs are organized, stored, queried, and explored in Netdata — sources, field filters with counters, full-text search, histograms, live tail, and sampling at scale."
sidebar_position: "10"
learn_link: "https://learn.netdata.cloud/docs/logs-management/managing-logs"
slug: "/logs-management/managing-logs"
---
...
[Logs Management](/docs/logs-management).
...
|:-------|:---------|:--------------|:----------|
| `systemd-journal` | Linux | systemd journal files: system, user, namespace, and remote journals, merged by default | [Systemd Journal Plugin Reference](/docs/logs-management/systemd-journal-logs/systemd-journal-plugin-reference) |
| `windows-events` | Windows | Windows event channels, auto-detected and aggregated, including forwarded-events channels on a Windows Event Collector | [Windows Events Plugin Reference](/docs/logs-management/windows-event-logs) |
| `macos-logs` | macOS | The native unified log store, through Apple's OSLog framework | [macOS Logs Plugin Reference](/docs/logs-management/macos-unified-logs) |
| `otel-logs` | Any, via OTLP | Logs received by the OpenTelemetry plugin and stored in Netdata's indexed log store | [OpenTelemetry plugin reference](/docs/opentelemetry/opentelemetry-plugin-reference) |
| `snmp:traps` | Any | Journal-compatible files Netdata writes for the traps received by its SNMP traps collector | [SNMP Trap Logs](/docs/logs-management/snmp-trap-logs) |

Application text files appear under `systemd-journal` when converted with log2journal, or under `otel-logs` when
shipped through an OpenTelemetry Collector; see [Text Files to Journals](/docs/logs-management/text-files-to-journals).
Network flows are stored the same way as SNMP traps but have their own view,
[Network Flows](/docs/logs-management/network-flows), instead of the Logs tab.

:::note

On Linux systems without systemd, such as Alpine Linux, the `systemd-journal` source is unavailable. Convert and push
logs to a remote `systemd-journal-remote` with [`systemd-cat-native --url`](/docs/logs-management/text-files-to-journals/systemd-cat-native),
or send them through [OpenTelemetry](/docs/opentelemetry/otlp-ingestion).
...
- Where journal sequence numbers are available, estimates use them for precision. See
  [Performance at scale](/docs/logs-management/systemd-journal-logs/systemd-journal-plugin-reference#performance-at-scale) for the math.
...
- Sign in with Netdata Cloud, which is free for community use. Log content is transmitted encrypted to your browser
  ...
  [Netdata Cloud On-Prem](/docs/netdata-cloud-on-prem).
- Platform requirements, packages, and container settings are documented in each plugin's reference:
  [systemd journal](/docs/logs-management/systemd-journal-logs/systemd-journal-plugin-reference#prerequisites),
  [Windows events](/docs/logs-management/windows-event-logs#prerequisites),
  [macOS logs](/docs/logs-management/macos-unified-logs#prerequisites),
  [OpenTelemetry](/docs/opentelemetry/opentelemetry-plugin-reference).
...
your infrastructure. See [Netdata AI](/docs/netdata-ai).

## Where to next

- [Logs Management](/docs/logs-management) — where logs are stored, how to decide per
  source, and the current limitations.
- [Text Files to Journals](/docs/logs-management/text-files-to-journals) — bring application log files into these sources.
