...
- Run it in the debug mode `bin/godplugin -d -m <MODULE_NAME>`. This will output the `STDOUT` of the collector, the same
  ...
  our [documentation](/docs/contributing/external-plugins#external-plugins-api).
- If you want to test the collector with the actual Netdata Agent, you need to replace the `go.d.plugin` binary that
  ...
  you [restart](/docs/maintenance/starting-and-stopping-netdata-agents) the Netdata Agent, it will detect and run
...
:exclamation: Netdata module produces [`charts`](/docs/contributing/external-plugins#chart), not
raw metrics.
...
- `Charts` returns the [charts](/docs/contributing/external-plugins#chart) (`*module.Charts`).
