...
By default, `charts.d.plugin` is not included as part of the install when using [our official native DEB/RPM packages](/docs/installation/native-linux-distribution-packages). You can install it by installing the `netdata-plugin-chartsd` package.
