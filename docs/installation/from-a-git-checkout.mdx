...
-   `--local-files`: Used for [offline installations](/docs/installation/offline-systems). Pass four file paths: the Netdata
...
Unlike the [`kickstart.sh`](/docs/installation/one-line-installer-for-all-linux-systems), the `netdata-installer.sh` script does
