...
The [Netdata Helm chart](/docs/installation/kubernetes-helm-chart-reference) installs one `parent` pod for storing metrics and managing alarm notifications, plus an additional
...
2. To install Netdata using the `helm install` command, run:
  ...
  For more installation options, please read our [Netdata Helm chart for Kubernetes](/docs/installation/kubernetes-helm-chart-reference) reference.
