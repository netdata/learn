...
Netdata collects anonymous usage information from the Agent to guide product development and improve stability. Observability data (metrics and logs) is never collected, and metadata is anonymized before transmission. Netdata does not sell or share this data with third parties. See [Anonymous telemetry events](/docs/netdata-agent/anonymous-telemetry-events) for the full details and all opt-out methods.
