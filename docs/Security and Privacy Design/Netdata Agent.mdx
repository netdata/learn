...
Telemetry is **on by default** and carries only anonymized metadata — never raw metrics. See [Anonymous telemetry events](/docs/netdata-agent/anonymous-telemetry-events) for exactly what is collected and the opt-out methods.

### Agent-Cloud Link (ACLK)

The [ACLK](/docs/netdata-cloud/agent-cloud-link-aclk) is the channel the Agent uses to communicate with Netdata Cloud. It:
...
IP addresses can change without notice and vary based on your geographic location due to CDN-edge servers, so **always prefer domain-based allowlisting**. For the complete firewall allowlist with ports, see [Configure Netdata for cybersecurity platforms](/docs/netdata-agent/configuration/securing-agents/configure-netdata-for-cybersecurity-platforms#required-endpoints-and-ports). If your Agent requires a proxy to reach the internet, see [proxy configuration](/docs/netdata-cloud/connect-agent#proxy-configuration).
...
Disable automatic updates with the `--no-updates` flag at install time, or by disabling the updater on an existing install. For a fully disconnected deployment, see [Install Netdata on Offline Systems](/docs/netdata-agent/installation/linux/offline-systems).
