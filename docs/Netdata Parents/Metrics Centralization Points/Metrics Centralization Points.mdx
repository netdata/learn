...
For full details on high-availability Parent clusters, including failover and maintenance procedures, see [Clustering and High Availability of Netdata Parents](/docs/netdata-parents/metrics-centralization-points/clustering-and-high-availability-of-netdata-parents).
...
1. Install Netdata on your Parent node.
   ...
   For installation instructions, see the [Installation section](/docs/netdata-agent/installation).
...
A Netdata Parent uses the standard [`netdata/netdata`](/docs/netdata-agent/installation/docker) Docker image—a Parent is simply a regular Agent whose `stream.conf` is configured to receive metrics, so no separate image or flag is required.

Deploy the container with the `docker run` command from the [Docker installation guide](/docs/netdata-agent/installation/docker). The persistent volumes (`netdataconfig`, `netdatalib`, `netdatacache`) created by that command keep the Parent's configuration and metrics retention across container restarts.
...
Set the `[API_KEY]` section as described in [Configuring Metrics Centralization Points](/docs/netdata-parents/metrics-centralization-points/configuring-metrics-centralization-points), then apply the change with `docker restart netdata`.
...
For detailed guidelines, check [Best Practices for Observability Centralization Points](/docs/netdata-parents/parent-configuration-best-practices).

To understand how node connection states (Live, Stale, Offline) work with Parent-Child setups, see [Node States and Transitions](/docs/netdata-cloud/node-states-and-transitions).
