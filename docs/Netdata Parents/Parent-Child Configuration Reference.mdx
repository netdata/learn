...
For a quick reference on setting up the Parent-Child relationship, see the [Configuration Examples](#complete-configuration-examples) or refer to our comprehensive [Parents: Your Centralization Points](/docs/netdata-parents) documentation for more details.
...
On the Parent node, you can [configure retention settings](/docs/netdata-agent/configuration/database#retention-settings) to control how long metrics are stored.
