...
Read more about [Starting, Stopping and Restarting the Agent](/docs/getting-started/manage-and-configure/start-stop-or-restart-the-netdata-agent).

:::

## Claiming on an older, deprecated version of the Agent

Make sure that you are using the latest version of Netdata if you are using the [Claiming script](/docs/getting-started/monitor-your-infrastructure/connect-agent-to-cloud#claiming-script).

With the introduction of our new architecture, Agents running versions lower than `v1.32.0` can face claiming problems, so we recommend you [update the Netdata Agent](/docs/installation/update-netdata) to the latest stable version.
