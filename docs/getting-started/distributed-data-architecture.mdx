...
visualize your nodes' metrics. When you [look at charts in Netdata Cloud](/docs/getting-started/monitor-your-infrastructure/interact-with-charts)
...
- **Performance**: Every query to a node's database takes only a few milliseconds to complete for responsiveness when
  ...
  like [Metric Correlations](/docs/troubleshooting-and-machine-learning/metric-correlations).
...
Read our document on changing [how long Netdata stores metrics](/docs/deployment-in-production/metric-retention---database/change-how-long-netdata-stores-metrics) on your nodes for
details.

You can also stream between nodes using [streaming](/docs/deployment-in-production/streaming-and-replication/streaming-configuration-reference), allowing to replicate databases and create
...
TimescaleDB, and many others, see details on [integrating Netdata via exporting](/docs/exporting-data).
...
To enable certain features, such as [viewing active alarms](/docs/getting-started/monitor-your-infrastructure/view-active-alerts)
or [filtering by hostname/service](/docs/getting-started/monitor-your-infrastructure/war-rooms), Netdata Cloud does
