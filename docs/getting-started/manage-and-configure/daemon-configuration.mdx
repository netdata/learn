...
the [web server access lists](/docs/deployment-in-production/secure-your-nodes/web-server#access-lists).

`netdata.conf` has sections stated with `[section]`. You will see the following sections:

1. `[global]` to [configure](#global-section-options) the [Netdata daemon](/docs/getting-started/manage-and-configure/netdata-daemon).
...
5. `[environment variables]` to [configure](#environment-variables-section-options) the environment variables used
   Netdata.
6. `[sqlite]` to [configure](#sqlite-section-options) the [Netdata daemon](/docs/getting-started/manage-and-configure/netdata-daemon) SQLite settings.
7. `[ml]` to configure settings for [machine learning](/docs/troubleshooting-and-machine-learning/machine-learning-ml-powered-anomaly-detection).
8. `[health]` to [configure](#health-section-options) general settings for [health monitoring](/docs/alerts-and-notifications).
9. `[web]` to [configure the web server](/docs/deployment-in-production/secure-your-nodes/web-server).
10. `[registry]` for the [Netdata registry](/docs/deployment-in-production/secure-your-nodes/registry).
11. `[global statistics]` for the [Netdata registry](/docs/deployment-in-production/secure-your-nodes/registry).
12. `[statsd]` for the general settings of the [stats.d.plugin](/docs/data-collection/monitor-anything/Any%20Application%20-%20Generic%20Collectors/StatsD).
...
After `netdata.conf` has been modified, Netdata needs to be [restarted](/docs/getting-started/manage-and-configure/start-stop-or-restart-the-netdata-agent) for
changes to apply:
...
|:-------------------------------------:|:-------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       process scheduling policy       |    `keep`     | See [Netdata process scheduling policy](/docs/getting-started/manage-and-configure/netdata-daemon#netdata-process-scheduling-policy)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|               OOM score               |      `0`      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
|  glibc malloc arena max for plugins   |      `1`      | See [Virtual memory](/docs/getting-started/manage-and-configure/netdata-daemon#virtual-memory).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|  glibc malloc arena max for Netdata   |      `1`      | See [Virtual memory](/docs/getting-started/manage-and-configure/netdata-daemon#virtual-memory).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
|                   retention                   |   `3600`   | Used with `mode = save/map/ram/alloc`, not the default `mode = dbengine`. This number reflects the number of entries the `netdata` daemon will by default keep in memory for each chart dimension. Check [Memory Requirements](/docs/deployment-in-production/metric-retention---database) for more information.                                                                                                                                                                                                                                                                                                                                                           |
|                 storage tiers                 |    `1`     | The number of storage tiers you want to have in your dbengine. Check the tiering mechanism in the [dbengine's reference](/docs/deployment-in-production/metric-retention---database/database-engine#tiering). You can have up to 5 tiers of data (including the _Tier 0_). This number ranges between 1 and 5.                                                                                                                                                                                                                                                                                                                                                                      |
...
|                 update every                  |    `1`     | The frequency in seconds, for data collection. For more information see the [performance guide](/docs/deployment-in-production/how-to-optimize-the-netdata-agent-s-performance). These metrics stored as _Tier 0_ data. Explore the tiering mechanism in the [dbengine's reference](/docs/deployment-in-production/metric-retention---database/database-engine#tiering).                                                                                                                                                                                                                                                                                                                                                     |
...
|          memory deduplication (ksm)           |   `yes`    | When set to `yes`, Netdata will offer its in-memory round robin database and the dbengine page cache to kernel same page merging (KSM) for deduplication. For more information check [Memory Deduplication - Kernel Same Page Merging - KSM](/docs/deployment-in-production/metric-retention---database#ksm)                                                                                                                                                                                                                                                                                                                                                               |
|      cleanup obsolete charts after secs       |   `3600`   | See [monitoring ephemeral containers](/docs/data-collection/monitor-anything/Virtualized%20Environments/Containers-and-VMs-cgroups#monitoring-ephemeral-containers), also sets the timeout for cleaning up obsolete dimensions                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
|         delete obsolete charts files          |   `yes`    | See [monitoring ephemeral containers](/docs/data-collection/monitor-anything/Virtualized%20Environments/Containers-and-VMs-cgroups#monitoring-ephemeral-containers), also affects the deletion of files for obsolete dimensions                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
|         log         |                         `/var/log/netdata`                         | The directory in which the [log files](/docs/getting-started/manage-and-configure/netdata-daemon#log-files) are kept.                                                                                                                                                     |
...
|      registry       |              `/opt/netdata/var/lib/netdata/registry`               | Contains the [registry](/docs/deployment-in-production/secure-your-nodes/registry) database and GUID that uniquely identifies each Netdata Agent                                                                                                                        |

### [logs] section options

|              setting               |            default            | info                                                                                                                                                                                                                                                                               |
|:----------------------------------:|:-----------------------------:|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|            debug flags             |     `0x0000000000000000`      | Bitmap of debug options to enable. For more information check [Tracing Options](/docs/getting-started/manage-and-configure/netdata-daemon#debugging).                                                                                                                                                                      |
|               debug                | `/var/log/netdata/debug.log`  | The filename to save debug information. This file will not be created if debugging is not enabled. You can also set it to `syslog` to send the debug messages to syslog, or `none` to disable this log. For more information check [Tracing Options](/docs/getting-started/manage-and-configure/netdata-daemon#debugging). |
...
monitoring](/docs/alerts-and-notifications).

[Alarm notifications](/docs/alerts-and-notifications/changed-notifications/agent-alert-notifications) are configured in `health_alarm_notify.conf`.
...
|                enabled alarms                  |                       *                          | Defines which alarms to load from both user and stock directories. This is a [simple pattern](/docs/improving-netdata---developers/libnetdata/simple-patterns) list of alarm or template names. Can be used to disable specific alarms. For example, `enabled alarms =  !oom_kill *` will load all alarms except `oom_kill`. |

### [web] section options

Refer to the [web server documentation](/docs/deployment-in-production/secure-your-nodes/web-server)
...
the [registry documentation](/docs/deployment-in-production/secure-your-nodes/registry).
...
|     setting     |                   default                    | info                                                                                                                                                  |
|:---------------:|:--------------------------------------------:|:------------------------------------------------------------------------------------------------------------------------------------------------------|
|  update every   | the value of `[global].update every` setting | The frequency in seconds the plugin should collect values. For more information check the [performance guide](/docs/deployment-in-production/how-to-optimize-the-netdata-agent-s-performance). |
