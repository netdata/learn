...
installation](/docs/getting-started/install-netdata#install-on-linux-with-one-line-installer), the **Netdata config
...
- `netdata.conf` is the main configuration file. This is where you'll find most configuration options. Read descriptions
  for each in the [daemon config](/docs/getting-started/manage-and-configure/daemon-configuration) doc.
...
- Various configuration files ending in `.conf` for [configuring plugins or
  collectors](/docs/data-collection/collectors-configuration) behave. Examples: `go.d.conf`,
...
- Various directories ending in `.d`, which contain other configuration files, each ending in `.conf`, for [configuring
  specific collectors](/docs/data-collection/collectors-configuration).
- `apps_groups.conf` is a configuration file for changing how applications/processes are grouped when viewing the
  **Application** charts from [`apps.plugin`](/docs/data-collection/monitor-anything/System%20Metrics/Application-monitoring) or
  [`ebpf.plugin`](/docs/data-collection/monitor-anything/System%20Metrics/Kernel-traces-metrics-eBPF).
- `health.d/` is a directory that contains [health configuration files](/docs/alerts-and-notifications/configure-alerts).
- `health_alarm_notify.conf` enables and configures [alarm notifications](/docs/alerts-and-notifications/notifications).
- `statsd.d/` is a directory for configuring Netdata's [statsd collector](/docs/data-collection/monitor-anything/Any%20Application%20-%20Generic%20Collectors/StatsD).
- `stream.conf` configures [parent-child streaming](/docs/deployment-in-production/streaming-and-replication/streaming-configuration-reference) between separate nodes running the Agent.
- `.environment` is a hidden file that describes the environment in which the Netdata Agent is installed, including the
  `PATH` and any installation options. Useful for [reinstalling](/docs/moved-installation/reinstall-netdata) or
  [uninstalling](/docs/moved-installation/uninstall-netdata) the Agent.
...
See [configure agent containers](/docs/moved-installation/moved-installation-methods/docker#configure-agent-containers).
...
After you make your changes, you need to [restart the Agent](/docs/getting-started/manage-and-configure/start-stop-or-restart-the-netdata-agent) with `sudo systemctl
