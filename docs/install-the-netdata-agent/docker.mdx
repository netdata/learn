...
> :warning: Warning
>  
> The [edit-config](/docs/typical-netdata-agent-configurations/configuration#the-netdata-config-directory) script doesn't work when executed on
> the host system.
...
[HAProxy](/docs/typical-netdata-agent-configurations/securing-netdata-agents/reverse-proxies/haproxy) or
...
[Netdata daemon command line options](/docs/typical-netdata-agent-configurations/netdata-daemon#command-line-options) such as the IP address Netdata will be
...
instance](/docs/netdata-architecture/security-and-privacy-design). This section contains an example of how to install Netdata with an SSL
