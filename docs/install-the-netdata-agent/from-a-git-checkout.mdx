...
-   `--disable-telemetry`: Opt-out of [anonymous statistics](/docs/typical-netdata-agent-configurations/securing-netdata-agents/anonymous-telemetry-events) we use to make
    Netdata better.
...
-   `--local-files`: Used for [offline installations](/docs/install-the-netdata-agent/offline-systems). Pass four file paths: the Netdata
...
Unlike the [`kickstart.sh`](/docs/install-the-netdata-agent/one-line-installer-for-all-linux-systems), the `netdata-installer.sh` script does
not allow you to automatically [connect](/docs/typical-netdata-agent-configurations/connect-agent-to-cloud) your node to Netdata Cloud immediately after installation.

See the [connect to cloud](/docs/typical-netdata-agent-configurations/connect-agent-to-cloud) doc for details on connecting a node with a manual installation of Netdata.
