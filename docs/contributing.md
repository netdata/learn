...
We already support various [backends](/docs/backends) and we intend to support more. A "sponsor" for a backend: 
...
Netdata delivers alarms via various [notification methods](/docs/health/notifications). A "sponsor" for a notification method:
...
We expect most contributions to be for new data collection plugins. You can read about how external plugins work [here](/docs/collectors/plugins.d/). Additional instructions are available for [Node.js plugins](/docs/collectors/node.d.plugin) and [Python plugins](/docs/collectors/python.d.plugin).

Of course we appreciate contributions for any other part of the NetData agent, including the [daemon](/docs/daemon), [backends for long term archiving](/docs/backends/), innovative ways of using the [REST API](/docs/web/api) to create cool [Custom Dashboards](/docs/web/gui/custom/) or to include NetData charts in other applications, similarly to what can be done with [Confluence](/docs/web/gui/confluence/).
