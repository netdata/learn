...
- **Intelligent ML**: Machine learning runs as low-priority background tasks, automatically yielding resources when needed for data collection. See [AI & ML Features](/docs/netdata-ai)
- **Built-in Scalability**: Native clustering and high-availability features enable organizations to scale horizontally without architectural changes. See [Observability Centralization Points](/docs/netdata-parents).
...
Netdata Agent is designed to be lightweight, making it suitable for deployment across entire infrastructures. Even with Machine Learning enabled, Netdata Agents typically consume less than 5% CPU of a single core (see [CPU Requirements](/docs/netdata-agent/resource-utilization/cpu) for default-settings sizing guidance). The exact figures depend on the number of metrics collected per server.
...
For more information and ways to further reduce Agent resource utilization, see [Agent Resource Utilization](/docs/netdata-agent/resource-utilization).
