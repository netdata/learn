...
engine](/database/engine.md) "spilling" historical metrics to disk when it uses all its available RAM.
...
See the [database engine documentation](/database/engine.md) or our [tutorial on metrics
retention](/docs/tutorials/longer-metrics-storage.md) for more details on lowering the database engine's memory requirements.
