...
- The [open-source Netdata](https://github.com/netdata/netdata) monitoring agent. If you don't have it installed on your
  node yet, [get started now](/docs/install-the-netdata-agent).

## Install dependencies

First make sure Netdata is using Python 3 when it runs Python-based data collectors. 

Next, open `netdata.conf` using [`edit-config`](/docs/typical-netdata-agent-configurations/configuration#use-edit-config-to-edit-configuration-files)
from within the [Netdata config directory](/docs/typical-netdata-agent-configurations/configuration#the-netdata-config-directory). Scroll down to the
...
Now you're ready to enable the collector and [restart Netdata](/docs/maintenance-operations-on-netdata-agents/starting-and-stopping-netdata-agents).
