...
collector](/docs/data-collection/monitor-anything/Any%20Application%20-%20Generic%20Collectors/Anomalies) on a Raspberry Pi. To our delight, it's actually quite
straightforward! 
...
- The [open-source Netdata](https://github.com/netdata/netdata) monitoring agent. If you don't have it installed on your
  node yet, [get started now](/docs/getting-started/install-netdata).

## Install dependencies

First make sure Netdata is using Python 3 when it runs Python-based data collectors. 

Next, open `netdata.conf` using [`edit-config`](/docs/getting-started/manage-and-configure/configuration#use-edit-config-to-edit-configuration-files)
from within the [Netdata config directory](/docs/getting-started/manage-and-configure/configuration#the-netdata-config-directory). Scroll down to the
...
Now you're ready to enable the collector and [restart Netdata](/docs/getting-started/manage-and-configure/start-stop-or-restart-the-netdata-agent).
...
retraining](/docs/data-collection/monitor-anything/Any%20Application%20-%20Generic%20Collectors/Anomalies#configuration) to happen less often if you wish.
