...
sidebar_position: "1434000"
learn_link: "https://learn.netdata.cloud/docs/miscellaneous/unsupervised-anomaly-detection-for-raspberry-pi-monitoring"
---
...
collector](/docs/data-collection/netdata-specific/anomalies) on a Raspberry Pi. To our delight, it's actually quite
straightforward! 
...
- The [open-source Netdata](https://github.com/netdata/netdata) monitoring agent. If you don't have it installed on your
  node yet, [get started now](/docs/installing).

## Install dependencies

First make sure Netdata is using Python 3 when it runs Python-based data collectors. 

Next, open `netdata.conf` using [`edit-config`](/docs/configuring/configuration#use-edit-config-to-edit-configuration-files)
from within the [Netdata config directory](/docs/configuring/configuration#the-netdata-config-directory). Scroll down to the
...
Now you're ready to enable the collector and [restart Netdata](/docs/maintaining/starting-and-stopping-netdata-agents).
...
retraining](/docs/data-collection/netdata-specific/anomalies#configuration) to happen less often if you wish.
