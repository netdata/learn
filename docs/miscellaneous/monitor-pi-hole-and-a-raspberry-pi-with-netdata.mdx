...
collector](/docs/data-collection/monitor-anything/Applications/Pi-hole) will autodetect the new service
...
If you're completely new to Netdata, look at the [Introduction](/docs/getting-started) section for a walkthrough of all its features. For a more expedited tour, see the [get started documentation](/docs/getting-started/install-netdata).

### Enable temperature sensor monitoring

You need to manually enable Netdata's built-in [temperature sensor
collector](/docs/data-collection/monitor-anything/Hardware/lm-sensors) to start collecting metrics.

> Netdata uses a few plugins to manage its [collectors](/docs/data-collection/collectors-configuration), each using a different language: Go,
...
engine](/docs/deployment-in-production/metric-retention---database/database-engine). On the Raspberry Pi used for this guide, Netdata collects 1,500 metrics every
...
and the [Database configuration documentation](/docs/deployment-in-production/metric-retention---database) to help you determine the right
...
[300+ integrations](/docs/data-collection/monitor-anything).

-   [Optimize performance](/docs/deployment-in-production/how-to-optimize-the-netdata-agent-s-performance) using tweaks developed for IoT devices.
-   [Stream Raspberry Pi metrics](/docs/deployment-in-production/streaming-and-replication/streaming-configuration-reference) to a parent host for easy access or longer-term storage.
-   [Tweak alarms](/docs/alerts-and-notifications/configure-alerts) for either Pi-hole or the health of your Raspberry Pi.
-   [Export metrics to external databases](/docs/second-move-exporting-data/exporting-reference) with the exporting engine.
