...
If you're completely new to Netdata, look at the [Introduction](https://github.com/netdata/netdata/blob/master/docs/getting-started/introduction.md) section for a walkthrough of all its features. For a more expedited tour, see the [get started documentation](/docs/install-the-netdata-agent).

### Enable temperature sensor monitoring

You need to manually enable Netdata's built-in [temperature sensor
collector](/docs/data-collection/hardware-devices-and-sensors/lm-sensors) to start collecting metrics.
...
engine](/docs/contributing-to-netdata/database-engine). On the Raspberry Pi used for this guide, Netdata collects 1,500 metrics every
...
and the [Database configuration documentation](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/database-modes-for-parent-child-setups) to help you determine the right
