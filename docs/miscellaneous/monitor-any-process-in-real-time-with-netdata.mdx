...
- One or more Linux nodes running [Netdata](/docs/install-the-netdata-agent)
- A general understanding of how
  to [configure the Netdata Agent](/docs/typical-netdata-agent-configurations/configuration)
  using `edit-config`.
...
[**`apps.plugin`**](/docs/data-collection/apm/application-monitoring) looks at the Linux
...
[**`ebpf.plugin`**](/docs/data-collection/linux-systems/ebpf-for-systems-monitoring): Netdata's extended
...
These groups are then reflected as [dimensions](/docs/contributing-to-netdata/web#dimensions)
...
the [appropriate method](/docs/maintenance-operations-on-netdata-agents/starting-and-stopping-netdata-agents) for your system, to start collecting utilization metrics
