...
sidebar_position: "50"
learn_link: "https://learn.netdata.cloud/docs/opentelemetry/transformations"
slug: "/opentelemetry/transformations"
---
...
Before you begin, complete [Ingest OpenTelemetry Metrics and Logs](/docs/opentelemetry/otlp-ingestion). These examples use the `transform` and `filter` processors from OpenTelemetry Collector Contrib `0.157.0`.
...
Use a receiver-level `json_parser` instead when every line from one file has the same format and you also need to parse its timestamp or severity during ingestion. See [Collect Logs with OpenTelemetry Collector](/docs/opentelemetry/logs-collection#parse-json-lines).
...
For production file collection, add persistent offsets as described in [Logs Collection](/docs/opentelemetry/logs-collection#application-log-files).

## Create metrics from matching logs

Transform and filter processors stay within one signal pipeline; they do not turn logs into metrics. Use a connector to bridge the logs and metrics pipelines. The maintained [logs-to-metrics recipe](/docs/opentelemetry/logs-to-metrics) shows a count connector, a real filter processor, metric-unit normalization, rate semantics, verification, and alerting guidance.
