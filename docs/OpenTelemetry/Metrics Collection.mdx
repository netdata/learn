...
sidebar_position: "30"
learn_link: "https://learn.netdata.cloud/docs/opentelemetry/metrics-collection"
slug: "/opentelemetry/metrics-collection"
---
...
Before you begin, complete [Ingest OpenTelemetry Metrics and Logs](/docs/opentelemetry/otlp-ingestion). The examples on this page use OpenTelemetry Collector Contrib `0.157.0` and the local, plaintext loopback endpoint from that guide. Use TLS when the Collector and Netdata Agent are on different hosts.
...
Use the smaller [host metrics smoke test](/docs/opentelemetry/otlp-ingestion#smoke-test-host-metrics) to verify the OTLP path. For a broader existing OpenTelemetry host pipeline, add the shared exporter and this receiver and pipeline:
...
The `prometheus` receiver scrapes Prometheus-format HTTP endpoints without requiring a Prometheus server. Netdata also has a [native Prometheus endpoint collector](/docs/collecting-metrics/collectors/applications/prometheus-endpoint), which is simpler when Netdata is the only destination.
...
The `redis` receiver connects to one Redis instance and uses the `INFO` command to collect client, memory, CPU, keyspace, command, network, and replication statistics. The Collector must be able to reach the Redis endpoint. Netdata also has a [native Redis collector](/docs/collecting-metrics/collectors/databases/redis).
...
The `nginx` receiver reads NGINX's built-in `stub_status` endpoint. Netdata's [native NGINX collector](/docs/collecting-metrics/collectors/web-servers-and-proxies/nginx) uses the same endpoint and is simpler when Netdata is the only destination.
...
- **Netdata shows one dimension per chart:** add or adjust an OTel metric mapping. See the mapping section of the [OpenTelemetry plugin reference](/docs/opentelemetry/opentelemetry-plugin-reference).
