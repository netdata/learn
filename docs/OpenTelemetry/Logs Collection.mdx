...
keywords: [opentelemetry, otel, journald, filelog, windows event log, kubernetes, macos, syslog, logs, receivers]
description: "Collect systemd journal, application file, Windows event channel, Kubernetes container, macOS unified log, and syslog records through OpenTelemetry Collector receivers"
sidebar_position: "40"
learn_link: "https://learn.netdata.cloud/docs/opentelemetry/logs-collection"
slug: "/opentelemetry/logs-collection"
---

# Collect Logs with OpenTelemetry Collector

Use these recipes to send systemd journal entries, application log files, Windows event channels, Kubernetes container logs, and the macOS unified log through an OpenTelemetry Collector to Netdata. For network-device syslog, use the dedicated [OpenTelemetry Collector syslog setup](/docs/opentelemetry/syslog-from-network-devices/opentelemetry-collector-setup). For when to centralize a source at all, see [Centralizing Logs with OpenTelemetry](/docs/logs-management/centralizing-logs-with-opentelemetry).

Before you begin, complete [Ingest OpenTelemetry Metrics and Logs](/docs/opentelemetry/otlp-ingestion). The examples use OpenTelemetry Collector Contrib `0.157.0` and the local, plaintext loopback endpoint from that guide. Use TLS when the Collector and Netdata Agent are on different hosts.
...
[Securing the OTLP Endpoint](/docs/opentelemetry/securing-the-otlp-endpoint). When you combine blocks, list every
...
If formats vary or transformation must happen after several receivers, use the shared [Transformations](/docs/opentelemetry/transformations) guide instead.
...
See [Log Storage and Retention](/docs/logs-management/log-storage-and-retention) for how tenant retention and offloading work.
