...
The `FATAL` and `ERROR` messages will always appear in the logs, and `INFO`can be filtered using [severity level](/docs/netdata-agent/configuration/daemon#logs-section-options) option.
