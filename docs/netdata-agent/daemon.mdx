...
For details about Netdata's logging system and configuration, see [Netdata Logging](/docs/developer-and-contributor-corner/libnetdata/log).
...
To modify the scheduling policy, [edit](/docs/netdata-agent/configuration#edit-a-configuration-file-using-edit-config) `netdata.conf`:
