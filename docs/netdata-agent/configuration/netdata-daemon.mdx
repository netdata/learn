...
[start, stop, or restart the daemon](/docs/netdata-agent/installation#maintaining-a-netdata-agent-installation).
...
You can send commands during runtime via [netdatacli](/docs/netdata-agent/agent-cli).
...
The `FATAL` and `ERROR` messages will always appear in the logs, and `INFO`can be filtered using [severity level](/docs/netdata-agent/configuration/daemon-configuration#logs-section-options) option.
...
The `collector.log` is the `stderr` of all [collectors](/docs/collecting-metrics/monitor-anything)
 run by `netdata`.
...
then execute this to [restart Netdata](/docs/netdata-agent/installation#maintaining-a-netdata-agent-installation):
