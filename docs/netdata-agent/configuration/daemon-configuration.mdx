...
the [web server access lists](/docs/netdata-agent/configuration/securing-agents/web-server-reference#access-lists).

`netdata.conf` has sections stated with `[section]`. You will see the following sections:

1. `[global]` to [configure](#global-section-options) the [Netdata daemon](/docs/netdata-agent/configuration/netdata-daemon).
...
5. `[environment variables]` to [configure](#environment-variables-section-options) the environment variables used
   Netdata.
6. `[sqlite]` to [configure](#sqlite-section-options) the [Netdata daemon](/docs/netdata-agent/configuration/netdata-daemon) SQLite settings.
7. `[ml]` to configure settings for [machine learning](/docs/machine-learning-and-anomaly-detection/ml-models-and-anomaly-detection).
8. `[health]` to [configure](#health-section-options) general settings for [health monitoring](/docs/alerts-&-notifications).
9. `[web]` to [configure the web server](/docs/netdata-agent/configuration/securing-agents/web-server-reference).
10. `[registry]` for the [Netdata registry](/docs/netdata-agent/registry).
11. `[global statistics]` for the [Netdata registry](/docs/netdata-agent/registry).
...
13. `[plugins]` to [configure](#plugins-section-options) which [collectors](/docs/collecting-metrics) to use and PATH
    settings.
...
After `netdata.conf` has been modified, Netdata needs to be [restarted](/docs/netdata-agent/maintenance/service-control) for
changes to apply:
...
|:----------------------------------:|:-------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|     process scheduling policy      |    `keep`     | See [Netdata process scheduling policy](/docs/netdata-agent/configuration/netdata-daemon#netdata-process-scheduling-policy)                                                                                                                                             |
|             OOM score              |      `0`      |                                                                                                                                                                                                                                              |
| glibc malloc arena max for plugins |      `1`      | See [Virtual memory](/docs/netdata-agent/configuration/netdata-daemon#virtual-memory).                                                                                                                                                                                  |
| glibc malloc arena max for Netdata |      `1`      | See [Virtual memory](/docs/netdata-agent/configuration/netdata-daemon#virtual-memory).                                                                                                                                                                                  |
...
|                   retention                   |             `3600`             | Used with `mode = ram/alloc`, not the default `mode = dbengine`. This number reflects the number of entries the `netdata` daemon will by default keep in memory for each chart dimension. Check [Memory Requirements](/docs/netdata-agent/resource-utilization/disk-&-retention) for more information.                                                                                                                                                                                                                                                                          |
|                 storage tiers                 |              `3`               | The number of storage tiers you want to have in your dbengine. Check the tiering mechanism in the [dbengine's reference](/docs/developer-and-contributor-corner/database-engine#tiering). You can have up to 5 tiers of data (including the _Tier 0_). This number ranges between 1 and 5.                                                                                                                                                                                                                                                                                                                                 |
...
|                 update every                  |              `1`               | The frequency in seconds, for data collection. For more information see the [performance guide](/docs/netdata-agent/configuration/how-to-optimize-the-netdata-agent-s-performance). These metrics stored as _Tier 0_ data. Explore the tiering mechanism in the [dbengine's reference](/docs/developer-and-contributor-corner/database-engine#tiering).                                                                                                                                                                                                                                                                         |
...
|         cleanup obsolete charts after         |              `1h`              | See [monitoring ephemeral containers](https://github.com/netdata/netdata/blob/master/src/collectors/cgroups.plugin/README.md#monitoring-ephemeral-containers), also sets the timeout for cleaning up obsolete dimensions                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
|         log         |                         `/var/log/netdata`                         | The directory in which the [log files](/docs/netdata-agent/configuration/netdata-daemon#log-files) are kept.                                                                                                  |
...
|      registry       |              `/opt/netdata/var/lib/netdata/registry`               | Contains the [registry](/docs/netdata-agent/registry) database and GUID that uniquely identifies each Netdata Agent                                                                     |

### [logs] section options

There are additional configuration options for the logs. For more info, see [Netdata Logging](/docs/developer-and-contributor-corner/libnetdata/log).

|             setting              |            default            | info                                                                                                                                                                                                                                                                                  |
|:--------------------------------:|:-----------------------------:|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|           debug flags            |     `0x0000000000000000`      | Bitmap of debug options to enable. For more information check [Tracing Options](/docs/netdata-agent/configuration/netdata-daemon#debugging).                                                                                                                                                                     |
|              debug               | `/var/log/netdata/debug.log`  | The filename to save debug information. This file will not be created if debugging is not enabled. You can also set it to `syslog` to send the debug messages to syslog, or `off` to disable this log. For more information check [Tracing Options](/docs/netdata-agent/configuration/netdata-daemon#debugging). |
...
|             enabled alarms             |                        *                         | Defines which alerts to load from both user and stock directories. This is a [simple pattern](/docs/developer-and-contributor-corner/libnetdata/simple-patterns) list of alert or template names. Can be used to disable specific alerts. For example, `enabled alarms =  !oom_kill *` will load all alerts except `oom_kill`. |

### [web] section options

Refer to the [web server documentation](/docs/netdata-agent/configuration/securing-agents/web-server-reference)
...
the [registry documentation](/docs/netdata-agent/registry).
...
Most internal plugins will provide additional options. Check [Internal Plugins](/docs/collecting-metrics) for more
information.
...
|     setting     |                   default                    | info                                                                                                                                                                                         |
|:---------------:|:--------------------------------------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|  update every   | the value of `[global].update every` setting | The frequency in seconds the plugin should collect values. For more information check the [performance guide](/docs/netdata-agent/configuration/how-to-optimize-the-netdata-agent-s-performance). |
