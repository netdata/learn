...
1. `dbengine`, (the default) data are in database files. The [Database Engine](/docs/developer-and-contributor-corner/database-engine) works like a
   ...
   details see [here](/docs/developer-and-contributor-corner/database-engine).
...
[Parent - Child](/docs/observability-centralization-points) setups at the children side and only when the
resource constraints are very strict.

So,

- On a single node setup, use `[db].mode = dbengine`.
- On a [Parent - Child](/docs/observability-centralization-points) setup, use `[db].mode = dbengine` on the
...
When having a [**parent - child**](/docs/observability-centralization-points) setup, the child (the
...
You can also disable [data collection plugins](/docs/collecting-metrics) that you don't need. Disabling such plugins will also
