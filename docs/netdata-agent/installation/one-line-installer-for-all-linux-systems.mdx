...
learn_link: "https://learn.netdata.cloud/docs/netdata-agent/installation/one-line-installer-for-all-linux-systems"
---
...
If you are installing on macOS, make sure to check the [install documentation for macOS](/docs/netdata-agent/installation/macos) before continuing.
...
> :bulb: Tip
>
> If you are unsure whether you want nightly or stable releases, read the [installation guide](/docs/netdata-agent/installation#nightly-vs-stable-releases).  
...
- `--claim-proxy`
  Specify a proxy to use when connecting to the cloud in the form of `http://[user:pass@]host:ip` for an HTTP(S) proxy. See [connecting through a proxy](/docs/developer-and-contributor-corner/configuring/connect-agent-to-cloud#connect-through-a-proxy) for details.
- `--claim-only`
...
- `--prepare-offline-install-source`
  Instead of insallling the agent, prepare a directory that can be used to install on another system without needing to download anything. See our [offline installation documentation](/docs/netdata-agent/installation/offline-systems) for more info.
