...
For most DEB- or RPM-based Linux distributions, Netdata provides pre-built native packages, following our [platform support policy](/docs/netdata-agent/versions-&-platforms).  

Our [kickstart.sh installer](/docs/netdata-agent/installation/linux) uses these by default on supported platforms. To force native DEB or RPM packages, add `--native-only` when running the script—it will fail if native packages aren’t available.
...
Netdata’s official RPM repositories are hosted at [https://repository.netdata.cloud/repos/index.html](https://repository.netdata.cloud/repos/index.html). We provide four groups of
...
Netdata’s official DEB repositories are hosted at [https://repository.netdata.cloud/repos/index.html](https://repository.netdata.cloud/repos/index.html). 
