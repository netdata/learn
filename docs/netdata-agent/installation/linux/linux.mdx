...
- Attempts to install Netdata using our [official native binary packages](/docs/netdata-agent/installation/linux/native-linux-distribution-packages).
- If there are no official native binary packages for your system (or installing that way failed), tries to install using a [static build of Netdata](/docs/netdata-agent/installation/linux/static-binary-linux-packages) if one is available.
...
> **Tip**
>
> If you are unsure whether you want nightly or stable releases, read the [related section](/docs/netdata-agent/versions-&-platforms) of our Documentation, detailing the pros and cons of each release type.
...
```bash
[ "e8924943fb2b089190df8c9f2cb12162" = "$(curl -Ss https://get.netdata.cloud/kickstart.sh | md5sum | cut -d ' ' -f 1)" ] && echo "OK, VALID" || echo "FAILED, INVALID"
```
