...
> :bulb: Tip
>
> If you are unsure whether you want nightly or stable releases, read the [related section](/docs/netdata-agent/installation#nightly-vs-stable-releases) of our Documentation, detailing the pros and cons of each release type.
...
```bash
[ "895ba67aadc9c3fb20b6ad386149c77d" = "$(curl -Ss https://get.netdata.cloud/kickstart.sh | md5sum | cut -d ' ' -f 1)" ] && echo "OK, VALID" || echo "FAILED, INVALID"
```
...
>
> You do not need to restart the Netdata Agent between changes to health configuration files, see the relevant section on [reloading health configuration](/docs/alerts-&-notifications/alert-configuration-reference#reload-health-configuration).
...
The Netdata Agent also comes with a [CLI tool](/docs/netdata-agent/agent-cli) capable of performing shutdowns. Start the Agent back up using your preferred method listed above.
...
- `--claim-proxy`
  Specify a proxy to use when connecting to the cloud in the form of `http://[user:pass@]host:ip` for an HTTP(S) proxy. See [connecting through a proxy](/docs/netdata-cloud/connect-agent-to-cloud#automatically-via-a-provisioning-system-or-the-command-line) for details.
- `--claim-only`
...
- `--prepare-offline-install-source`
  Instead of installing the agent, prepare a directory that can be used to install on another system without needing to download anything. See our [offline installation documentation](/docs/netdata-agent/installation/linux/offline-systems) for more info.
...
We publish [official DEB/RPM packages](/docs/netdata-agent/installation/linux/native-linux-distribution-packages) for a number of common Linux distributions as part of our releases and nightly
...
We publish pre-built [static builds](/docs/netdata-agent/installation/linux/static-binary-linux-packages) of Netdata for Linux systems. Currently, these are published for 64-bit x86, ARMv7,
