...
```bash
[ "7ca458b7f1dbe32485c06b6d232a3573" = "$(curl -Ss https://get.netdata.cloud/kickstart.sh | md5sum | cut -d ' ' -f 1)" ] && echo "OK, VALID" || echo "FAILED, INVALID"
```
...
- `--claim-proxy`
  Specify a proxy to use when connecting to the cloud in the form of `http://[user:pass@]host:ip` for an HTTP(S) proxy. See [connecting through a proxy](/docs/netdata-cloud/connect-agent-to-cloud#connect-through-a-proxy) for details.
- `--claim-only`
