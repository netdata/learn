...
```bash
[ "581e8c1aa843ed8f277219eb24ccd761" = "$(curl -Ss https://get.netdata.cloud/kickstart.sh | md5sum | cut -d ' ' -f 1)" ] && echo "OK, VALID" || echo "FAILED, INVALID"
```
...
>
> You do not need to restart the Netdata Agent between changes to health configuration files, see the relevant section on [reloading health configuration](/docs/alerts-&-notifications/health-configuration-reference#reload-health-configuration).
...
- `--claim-proxy`
  Specify a proxy to use when connecting to the cloud in the form of `http://[user:pass@]host:ip` for an HTTP(S) proxy. See [connecting through a proxy](/docs/netdata-agent/configuration/connect-agent-to-cloud#connect-through-a-proxy) for details.
- `--claim-only`
...
- `--prepare-offline-install-source`
  Instead of insallling the agent, prepare a directory that can be used to install on another system without needing to download anything. See our [offline installation documentation](/docs/netdata-agent/installation/linux/offline-systems) for more info.
