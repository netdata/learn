...
Discover the recommended way to [configure Netdata's settings or behavior](/docs/netdata-agent/configuration) using our built-in
`edit-config` script, then apply that knowledge to mission-critical tweaks, such as [changing how long Netdata stores
metrics](/docs/developer-and-contributor-corner/configuring/optimizing-metrics-database/change-how-long-netdata-stores-metrics).
...
collector](/docs/data-collection/monitor-anything) for metrics you need, [configure the collector](/docs/developer-and-contributor-corner/configuring/collectors-configuration)
...
community, but you may want to [edit alerts](/docs/alerting/health-configuration-reference) or
...
[kickstart script](/docs/netdata-agent/installation/one-line-installer-for-all-linux-systems).
...
<InstallRegexLink>
  <InstallBoxRegexLink
    to="[](/docs/netdata-agent/installation/docker)"
    os="Run with Docker"
    svg="docker" />
  <InstallBoxRegexLink
    to="[](/docs/netdata-agent/installation/kubernetes)"
    os="Deploy on Kubernetes"
    svg="kubernetes" />
   <InstallBoxRegexLink
    to="[](/docs/netdata-agent/installation/macos)"
    os="Install on macOS"
    svg="macos" />
  <InstallBoxRegexLink
    to="[](/docs/netdata-agent/installation/native-linux-distribution-packages)"
    os="Native DEB/RPM packages"
    svg="linux" />
  <InstallBoxRegexLink
    to="[](/docs/netdata-agent/installation/from-a-git-checkout)"
    os="Linux from Git"
    svg="linux" />
  <InstallBoxRegexLink
    to="[](/docs/netdata-agent/installation/build-the-netdata-agent-yourself/compile-from-source-code)"
    os="Linux from source"
    svg="linux" />
  <InstallBoxRegexLink
    to="[](/docs/netdata-agent/installation/offline-systems)"
    os="Linux for offline nodes"
    svg="linux" />
</InstallRegexLink>

- [Native DEB/RPM packages](/docs/netdata-agent/installation/native-linux-distribution-packages)
- [Run with Docker](/docs/netdata-agent/installation/docker)
- [Deploy on Kubernetes](/docs/netdata-agent/installation/kubernetes)
- [Install on macOS](/docs/netdata-agent/installation/macos)
- [Linux from Git](/docs/netdata-agent/installation/from-a-git-checkout)
- [Linux from source](/docs/netdata-agent/installation/build-the-netdata-agent-yourself/compile-from-source-code)
- [Linux for offline nodes](/docs/netdata-agent/installation/offline-systems)
...
2. If you are using a system that we publish native packages for (see our [platform support
   policy](/docs/netdata-agent/netdata-agent-versions-&-platforms) for more details),
   you can manually set up our native package repositories as outlined in our [native package install
   documentation](/docs/netdata-agent/installation/native-linux-distribution-packages). Our official
...
3. If neither of the above options work for you, you can still install using our [offline installation
   instructions](/docs/netdata-agent/installation/offline-systems), though
...
PowerTools, to gather hard dependencies. See the [CentOS 6](/docs/netdata-agent/installation/from-a-git-checkout#centos--rhel-6x) and
[CentOS 8](/docs/netdata-agent/installation/from-a-git-checkout#centos--rhel-8x) sections for more information.
...
errors](/docs/netdata-agent/installation/from-a-git-checkout#nonrepresentable-section-on-output-errors) for a workaround.
