...
If Netdata didn't autodetect all the hardware, containers, services, or applications running on your node, you should learn more about [how data collectors work](/docs/collecting-metrics). If there's a [supported integration](/docs/collecting-metrics/monitor-anything) for metrics you need, refer to its respective page and read about its requirements to configure your endpoint to publish metrics in the correct format and endpoint.
...
option when you install or update Netdata using the [automatic one-line installation script](/docs/netdata-agent/installation/linux).

```bash
wget -O /tmp/netdata-kickstart.sh https://get.netdata.cloud/kickstart.sh && sh /tmp/netdata-kickstart.sh --no-updates
```

With automatic updates disabled, you can choose exactly when and how you [update Netdata](/docs/netdata-agent/maintenance/update-netdata-agents).
