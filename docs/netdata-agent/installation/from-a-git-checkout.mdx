...
sidebar_position: "106520"
learn_link: "https://learn.netdata.cloud/docs/netdata-agent/installation/from-a-git-checkout"
---
...
Otherwise, [perform a cleanup](/docs/netdata-agent/installation/from-a-git-checkout#perform-a-cleanup-in-your-netdata-repo)
...
-   `--disable-telemetry`: Opt-out of [anonymous statistics](/docs/developer-and-contributor-corner/configuring/securing-netdata-agents/anonymous-telemetry-events) we use to make
    Netdata better.
...
-   `--local-files`: Used for [offline installations](/docs/netdata-agent/installation/offline-systems). Pass four file paths: the Netdata
...
Unlike the [`kickstart.sh`](/docs/netdata-agent/installation/one-line-installer-for-all-linux-systems), the `netdata-installer.sh` script does
not allow you to automatically [connect](/docs/developer-and-contributor-corner/configuring/connect-agent-to-cloud) your node to Netdata Cloud immediately after installation.

See the [connect to cloud](/docs/developer-and-contributor-corner/configuring/connect-agent-to-cloud) doc for details on connecting a node with a manual installation of Netdata.
