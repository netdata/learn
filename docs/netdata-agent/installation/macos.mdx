...
You could also use a macOS system as the parent node in a [streaming configuration](/docs/observability-centralization-points/metrics-centralization-points-%20netdata-parents%20/configuring-metrics-centralization-points).
...
> Community-maintained packages _may_ receive support from Netdata, but are only a best-effort affair. Learn more about [Netdata's platform support policy](/docs/netdata-agent/netdata-agent-versions-&-platforms).

## Install Netdata with our automatic one-line installation script

**Local Netdata Agent installation**
To install Netdata using our automatic [kickstart](/docs/netdata-agent/installation#automatic-one-line-installation-script) open a new terminal and run:
...
The `kickstart.sh` script accepts additional parameters to automatically [connect](/docs/developer-and-contributor-corner/configuring/connect-agent-to-cloud) your node to Netdata
...
- `--claim-proxy`: Specify a proxy to use when connecting to the cloud in the form of `http://[user:pass@]host:ip` for an HTTP(S) proxy.
  See [connecting through a proxy](/docs/developer-and-contributor-corner/configuring/connect-agent-to-cloud#connect-through-a-proxy) for details.
...
If you experience issues while claiming your node, follow the steps in our [Troubleshooting](/docs/developer-and-contributor-corner/configuring/connect-agent-to-cloud#troubleshooting) documentation.
