...
The simplest method is to use the single line [kickstart script](/docs/netdata-agent/installation/linux)
...
page](/docs/netdata-agent/configuration/anonymous-telemetry-events).
...
- `--prepare-offline-install-source`: Instead of installing the Agent, prepare a directory that can be used to install on another system without needing to download anything. See our [offline installation documentation](/docs/netdata-agent/installation/linux/offline-systems) for more info.
