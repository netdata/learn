...
installation](/docs/netdata-agent/installation#install-on-linux-with-one-line-installer), the **Netdata config
...
- `netdata.conf` is the main configuration file. This is where you'll find most configuration options. Read descriptions
  for each in the [daemon config](/docs/developer-and-contributor-corner/configuring/daemon-configuration) doc.
...
- Various configuration files ending in `.conf` for [configuring plugins or
  collectors](/docs/developer-and-contributor-corner/configuring/collectors-configuration) behave. Examples: `go.d.conf`,
...
- Various directories ending in `.d`, which contain other configuration files, each ending in `.conf`, for [configuring
  specific collectors](/docs/developer-and-contributor-corner/configuring/collectors-configuration).
...
- `stream.conf` configures [parent-child streaming](/docs/observability-centralization-points/metrics-centralization-points-%20netdata-parents%20/configuring-metrics-centralization-points) between separate nodes running the Agent.
...
See [configure agent containers](/docs/netdata-agent/installation/docker#configure-agent-containers).
