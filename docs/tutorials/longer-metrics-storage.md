...
connector](/exporting.md).

This tutorial gives two options for configuring Netdata to store more metrics. **We recommend the default [database
engine](/docs/tutorials/#using-the-database-engine)**, but you can stick with or switch to the round-robin database if you prefer.
...
footprint](/database/engine.md#memory-requirements#memory-requirements).
...
charts](/web.md#using-charts#using-charts).

And if you'd now like to reduce Netdata's resource usage, view our [performance guide](/performance.md) for
