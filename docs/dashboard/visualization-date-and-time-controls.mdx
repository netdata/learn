...
While [panning through time and zooming in/out](/docs/dashboard/interact-charts) from charts it is helpful when
...
At any time, [configure the internal TSDB's storage capacity](/docs/store/change-metrics-storage) to expand your
depth of historical metrics.

## What's next?

One useful next step after selecting a timeframe is [exporting the
metrics](/docs/dashboard/import-export-print-snapshot) into a snapshot file, which can then be shared and imported
into any other Netdata dashboard.

There are also many ways to [customize](/docs/dashboard/customize) the standard dashboard experience, from changing
