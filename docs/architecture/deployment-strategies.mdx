...
The Agent will act as a _stand-alone_ Agent by default, and this is great to start out with for small setups and home labs. By [connecting each Agent to Cloud](/docs/configuring/connect-agent-to-cloud), you can see an overview of all your nodes, with aggregated charts and centralized alerting, without setting up a Parent.

![image](/img/architecture/deployment-standalone.svg)
...
![image](/img/architecture/deployment-parent.svg)
...
![image](/img/architecture/deployment-parents.svg)
...
As mentioned above, the recommendation is to not claim the Child to Cloud directly during your setup, avoiding establishing an [ACLK](/docs/agent/aclk) connection.
...
For the Parent, besides setting up streaming, the example will also provide an example configuration of multiple [tiers](/docs/agent/database/engine#tiering) of metrics [storage](/docs/store/change-metrics-storage), for 10 children, with about 2k metrics each.
