...
As mentioned above, the recommendation is to not claim the Child to Cloud directly during your setup, avoiding establishing an [ACLK](/docs/configuring/securing-netdata-agents/agent-cloud-link-aclk) connection.
...
For the Parent, besides setting up streaming, the example will also provide an example configuration of multiple [tiers](/docs/architecture/database-engine#tiering) of metrics [storage](/docs/configuring/optimizing-metrics-database/change-how-long-netdata-stores-metrics), for 10 children, with about 2k metrics each.
