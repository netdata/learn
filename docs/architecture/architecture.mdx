...
But the best way to get under the hood or in the steering wheel of our highly efficient, low-latency system (supporting multiple readers and one writer on each metric) is to read the rest of our docs, or just to jump in and [get started](/docs/installation). But here's a good breakdown:
...
Package maintainers should read the guide on [building Netdata from source](/docs/installation/build-the-netdata-agent-yourself/compile-from-source-code) for
