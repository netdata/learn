...
list](/docs/collectors/collectors). The only exception is the [third-party collectors](/docs/collectors/collectors#third-party-plugins), which
...
From here on out, this quickstart guide will use the [Nginx collector](/docs/collectors/go.d.plugin/modules/nginx) as an
...
Netdata's collector. Our [collector support list](/docs/collectors/collectors) contains links to each collector's documentation page
so you can learn more.

For example, the [Nginx collector documentation](/docs/collectors/go.d.plugin/modules/nginx) states that your Nginx
...
directory](/docs/step-by-step/step-04#find-your-netdataconf-file). Launch `edit-config` with the path to the
...
For example, the [Nginx collector](/docs/collectors/go.d.plugin/modules/nginx) has its configuration file at `go.d/nginx.conf`.
...
methods](/docs/getting-started#start-stop-and-restart-netdata).
...
reference](/docs/collectors/reference#troubleshoot-a-collector).

View our [collectors tutorials](/docs/collectors/readme#tutorials) to get specific instructions on enabling new and popular
collectors.

Finally, learn more advanced collector features, such as disabling plugins or developing a custom collector, in our
[internal plugin API](/docs/collectors/reference#internal-plugins-api) or our [external plugin docs](/docs/collectors/plugins.d).
