...
[Change how long Netdata stores metrics](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/change-how-long-netdata-stores-metrics) | | :heavy_check_mark: | :heavy_check_mark:
[Use a different metric storage database](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/database-modes-for-parent-child-setups) | | :heavy_check_mark: | :heavy_check_mark:
...
The Netdata Agent runs with the lowest possible [process scheduling policy](/docs/typical-netdata-agent-configurations/netdata-daemon#netdata-process-scheduling-policy), which is `nice 19`, and uses the `idle` process scheduler.
...
To estimate and control memory consumption, you can [disable unneeded plugins or collectors](#disable-unneeded-plugins-or-collectors), [change how long Netdata stores metrics](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/change-how-long-netdata-stores-metrics), or [use a different metric storage database](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/database-modes-for-parent-child-setups).
...
To configure retention, you can [change how long Netdata stores metrics](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/change-how-long-netdata-stores-metrics).
To control disk I/O [use a different metric storage database](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/database-modes-for-parent-child-setups), avoid querying the 
...
collected data from children Netdata nodes running on the production infrastructure, using [streaming and replication](/docs/metrics-streaming-and-replication/understanding-how-streaming-works).
...
See [using a different metric storage database](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/database-modes-for-parent-child-setups).

## Disable unneeded plugins or collectors

If you know that you don't need an [entire plugin or a specific
collector](/docs/data-collection#collector-architecture-and-terminology), you can disable any of them.
...
dashboard, [configure the Agent](/docs/typical-netdata-agent-configurations/configuration) to collect metrics less often.
...
every` for an individual collector is less than the global, the Netdata Agent uses the global setting. See the [collectors configuration reference](/docs/data-collection/collectors-configuration) for details.

To reduce the frequency of an [internal
plugin/collector](/docs/data-collection#collector-architecture-and-terminology), open `netdata.conf` and
...
To [configure an individual collector](/docs/data-collection/collectors-configuration#configure-a-collector), open its specific configuration file with
...
See how to [change how long Netdata stores metrics](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/change-how-long-netdata-stores-metrics).

## Use a different metric storage database

Consider [using a different metric storage database](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/database-modes-for-parent-child-setups) when running Netdata on IoT devices,
and for children in a parent-child set up based on [streaming and replication](/docs/metrics-streaming-and-replication/understanding-how-streaming-works).
...
A dedicated web server like nginx provides more robustness than the Agent's internal [web server](/docs/contributing-to-netdata/web).
Nginx can handle more concurrent connections, reuse idle connections, and use fast gzip compression to reduce payloads.

For details on installing another web server as a proxy for the local Agent dashboard, see [reverse proxies](/docs/typical-netdata-agent-configurations/securing-netdata-agents/reverse-proxies).
