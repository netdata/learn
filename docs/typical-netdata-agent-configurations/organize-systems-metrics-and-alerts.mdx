...
[Spaces](/docs/monitor-your-infrastructure/spaces) are used for organization-level or infrastructure-level 
grouping of nodes and people. A node can only appear in a single space, while people can have access to multiple spaces.

The [war rooms](/docs/monitor-your-infrastructure/war-rooms) in a space bring together nodes and people in 
collaboration areas. War rooms can also be used for fine-tuned 
[role based access control](/docs/typical-netdata-agent-configurations/securing-netdata-agents/role-based-access-model). 
...
to violate any of the [host label naming rules](/docs/typical-netdata-agent-configurations/common-configuration-changes#organize-nodes-with-host-labels).
...
> documentation](/docs/metrics-streaming-and-replication/streaming-configuration-reference#securing-streaming-communications) for details. You may also want to use
> [access lists](/docs/typical-netdata-agent-configurations/securing-netdata-agents/web-server#access-lists) or [expose the API only to LAN/localhost
> connections](/docs/netdata-architecture/security-and-privacy-design#expose-netdata-only-in-a-private-lan).
...
documentation](/docs/alerts-and-notifications/health-configuration-reference#alarm-line-host-labels) for more details, and then get creative!

### Host labels in metrics exporting

If you have enabled any metrics exporting via our experimental [exporters](/docs/exporting-data-to-other-systems/exporting-reference), any new host
...
more about exporting, read the [documentation](/docs/exporting-data-to-other-systems/exporting-reference).
