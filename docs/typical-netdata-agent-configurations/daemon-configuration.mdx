...
learn_rel_path: "Typical Netdata Agent Configurations"
...
the [web server access lists](/docs/typical-netdata-agent-configurations/securing-netdata-agents/web-server#access-lists).

`netdata.conf` has sections stated with `[section]`. You will see the following sections:

1. `[global]` to [configure](#global-section-options) the [Netdata daemon](/docs/typical-netdata-agent-configurations/netdata-daemon).
...
5. `[environment variables]` to [configure](#environment-variables-section-options) the environment variables used
   Netdata.
6. `[sqlite]` to [configure](#sqlite-section-options) the [Netdata daemon](/docs/typical-netdata-agent-configurations/netdata-daemon) SQLite settings.
7. `[ml]` to configure settings for [machine learning](/docs/machine-learning-and-assisted-troubleshooting/machine-learning-ml-powered-anomaly-detection).
8. `[health]` to [configure](#health-section-options) general settings for [health monitoring](/docs/alerts-and-notifications).
9. `[web]` to [configure the web server](/docs/typical-netdata-agent-configurations/securing-netdata-agents/web-server).
10. `[registry]` for the [Netdata registry](/docs/typical-netdata-agent-configurations/securing-netdata-agents/registry).
11. `[global statistics]` for the [Netdata registry](/docs/typical-netdata-agent-configurations/securing-netdata-agents/registry).
...
After `netdata.conf` has been modified, Netdata needs to be [restarted](/docs/maintenance-operations-on-netdata-agents/starting-and-stopping-netdata-agents) for
changes to apply:
...
|:-------------------------------------:|:-------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       process scheduling policy       |    `keep`     | See [Netdata process scheduling policy](/docs/typical-netdata-agent-configurations/netdata-daemon#netdata-process-scheduling-policy)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|               OOM score               |      `0`      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
|  glibc malloc arena max for plugins   |      `1`      | See [Virtual memory](/docs/typical-netdata-agent-configurations/netdata-daemon#virtual-memory).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|  glibc malloc arena max for Netdata   |      `1`      | See [Virtual memory](/docs/typical-netdata-agent-configurations/netdata-daemon#virtual-memory).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
|                   retention                   |   `3600`   | Used with `mode = save/map/ram/alloc`, not the default `mode = dbengine`. This number reflects the number of entries the `netdata` daemon will by default keep in memory for each chart dimension. Check [Memory Requirements](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/database-modes-for-parent-child-setups) for more information.                                                                                                                                                                                                                                                                                                                                                           |
|                 storage tiers                 |    `1`     | The number of storage tiers you want to have in your dbengine. Check the tiering mechanism in the [dbengine's reference](/docs/contributing-to-netdata/database-engine#tiering). You can have up to 5 tiers of data (including the _Tier 0_). This number ranges between 1 and 5.                                                                                                                                                                                                                                                                                                                                                                      |
...
|                 update every                  |    `1`     | The frequency in seconds, for data collection. For more information see the [performance guide](/docs/typical-netdata-agent-configurations/how-to-optimize-the-netdata-agent-s-performance). These metrics stored as _Tier 0_ data. Explore the tiering mechanism in the [dbengine's reference](/docs/contributing-to-netdata/database-engine#tiering).                                                                                                                                                                                                                                                                                                                                                     |
...
|         log         |                         `/var/log/netdata`                         | The directory in which the [log files](/docs/typical-netdata-agent-configurations/netdata-daemon#log-files) are kept.                                                                                                                                                     |
...
|      registry       |              `/opt/netdata/var/lib/netdata/registry`               | Contains the [registry](/docs/typical-netdata-agent-configurations/securing-netdata-agents/registry) database and GUID that uniquely identifies each Netdata Agent                                                                                                                        |

### [logs] section options

|              setting               |            default            | info                                                                                                                                                                                                                                                                               |
|:----------------------------------:|:-----------------------------:|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|            debug flags             |     `0x0000000000000000`      | Bitmap of debug options to enable. For more information check [Tracing Options](/docs/typical-netdata-agent-configurations/netdata-daemon#debugging).                                                                                                                                                                      |
|               debug                | `/var/log/netdata/debug.log`  | The filename to save debug information. This file will not be created if debugging is not enabled. You can also set it to `syslog` to send the debug messages to syslog, or `none` to disable this log. For more information check [Tracing Options](/docs/typical-netdata-agent-configurations/netdata-daemon#debugging). |
...
[Alarm notifications](/docs/alerts-and-notifications/notifications/netdata-agent-notifications) are configured in `health_alarm_notify.conf`.
...
|                enabled alarms                  |                       *                          | Defines which alarms to load from both user and stock directories. This is a [simple pattern](/docs/contributing-to-netdata/libnetdata/simple-patterns) list of alarm or template names. Can be used to disable specific alarms. For example, `enabled alarms =  !oom_kill *` will load all alarms except `oom_kill`. |

### [web] section options

Refer to the [web server documentation](/docs/typical-netdata-agent-configurations/securing-netdata-agents/web-server)
...
the [registry documentation](/docs/typical-netdata-agent-configurations/securing-netdata-agents/registry).
...
|     setting     |                   default                    | info                                                                                                                                                  |
|:---------------:|:--------------------------------------------:|:------------------------------------------------------------------------------------------------------------------------------------------------------|
|  update every   | the value of `[global].update every` setting | The frequency in seconds the plugin should collect values. For more information check the [performance guide](/docs/typical-netdata-agent-configurations/how-to-optimize-the-netdata-agent-s-performance). |
