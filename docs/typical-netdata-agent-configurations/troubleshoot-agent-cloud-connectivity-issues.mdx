...
The most common issue we have seen claiming nodes in Docker is [running on older hosts with seccomp enabled](/docs/typical-netdata-agent-configurations/connect-agent-to-cloud#known-issues-on-older-hosts-with-seccomp-enabled).
...
> Read more about [Starting, Stopping and Restarting the Agent](/docs/maintenance-operations-on-netdata-agents/starting-and-stopping-netdata-agents).
