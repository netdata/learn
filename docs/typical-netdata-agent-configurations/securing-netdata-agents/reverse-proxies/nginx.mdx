...
In case Netdata's web server has been [configured to use TLS](/docs/typical-netdata-agent-configurations/securing-netdata-agents/web-server#enabling-tls-support), it is
...
`allow connections from` accepts [Netdata simple patterns](/docs/contributing-to-netdata/libnetdata/simple-patterns) to match against the
