...
learn_rel_path: "Typical Netdata Agent Configurations/Optimizing Metrics Database"
...
1. `dbengine`, (the default) data are in database files. The [Database Engine](/docs/contributing-to-netdata/database-engine) works like a
   ...
   details see [here](/docs/contributing-to-netdata/database-engine).
...
[Parent - Child](/docs/metrics-streaming-and-replication/understanding-how-streaming-works) setups at the children side and only when the
resource constraints are very strict.

So,

- On a single node setup, use `[db].mode = dbengine`.
- On a [Parent - Child](/docs/metrics-streaming-and-replication/understanding-how-streaming-works) setup, use `[db].mode = dbengine` on the
...
When having a [**parent - child**](/docs/metrics-streaming-and-replication/understanding-how-streaming-works) setup, the child (the
