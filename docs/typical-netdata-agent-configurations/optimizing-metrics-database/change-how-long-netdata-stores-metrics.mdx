...
sidebar_position: "306080"
---
# Change how long Netdata stores metrics

The Netdata Agent uses a custom made time-series database (TSDB), named the 
[`dbengine`](/docs/contributing-to-netdata/database-engine), to store metrics.
...
[DBENGINE memory requirements](/docs/contributing-to-netdata/database-engine#memory-requirements).
...
[`edit-config`](/docs/typical-netdata-agent-configurations/configuration#use-edit-config-to-edit-configuration-files) 
and make your changes in the `[db]` subsection. 

Save the file and restart the Agent with `sudo systemctl restart netdata`, or
the [appropriate method](/docs/maintenance-operations-on-netdata-agents/starting-and-stopping-netdata-agents) 
