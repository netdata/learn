...
installation](/docs/install-the-netdata-agent#install-on-linux-with-one-line-installer), the **Netdata config
...
- `netdata.conf` is the main configuration file. This is where you'll find most configuration options. Read descriptions
  for each in the [daemon config](/docs/typical-netdata-agent-configurations/daemon-configuration) doc.
...
- `apps_groups.conf` is a configuration file for changing how applications/processes are grouped when viewing the
  **Application** charts from [`apps.plugin`](/docs/data-collection/apm/application-monitoring) or
  [`ebpf.plugin`](/docs/data-collection/linux-systems/ebpf-for-systems-monitoring).
- `health.d/` is a directory that contains [health configuration files](/docs/alerts-and-notifications/health-configuration-reference).
- `health_alarm_notify.conf` enables and configures [alarm notifications](/docs/alerts-and-notifications/notifications).
- `statsd.d/` is a directory for configuring Netdata's [statsd collector](/docs/data-collection/apm/statsd).
- `stream.conf` configures [parent-child streaming](/docs/metrics-streaming-and-replication/streaming-configuration-reference) between separate nodes running the Agent.
- `.environment` is a hidden file that describes the environment in which the Netdata Agent is installed, including the
  `PATH` and any installation options. Useful for [reinstalling](/docs/maintenance-operations-on-netdata-agents/reinstall-netdata-agents) or
  [uninstalling](/docs/maintenance-operations-on-netdata-agents/uninstall-netdata-agents) the Agent.
...
See [configure agent containers](/docs/install-the-netdata-agent/docker#configure-agent-containers).
...
After you make your changes, you need to [restart the Agent](/docs/maintenance-operations-on-netdata-agents/starting-and-stopping-netdata-agents) with `sudo systemctl
