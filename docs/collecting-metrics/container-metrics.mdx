...
[Helmchart](/docs/netdata-agent/installation/kubernetes) to automate deployment, collectors for k8s agent services, and
...
[dockerd](/docs/collecting-metrics/containers-and-vms/docker) or [Docker
Engine](/docs/collecting-metrics/containers-and-vms/docker-engine). You can find all
container collectors in our supported collectors list under the
[containers/VMs](/docs/collecting-metrics/monitor-anything#containers-and-vms) and
[Kubernetes](/docs/collecting-metrics/monitor-anything#containers-and-vms) headings.
...
collector](/docs/collecting-metrics/databases/mysql) to look at `127.0.0.0:3306` for
MySQL metrics:
...
metrics](/docs/collecting-metrics/databases/mysql#charts) as well.

### Collect metrics from applications running in Docker containers

You could use this technique to monitor an entire infrastructure of Docker containers. The same [enable and configure](/docs/collecting-metrics/collectors-configuration) procedures apply whether an application runs on the host system or inside
a container. You may need to configure the target endpoint if it's not the application's default.

Netdata can even [run in a Docker container](/docs/netdata-agent/installation/docker) itself, and then collect metrics about the
host system, its own container with cgroups, and any applications you want to monitor.

See our [application metrics doc](/docs/collecting-metrics/application-metrics) for details about Netdata's application metrics
...
-   A [service discovery plugin](https://github.com/netdata/agent-service-discovery), which discovers and creates
    ...
    collector](/docs/collecting-metrics/generic-collecting-metrics/prometheus-endpoint). With these
...
-   A [Kubelet collector](/docs/collecting-metrics/kubernetes/kubelet), which runs
    on each node in a k8s cluster to monitor the number of pods/containers, the volume of operations on each container,
    and more.
-   A [kube-proxy collector](/docs/collecting-metrics/kubernetes/kubeproxy), which
...
with Netdata_](/docs/developer-and-contributor-corner/kubernetes-monitoring-with-netdata-overview-and-visualizations).

## What's next?

Netdata is capable of collecting metrics from hundreds of applications, such as web servers, databases, messaging
brokers, and more. See more in the [application metrics doc](/docs/collecting-metrics/application-metrics).

If you already have all the information you need about collecting metrics, move into Netdata's meaningful visualizations
with [seeing an overview of your infrastructure](/docs/dashboards-and-charts/infrastructure-overview) using Netdata Cloud.
