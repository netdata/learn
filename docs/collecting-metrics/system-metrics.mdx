...
devices, and [containers](/docs/collecting-metrics/container-metrics) with zero configuration.
...
[collecting container metrics](/docs/collecting-metrics/container-metrics) doc.
...
can find all system collectors in our [supported collectors list](/docs/collecting-metrics/monitor-anything#system-collectors).

## Collect Windows system metrics

Netdata is also capable of monitoring Windows systems. The [Windows
collector](/docs/collecting-metrics/windows-systems/windows) integrates with
...
details see [the requirements](/docs/collecting-metrics/windows-systems/windows#requirements).

Next, [configure](/docs/collecting-metrics/windows-systems/windows#configuration) the Windows
...
method](/docs/netdata-agent/installation#maintaining-a-netdata-agent-installation) for your system. You'll start seeing Windows system metrics, such as CPU
...
doc](/docs/collecting-metrics/application-metrics#collect-metrics-from-applications-running-on-windows).

## What's next?

Because there's some overlap between system metrics and [container metrics](/docs/collecting-metrics/container-metrics), you
should investigate Netdata's container compatibility if you use them heavily in your infrastructure.

If you don't use containers, skip ahead to collecting [application metrics](/docs/collecting-metrics/application-metrics) with
Netdata.
