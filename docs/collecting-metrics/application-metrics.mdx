...
- [Prometheus endpoints](/docs/collecting-metrics/generic-collecting-metrics/prometheus-endpoint): Gathers
  metrics from one or more Prometheus endpoints that use the OpenMetrics exposition format. Auto-detects more than 600
  endpoints.
- [Web server logs (Apache, NGINX)](/docs/collecting-metrics/web-servers-and-web-proxies/web-server-log-files):
...
- [MySQL](/docs/collecting-metrics/databases/mysql): Collect database global,
  replication, and per-user statistics.
- [Redis](/docs/collecting-metrics/databases/redis): Monitor database status by
  reading the server's response to the `INFO` command.
- [Apache](/docs/collecting-metrics/web-servers-and-web-proxies/apache): Collect Apache web server
  performance metrics via the `server-status?auto` endpoint.
- [Nginx](/docs/collecting-metrics/web-servers-and-web-proxies/nginx): Monitor web server status
  information by gathering metrics via `ngx_http_stub_status_module`.
- [Postgres](/docs/collecting-metrics/databases/postgresql): Collect database health
  and performance metrics.
- [ElasticSearch](/docs/collecting-metrics/search-engines/elasticsearch): Collect search
  engine performance and health statistics. Optionally collects per-index metrics.
- [PHP-FPM](/docs/collecting-metrics/web-servers-and-web-proxies/php-fpm): Collect application summary
  and processes health metrics by scraping the status page (`/status?full`).

Our [supported collectors list](/docs/collecting-metrics/monitor-anything#service-and-application-collectors) shows all Netdata's
...
Once you have Netdata running on that separate system, you can follow the [collectors configuration reference](/docs/collecting-metrics/collectors-configuration) documentation to tell the collector to look for exposed metrics on the Windows system's IP
...
collector](/docs/collecting-metrics/databases/mysql) to look at `203.0.113.0:3306`:
...
list](/docs/collecting-metrics/monitor-anything#service-and-application-collectors) that can run on Windows.

## What's next?

If you haven't yet seen the [supported collectors list](/docs/collecting-metrics/monitor-anything) give it a once-over for any
additional applications you may want to monitor using Netdata's native collectors, or the [generic Prometheus
collector](/docs/collecting-metrics/generic-collecting-metrics/prometheus-endpoint).
...
infrastructure](/docs/dashboards-and-charts/infrastructure-overview) using Netdata Cloud.
