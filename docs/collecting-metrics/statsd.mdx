...
Private charts are controlled with `create private charts for metrics matching = *`. This setting accepts a space-separated list of [simple patterns](/docs/developer-and-contributor-corner/libnetdata/simple-patterns). Netdata will create private charts for all metrics **by default**.
...
You can specify Netdata StatsD to have a different `memory mode` than the rest of the Netdata Agent. You can read more about `memory mode` in the [documentation](/docs/netdata-agent/database).
...
-   `metrics` is a Netdata [simple pattern](/docs/developer-and-contributor-corner/libnetdata/simple-patterns). This pattern should match all the possible StatsD metrics that will be participating in the application `myapp`.
...
-   `history` sets the size of the round-robin database for this application. The default is the global default for Netdata (not the global default for StatsD private charts). This is only relevant if you use `memory mode = save`. Read more on our documentation for the Agent's [Database](/docs/netdata-agent/database).
...
`pattern` is a keyword. When set, `METRIC` is expected to be a Netdata [simple pattern](/docs/developer-and-contributor-corner/libnetdata/simple-patterns) that will be used to match all the StatsD metrics to be added to the chart. So, `pattern` automatically matches any number of StatsD metrics, all of which will be added as separate chart dimensions.
...
- Create a configuration file for your app using [edit-config](/docs/netdata-agent/configuration): `sudo ./edit-config
  statsd.d/myapp.conf`
...
- A node with the [Netdata](/docs/netdata-agent/installation) installed.
...
Use [`edit-config`](/docs/netdata-agent/configuration#edit-a-configuration-file-using-edit-config) 
...
- **pattern**: A keyword that tells the StatsD server the `METRIC` string is actually a 
  [simple pattern](/docs/developer-and-contributor-corner/libnetdata/simple-patterns). 
...
To enable this StatsD configuration, [restart Netdata](/docs/netdata-agent/maintenance/service-control).
