...
Find available collectors in the [Collecting Metrics](/docs/collecting-metrics) guide and on our [Integrations page](https://www.netdata.cloud/integrations).
...
> **Info**
>
> Enable and configure Go collectors directly through the UI using the [Dynamic Configuration Manager](/docs/netdata-agent/configuration/dynamic-configuration-manager).
...
1. Open `netdata.conf` using [`edit-config`](/docs/netdata-agent/configuration#edit-a-configuration-file-using-edit-config).
...
2. Uncomment the collector's line and set it to `no`:
   ...
   modules:
       xyz_collector: no
   ```
3. [Restart](/docs/netdata-agent/maintenance/service-control) the Agent after making changes.
