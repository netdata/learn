...
For centralized retention and high availability, hubs stream to [Parents](/docs/netdata-parents/metrics-centralization-points/clustering-and-high-availability-of-netdata-parents) (a Parent handles on the order of 500 Agents); cluster Parents for HA.
...
If a heavy device pushes either, give it a longer interval or a smaller `max_repetitions` — see [Troubleshooting](/docs/network-performance-monitoring/device-metrics/troubleshooting).
...
- [Configuration](/docs/network-performance-monitoring/device-metrics/configuration) — intervals, `max_repetitions`, SNMPv3, discovery.
- [Validation](/docs/network-performance-monitoring/device-metrics/validation) — confirm clean polling.
- [Troubleshooting](/docs/network-performance-monitoring/device-metrics/troubleshooting) — pacing a device that struggles to answer.
