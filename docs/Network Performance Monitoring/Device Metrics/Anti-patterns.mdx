...
**How to avoid it.** Match the interval to the device. Keep 10 seconds for simple, important interfaces; use 30–60 seconds for heavy core devices. Tune `update_every` (and `max_repetitions`) per job — see [Sizing and Scaling](/docs/network-performance-monitoring/device-metrics/sizing-and-scaling).
...
**How to avoid it.** Read device metrics together with [SNMP Traps](/docs/network-performance-monitoring/snmp-traps) for the transitions a device reports and [Network Flows](/docs/network-performance-monitoring/network-flows) for whether traffic is actually moving. The hub co-locates all three for exactly this reason.
...
**How to avoid it.** Enable [SNMP Traps](/docs/network-performance-monitoring/snmp-traps) on the same hub. Traps catch the transitions between polls; polling confirms the steady state. Neither replaces the other.
...
- [Validation](/docs/network-performance-monitoring/device-metrics/validation) — confirm your data is trustworthy.
- [Sizing and Scaling](/docs/network-performance-monitoring/device-metrics/sizing-and-scaling) — the hub model these pitfalls point back to.
- [SNMP Traps](/docs/network-performance-monitoring/snmp-traps) and [Network Flows](/docs/network-performance-monitoring/network-flows) — the other legs of device monitoring.
