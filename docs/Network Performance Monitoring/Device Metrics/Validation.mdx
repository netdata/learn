...
- **SNMP timeout / retry rate** per device should sit near zero. A device with a persistently high rate is slow to answer (reduce `max_repetitions` or raise its `update_every`); many devices climbing together means the hub is overloaded — see [Sizing and Scaling](/docs/network-performance-monitoring/device-metrics/sizing-and-scaling).
...
- [Troubleshooting](/docs/network-performance-monitoring/device-metrics/troubleshooting) — fixes for each failure above.
- [Anti-patterns](/docs/network-performance-monitoring/device-metrics/anti-patterns) — the habits that cause bad data.
- [Sizing and Scaling](/docs/network-performance-monitoring/device-metrics/sizing-and-scaling) — keeping the hub inside its limits.
