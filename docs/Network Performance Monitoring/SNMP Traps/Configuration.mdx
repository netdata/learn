...
Profile metrics create time-series, so select only reviewed rules with bounded identity and labels. For rule syntax and cardinality rules, see [SNMP Trap Profile Format](/docs/network-performance-monitoring/snmp-traps/snmp-trap-profile-format).
