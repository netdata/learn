...
Use this page to harden a listener after the first trap has been received. If you have not tested receipt yet, start with [Quick Start](/docs/network-performance-monitoring/snmp-traps/quick-start).
...
Community strings are credentials. Use [Secrets Management](/docs/collecting-metrics/secrets-management) so the real value is not stored inline in `snmp_traps.conf`.
...
Use [Secrets Management](/docs/collecting-metrics/secrets-management) for `auth_key` and `priv_key`. The example uses file resolvers, but environment, command, and secretstore resolvers are also supported there.
...
For why the UDP peer is authoritative by default and how a trusted relay overrides source identity, see the [source-identity model in Enrichment](/docs/network-performance-monitoring/snmp-traps/enrichment#direct-senders-vs-trusted-relays).

### Reverse DNS is annotation only

Reverse DNS is disabled by default. When enabled, Netdata emits `TRAP_REVERSE_DNS`, which is never authoritative identity (see [Enrichment](/docs/network-performance-monitoring/snmp-traps/enrichment#reverse-dns)).
...
Trap rows can contain sensitive operational payloads, not only credentials. Review what Netdata emits and redacts before sharing exports, forwarding, or granting broad journal access; see [Field Reference sensitive-data cautions](/docs/network-performance-monitoring/snmp-traps/field-reference#sensitive-data-cautions).
...
Trap entries are written as journal-compatible files under the configured Netdata log directory, with a per-job root of `/var/log/netdata/traps/<job>/` (or `${NETDATA_LOG_DIR}/traps/<job>/` at runtime). They back the embedded `snmp:traps` Function, use `ND_LOG_SOURCE=snmp-trap`, and are not the host systemd-journald journal. For how the `journalctl --directory` path is built, see [Journal and Querying](/docs/network-performance-monitoring/snmp-traps/journal-and-querying); for the field list, see [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference).
...
For what each category and severity value means, see [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics#categories-and-severities); for where they appear as fields, see [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference#trap-meaning-fields).
...
Only committed traps update profile metrics. For the exact update rule and the diagnostics chart, see [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics#profile-defined-metrics).

Do not enable `auto` blindly on unreviewed custom rules. Profile metrics create time-series, so rule identity and labels must be bounded. For rule syntax and cardinality rules, see [SNMP Trap Profile Format](/docs/network-performance-monitoring/snmp-traps/snmp-trap-profile-format).
...
After saving the job and restarting Netdata, validate with [Quick Start](/docs/network-performance-monitoring/snmp-traps/quick-start). For local storage and export workflows, continue to [Journal and Querying](/docs/network-performance-monitoring/snmp-traps/journal-and-querying) or [Forwarding to SIEM](/docs/network-performance-monitoring/snmp-traps/forwarding-to-siem). For profile behavior and rule syntax, see [Trap Profiles](/docs/network-performance-monitoring/snmp-traps/trap-profiles). If the receiver is not installed or cannot bind the port, check [Installation](/docs/network-performance-monitoring/snmp-traps/installation).
