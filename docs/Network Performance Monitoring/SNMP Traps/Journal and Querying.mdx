...
For field meanings and population rules, see [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference).
...
Raw packet bytes are not written to decode-error rows because packets can contain community strings or binary payloads; see [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference#decode-error-fields) for the full decode-error field set. For troubleshooting workflow, see [Troubleshooting](/docs/network-performance-monitoring/snmp-traps/troubleshooting).

## Dedup summaries

When deduplication is enabled, repeated matching traps are suppressed and reported in a periodic summary row with `TRAP_REPORT_TYPE=deduplication_summary`. For what those rows mean, see [Usage and Output](/docs/network-performance-monitoring/snmp-traps/usage-and-output#dedup-summaries).
...
When querying, prefer the indexed `TRAP_VAR_*` fields for rules and use `TRAP_JSON` for audit, export, and residual searches; `TRAP_ENRICHMENT` is for debugging source/enrichment decisions, not as a broad default facet. For the `TRAP_VAR_*` vs `TRAP_JSON` model, skip rules, and enum `_RAW` handling, see [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference#varbind-fields).
...
- Minimize exported fields when integrating with a SIEM. See [Forwarding to SIEM](/docs/network-performance-monitoring/snmp-traps/forwarding-to-siem).

## Related pages

- [Usage and Output](/docs/network-performance-monitoring/snmp-traps/usage-and-output) - Understand report types, normal trap rows, decode errors, and dedup summaries.
...
- [Forwarding to SIEM](/docs/network-performance-monitoring/snmp-traps/forwarding-to-siem) - Forward trap events and map fields in an external log system.
- [Troubleshooting](/docs/network-performance-monitoring/snmp-traps/troubleshooting) - Investigate listener, decode, journal, and source issues.
