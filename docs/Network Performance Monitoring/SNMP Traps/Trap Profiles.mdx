...
Use this page when you need to understand why a trap has a specific name, category, severity, message, tag, or profile-derived chart when profile metrics are enabled. For listener setup and hardening, see [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration).
...
For the profile YAML schema, see [SNMP Trap Profile Format](/docs/network-performance-monitoring/snmp-traps/snmp-trap-profile-format).
...
Profile validation failures are visible as collector errors and profile-load-failure metrics. After editing profiles, check Logs and receiver metrics before assuming a change is active; see [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics) for the receiver diagnostics.
...
For field-level query details, see [Usage and Output](/docs/network-performance-monitoring/snmp-traps/usage-and-output) and [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference).
...
The listener-side `profile_metrics` settings — selection `mode`, `include`, the identity controls, and the cardinality `limits` — are documented in [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration#profile-metrics). Rule and chart syntax live in [SNMP Trap Profile Format](/docs/network-performance-monitoring/snmp-traps/snmp-trap-profile-format).
...
Only committed traps update profile metrics. For the exact update rule (dedup-suppressed and failed-write traps do not update metrics, and an OTLP export failure does not roll back an already-updated metric), see [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics#profile-defined-metrics).
...
For full metric rule syntax, validation rules, and examples, see [SNMP Trap Profile Format](/docs/network-performance-monitoring/snmp-traps/snmp-trap-profile-format).

## Next steps

- To configure listener jobs, overrides, outputs, and `profile_metrics`, see [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration).
- To query received traps and read profile-enriched output, see [Usage and Output](/docs/network-performance-monitoring/snmp-traps/usage-and-output).
- To understand source identity, relay handling, and device context, see [Enrichment](/docs/network-performance-monitoring/snmp-traps/enrichment).
- To understand every emitted trap field, see [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference).
