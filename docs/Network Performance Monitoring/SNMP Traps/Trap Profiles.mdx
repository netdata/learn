...
The listener-side `profile_metrics` settings are documented in [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration#profile-metrics). Rule and chart syntax live in [SNMP Trap Profile Format](/docs/network-performance-monitoring/snmp-traps/snmp-trap-profile-format).
