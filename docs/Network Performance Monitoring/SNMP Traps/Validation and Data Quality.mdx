...
Work through these checks in order during first deployment. Use [Quick Start](/docs/network-performance-monitoring/snmp-traps/quick-start) to send a known test trap, and use [Journal and Querying](/docs/network-performance-monitoring/snmp-traps/journal-and-querying) when you need local `journalctl` or Cloud Logs verification commands.
...
Compare `TRAP_SOURCE_IP` with `TRAP_SOURCE_UDP_PEER` (equal for direct senders, different for a trusted-relay override), then inspect the `source` object inside `TRAP_ENRICHMENT` to confirm `selected`, `method`, `trusted_relay`, and any `rejected_candidates`. For the full source-identity model and the step-by-step trust checklist, see [Enrichment](/docs/network-performance-monitoring/snmp-traps/enrichment#how-to-validate-source-identity). Keep `source.trusted_relays` narrow: a broad range lets senders on that path influence attribution through `snmpTrapAddress.0`.
...
For OTLP-only jobs, validate the same source relationship with OTLP attributes: `snmp.source.ip` is the selected source and `network.peer.address` is the UDP peer. Use the [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference#otlp-mapping-notes) for the journal-to-OTLP mapping.
...
- If deduplication is enabled, expect the first matching trap row plus later `TRAP_REPORT_TYPE=deduplication_summary` rows. Check `dedup.window_sec`, `dedup.cache_max_entries`, and `dedup.key_varbinds` in [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration) when summaries do not match expectations.
...
- Use [Journal and Querying](/docs/network-performance-monitoring/snmp-traps/journal-and-querying) for `snmp:traps` Function behavior, Cloud requirements, and local `journalctl --directory` commands.
...
- Check that retention settings keep enough local history for incident investigation. See [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration#direct-journal-retention) for retention defaults and settings.

For OTLP export:

- Confirm `snmp.trap.errors` `otlp_export_failed` stays clear.
- Confirm the downstream OTLP receiver stores the expected records.
- Confirm records carry `service.name=netdata-snmptrap` and `service.instance.id=<job>`. See [Forwarding to SIEM](/docs/network-performance-monitoring/snmp-traps/forwarding-to-siem) for the OTLP export checks.
...
For OTLP ingestion, use the OTLP mapping in [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference). Build rules against report type, selected source, UDP peer, trap OID, trap name, category, severity, version, varbind payload, decode-error details, and dedup summary attributes.
...
- [Enrichment](/docs/network-performance-monitoring/snmp-traps/enrichment) - Source identity, trusted relays, reverse DNS, and enrichment audit details.
- [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration) - Source controls, credentials, deduplication, rate limiting, retention, and output settings.
- [Trap Profiles](/docs/network-performance-monitoring/snmp-traps/trap-profiles) - How profiles turn OIDs and varbinds into names, categories, severities, labels, and profile-defined metrics.
- [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference) - Complete field meanings, population rules, sensitive-data cautions, and OTLP mapping.
- [Journal and Querying](/docs/network-performance-monitoring/snmp-traps/journal-and-querying) - Local `journalctl` and Cloud Logs query workflows.
- [Forwarding to SIEM](/docs/network-performance-monitoring/snmp-traps/forwarding-to-siem) - OTLP export, resource attributes, and SIEM field validation.
- [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics) - Receiver metrics, pipeline counters, and errors.
- [Alerts](/docs/network-performance-monitoring/snmp-traps/alerts) - Default health alert behavior on these receiver metrics.
- [Troubleshooting](/docs/network-performance-monitoring/snmp-traps/troubleshooting) - Failure investigation paths when validation does not match expectations.
- [Anti-Patterns](/docs/network-performance-monitoring/snmp-traps/anti-patterns) - Configurations and operational habits to avoid.
