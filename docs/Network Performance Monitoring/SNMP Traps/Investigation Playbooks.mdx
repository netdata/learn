...
- For OTLP-only jobs, inspect the equivalent downstream log attributes instead of local journal field names. Use the OTLP mapping in [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference#otlp-mapping-notes).
...
- [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference)
- [Journal and Querying](/docs/network-performance-monitoring/snmp-traps/journal-and-querying)
- [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics)
...
- **Inspect:** `TRAP_OID`, `TRAP_NAME`, `TRAP_CATEGORY`, and `snmp.trap.errors` `unknown_oid` first; for the full field and evidence list, see [Troubleshooting](/docs/network-performance-monitoring/snmp-traps/troubleshooting#unknown-oids-profile-load-failures-or-template-issues).
...
- **Inspect:** `TRAP_SUPPRESSED_COUNT`, `TRAP_SUPPRESSED_FINGERPRINTS`, and `TRAP_REPORT_PERIOD_SEC` first; for the full evidence list and config checks, see [Troubleshooting](/docs/network-performance-monitoring/snmp-traps/troubleshooting#rate-limiting-or-deduplication-surprise).
