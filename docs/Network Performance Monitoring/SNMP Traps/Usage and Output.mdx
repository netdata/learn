...
You can also read direct-journal jobs with normal journal tooling. The per-job root is normally below `/var/log/netdata/traps/<job>/`; the effective `journalctl --directory` path is its machine-id child, such as `/var/log/netdata/traps/<job>/$(tr -d '-' < /etc/machine-id)`. Use [Journal and Querying](/docs/network-performance-monitoring/snmp-traps/journal-and-querying) for query examples.
...
Drops that happen before the collector writes a row are visible in receiver metrics, not as trap rows. For drop and error counters, see [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics).
...
The remaining decode-error and packet-audit fields (`TRAP_CATEGORY`, `TRAP_SEVERITY`, `TRAP_SOURCE_UDP_PEER`, `TRAP_SOURCE_UDP_PORT`, `TRAP_VERSION`, `TRAP_PACKET_SIZE`, `TRAP_LISTENER`, `TRAP_ENGINE_ID`, `TRAP_JSON`) and why raw packet bytes are never stored are in [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference#decode-error-fields).
...
To track how often it happens, watch the `binary_encoded` counter; for its meaning and rate, see [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics#processing-errors), and for the default alert, see [Alerts](/docs/network-performance-monitoring/snmp-traps/alerts).
...
- [Field Reference](/docs/network-performance-monitoring/snmp-traps/field-reference) - Check every field, when it appears, and how to query it.
- [Journal and Querying](/docs/network-performance-monitoring/snmp-traps/journal-and-querying) - Query trap rows with Netdata Cloud Logs and journal tooling.
- [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics) - Use receiver metrics, error counters, and dedup counters to explain what rows do not show by themselves.
- [Alerts](/docs/network-performance-monitoring/snmp-traps/alerts) - See the default health alerts that fire on these receiver metrics.
