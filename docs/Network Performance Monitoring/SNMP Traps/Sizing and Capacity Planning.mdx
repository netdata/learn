...
For the byte-unit details of `max_size` and rotation, see [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration#direct-journal-retention).

## Shipped limits and defaults

Useful starting points and the fixed caps that bound capacity — not capacity promises. For the full configurable option list, see [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration#option-map).
...
Profile-defined metrics stay **disabled by default** and are the main cardinality risk — a rule with an unbounded label can create uncontrolled series. Enable selected rules only; fixed runtime caps provide final guardrails. For the cardinality model see [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration#profile-metrics).
...
For how each control reads out in the receiver counters, see [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics#receiver-pipeline).
...
For the full quality checklist, see [Validation and Data Quality](/docs/network-performance-monitoring/snmp-traps/validation-and-data-quality).

## What's next

- [Configuration](/docs/network-performance-monitoring/snmp-traps/configuration) — buffer, retention, storm-control, and OTLP options.
- [Metrics](/docs/network-performance-monitoring/snmp-traps/metrics) — the receiver counters to watch during a storm.
- [Alerts](/docs/network-performance-monitoring/snmp-traps/alerts) — the default storm and dedup alerts, plus the kernel UDP buffer-drop alert.
- [Troubleshooting](/docs/network-performance-monitoring/snmp-traps/troubleshooting) — missing traps, silent loss, and backend failures.
