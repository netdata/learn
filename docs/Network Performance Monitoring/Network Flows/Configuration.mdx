...
For the cross-cutting picture — order of evaluation, the `asn_providers` and `net_providers` chains, the MMDB shared mechanism, the static-vs-dynamic composition rules — see the [Enrichment](/docs/network-performance-monitoring/network-flows/enrichment) page. Per-method configuration details (URLs, refresh cadence, license, vendor commands) live on the integration cards under flows.enrichment-methods:

- IP intelligence (MMDB): [DB-IP](/docs/network-performance-monitoring/network-flows/enrichment-methods/db-ip-ip-intelligence), [MaxMind GeoIP / GeoLite2](/docs/network-performance-monitoring/network-flows/enrichment-methods/maxmind-geoip-geolite2), [IPtoASN](/docs/network-performance-monitoring/network-flows/enrichment-methods/iptoasn), [Custom MMDB](/docs/network-performance-monitoring/network-flows/enrichment-methods/custom-mmdb-database).
- BGP routing: [BMP](/docs/network-performance-monitoring/network-flows/enrichment-methods/bmp-bgp-monitoring-protocol), [bio-rd RIS](/docs/network-performance-monitoring/network-flows/enrichment-methods/bio-rd-ripe-ris).
- Network sources: [AWS IP Ranges](/docs/network-performance-monitoring/network-flows/enrichment-methods/aws-ip-ranges), [Azure IP Ranges](/docs/network-performance-monitoring/network-flows/enrichment-methods/azure-ip-ranges), [GCP IP Ranges](/docs/network-performance-monitoring/network-flows/enrichment-methods/gcp-ip-ranges), [NetBox](/docs/network-performance-monitoring/network-flows/enrichment-methods/netbox), [Generic JSON-over-HTTP IPAM](/docs/network-performance-monitoring/network-flows/enrichment-methods/generic-json-over-http-ipam).
- YAML-defined: [Static Metadata](/docs/network-performance-monitoring/network-flows/enrichment-methods/static-metadata), [Classifiers](/docs/network-performance-monitoring/network-flows/enrichment-methods/classifiers), [Decapsulation](/docs/network-performance-monitoring/network-flows/enrichment-methods/decapsulation).
- Operational: [Enrichment Intel Downloader](/docs/network-performance-monitoring/network-flows/enrichment-intel-downloader) — the bundled refresh tool for MMDB providers.
...
- [Retention and Querying](/docs/network-performance-monitoring/network-flows/retention-and-tiers) — How the four tiers work and how the dashboard picks one.
- [Sizing and Capacity Planning](/docs/network-performance-monitoring/network-flows/sizing-and-capacity-planning) — How much disk and CPU you need.
- [Validation and Data Quality](/docs/network-performance-monitoring/network-flows/validation-and-data-quality) — How to confirm the data is right.
- [Troubleshooting](/docs/network-performance-monitoring/network-flows/troubleshooting) — When things break.
