...
- **Host networking (`--network=host`)** — the recommended run mode for Netdata containers. The container shares the host's network, so the UDP listeners are reachable by your routers and switches with **no extra port flags**. Use the command from the [Docker installation guide](/docs/netdata-agent/installation/docker#create-a-new-netdata-agent-container) unchanged.
- **Bridge networking** — if you run without `--network=host`, the container's network is isolated. The image only declares the dashboard port (`19999`) in its `EXPOSE`, so you must publish it and the flow UDP ports yourself, otherwise no flow data arrives. Add these to the [recommended `docker run` command](/docs/netdata-agent/installation/docker#create-a-new-netdata-agent-container) in place of `--network=host`, keeping its other mounts and privileges:
...
Dropping `--network=host` isn't free even beyond netflow — `proc.plugin`, `go.d.plugin`, `local-listeners`, and `network-viewer.plugin` all require host network mode for full functionality (see the [privileges table](/docs/netdata-agent/installation/docker#create-a-new-netdata-agent-container)). Only switch to bridge networking if you have another reason to avoid host networking.

:::

To listen on different ports (for example, if `2055` or `6343` is in use on the host), edit `netflow.yaml` inside the container — see [Configure Agent Containers](/docs/netdata-agent/installation/docker#configure-agent-containers) for how to edit a config file in a running container — then publish the matching ports. See [Configuration](/docs/network-performance-monitoring/network-flows/configuration) for the netflow-specific options.
...
This populates `/var/cache/netdata/topology-ip-intel/` with the DB-IP-based MMDB files. The plugin auto-detects the cache copy on its next 30-second poll. See the [Enrichment Intel Downloader page](/docs/network-performance-monitoring/network-flows/enrichment-intel-downloader) for the refresh tool and the [DB-IP integration card](/docs/network-performance-monitoring/network-flows/enrichment-methods/db-ip-ip-intelligence) for cadence and license details.
...
Open the Netdata UI in your browser. Click the **Live** tab in the top navigation; **Network Flows** appears in the Functions list on the right (see [Live tab](/docs/dashboards-and-charts/tabs/live)). Selecting it opens the Sankey + Table view. The plugin's operational charts also appear under the standard charts page in the `netflow` family.

### "Connect this agent to Netdata to use this function"

If you select **Network Flows** on the Live tab and see the message **Connect this agent to Netdata to use this function**, the plugin is working — the block is access control, not a plugin failure. Network Flows (`flows:netflow`) is a [sensitive function](/docs/security-and-privacy-design/access-control-and-feature-availability): the local dashboard you reach anonymously at `http://<agent-ip>:19999` does not serve sensitive functions to anonymous (signed-out) users.

To use Network Flows from your standalone host:

1. **Sign in to Netdata Cloud** at [app.netdata.cloud](https://app.netdata.cloud). If the Agent is not connected yet, a Space Admin must claim/connect it to the Space. After it is connected, your own Space role must allow you to run sensitive functions; see the **Node Management** and **Functions** tables under [Detailed Permissions](/docs/netdata-cloud/authentication-&-authorization/role-based-access-model#detailed-permissions). The free Community tier is sufficient — no paid plan is required.
...
Connecting the agent does **not** move or offload its persistent flow storage. Collection and the four-tier journal (raw + 1-minute + 5-minute + 1-hour rollups) remain on the Agent under the configured `journal_dir`. The default relative directory is `${NETDATA_CACHE_DIR}/flows`, typically `/var/cache/netdata/flows/` for native packages; see [Configuration](/docs/network-performance-monitoring/network-flows/configuration#move-the-journal-directory). When you use the Cloud dashboard, the Function query results transit Cloud on their way to your browser; when you use the authenticated local dashboard, they travel directly from the Agent to your browser.
...
See [Netdata Access Control and Feature Availability](/docs/security-and-privacy-design/access-control-and-feature-availability) for the full Anonymous / Community / Paid access model.
...
- See [Troubleshooting](/docs/network-performance-monitoring/network-flows/troubleshooting).
...
- [Quick Start](/docs/network-performance-monitoring/network-flows/quick-start) — A 15-minute path to your first flow data.
- [Flow Protocols / NetFlow](/docs/network-performance-monitoring/network-flows/flow-protocols/netflow) — Vendor configurations for NetFlow.
- [Flow Protocols / IPFIX](/docs/network-performance-monitoring/network-flows/flow-protocols/ipfix) — Vendor configurations for IPFIX.
- [Flow Protocols / sFlow](/docs/network-performance-monitoring/network-flows/flow-protocols/sflow) — Vendor configurations for sFlow.
...
- [Quick Start](/docs/network-performance-monitoring/network-flows/quick-start) — Configure your first source and see traffic in the dashboard.
- [Configuration](/docs/network-performance-monitoring/network-flows/configuration) — Tune the listener, retention, and enrichment.
- [Troubleshooting](/docs/network-performance-monitoring/network-flows/troubleshooting) — When something doesn't work.
