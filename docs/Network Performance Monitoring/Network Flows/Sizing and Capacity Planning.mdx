...
The raw tier is queried directly for any IP-level investigation, full-text search, city / latitude / longitude maps, and anything that filters on a raw-only field (see [Field Reference](/docs/network-performance-monitoring/network-flows/field-reference) for which fields survive into rollups). At 50 000 flow records/s sustained, the raw tier produces ~1.7 TB / day of indexed writes that you may also be reading back in real time.
...
6. **Tune kernel UDP buffers** for burst headroom (see [Troubleshooting](/docs/network-performance-monitoring/network-flows/troubleshooting)).
7. **For multi-site deployments**, run one Netdata Agent per router or per site rather than central aggregation.

## What's next

- [Configuration](/docs/network-performance-monitoring/network-flows/configuration#per-tier-retention) — Per-tier retention schema.
- [Retention and Querying](/docs/network-performance-monitoring/network-flows/retention-and-tiers) — How tiers map to queries and the auto-tier-pick rules.
- [Field Reference](/docs/network-performance-monitoring/network-flows/field-reference) — Which fields survive into rollups and which are raw-only.
- [Troubleshooting](/docs/network-performance-monitoring/network-flows/troubleshooting) — UDP buffer tuning, query timeout, and disk write pressure.
