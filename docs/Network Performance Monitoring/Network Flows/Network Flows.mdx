...
Opening Network Flows requires an Agent connected to Netdata Cloud and a signed-in user whose Space role permits sensitive functions. See the [Network Flows access requirement](/docs/network-performance-monitoring/network-flows/installation#connect-this-agent-to-netdata-to-use-this-function).
...
To see real numbers, filter by one exporter and one interface. Each packet then appears in exactly one record on that interface. See the [Anti-patterns page](/docs/network-performance-monitoring/network-flows/anti-patterns) for the full framing.
...
For the selected canonical counters before sampling multiplication, see `RAW_BYTES` and `RAW_PACKETS` in the [field reference](/docs/network-performance-monitoring/network-flows/field-reference). The field reference also explains how Netdata selects between incoming and post-observation NetFlow/IPFIX counter families.
...
- **You're setting up the plugin for the first time** — [Installation](/docs/network-performance-monitoring/network-flows/installation), then [Quick Start](/docs/network-performance-monitoring/network-flows/quick-start).
- **You have data, you want to find a bandwidth hog or trace an IP** — [Investigation Playbooks](/docs/network-performance-monitoring/network-flows/investigation-playbooks).
- **You want to make sure your data is trustworthy** — [Validation and Data Quality](/docs/network-performance-monitoring/network-flows/validation-and-data-quality).
- **You want to avoid the most common mistakes** — [Anti-patterns](/docs/network-performance-monitoring/network-flows/anti-patterns).
