...
- The [netflow plugin is installed](/docs/network-performance-monitoring/network-flows/installation) on that host.
- A Space Admin has connected the Agent to your Netdata Cloud Space, you are signed in, and your Space role allows sensitive functions. The free Community tier is sufficient; see the [Network Flows access requirement](/docs/network-performance-monitoring/network-flows/installation#connect-this-agent-to-netdata-to-use-this-function).
...
If the plugin isn't installed yet, follow the [Installation page](/docs/network-performance-monitoring/network-flows/installation) first.
...
For more vendors and details, see [Flow Protocols / NetFlow](/docs/network-performance-monitoring/network-flows/flow-protocols/netflow), [IPFIX](/docs/network-performance-monitoring/network-flows/flow-protocols/ipfix), and [sFlow](/docs/network-performance-monitoring/network-flows/flow-protocols/sflow).

## Step 2 — Open the dashboard

In your browser, open either the authenticated local Agent dashboard or the connected node in Netdata Cloud, click the **Live** tab in the top navigation, and select **Network Flows** from the Functions list. If the Agent is not connected or you are signed out, follow the [access steps](/docs/network-performance-monitoring/network-flows/installation#connect-this-agent-to-netdata-to-use-this-function).
...
2. **Listener bound on the host.**
   ...
   Should show `netflow-plugin` listening. If not, see [Troubleshooting](/docs/network-performance-monitoring/network-flows/troubleshooting).

3. **Plugin actually decoding.**

   Open the standard Netdata charts page. `netflow.input_packets` confirms UDP arrival; `netflow.protocol_packets` shows whether the packets are v5, v7, v9, IPFIX, or sFlow. If UDP rises but no protocol does, check `parse_errors` in `netflow.decoder_exceptions`. For v9/IPFIX, check missing-template dimensions in `netflow.flow_sets`. See [Plugin Health Charts](/docs/network-performance-monitoring/network-flows/visualization/plugin-health-charts).
...
- [Configuration](/docs/network-performance-monitoring/network-flows/configuration) — Tune retention after first validation; production retention should be sized from observed flow rate.
- [Static Metadata integration card](/docs/network-performance-monitoring/network-flows/enrichment-methods/static-metadata) — Give your routers and your internal networks friendly names and labels. Without this, dashboards show raw IPs.
- [Investigation Playbooks](/docs/network-performance-monitoring/network-flows/investigation-playbooks) — Concrete recipes for the questions flow data is good at answering.
- [Anti-patterns](/docs/network-performance-monitoring/network-flows/anti-patterns) — Mistakes to avoid as you develop confidence with the data.
- [Validation and Data Quality](/docs/network-performance-monitoring/network-flows/validation-and-data-quality) — How to confirm your numbers are correct.

For more sources or vendors:

- [NetFlow](/docs/network-performance-monitoring/network-flows/flow-protocols/netflow) — More vendor configurations, sampling caveats.
- [IPFIX](/docs/network-performance-monitoring/network-flows/flow-protocols/ipfix) — When and why to prefer IPFIX over NetFlow v9.
- [sFlow](/docs/network-performance-monitoring/network-flows/flow-protocols/sflow) — Different protocol, different semantics.
