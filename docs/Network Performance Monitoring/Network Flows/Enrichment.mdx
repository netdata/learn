...
See the [Decapsulation integration card](/docs/network-performance-monitoring/network-flows/enrichment-methods/decapsulation) for exporter configuration recipes.
...
See [Retention and querying](/docs/network-performance-monitoring/network-flows/retention-and-tiers) and [Field reference](/docs/network-performance-monitoring/network-flows/field-reference) for the full picture.
...
- [DB-IP IP Intelligence](/docs/network-performance-monitoring/network-flows/enrichment-methods/db-ip-ip-intelligence) — the default that ships with Netdata
- [MaxMind GeoIP / GeoLite2](/docs/network-performance-monitoring/network-flows/enrichment-methods/maxmind-geoip-geolite2) — commercial GeoIP2 or free GeoLite2 with attribution
- [IPtoASN](/docs/network-performance-monitoring/network-flows/enrichment-methods/iptoasn) — public-domain ASN + country, hourly cadence
- [Custom MMDB Database](/docs/network-performance-monitoring/network-flows/enrichment-methods/custom-mmdb-database) — your own MMDB build

**BGP routing**
- [BMP (BGP Monitoring Protocol)](/docs/network-performance-monitoring/network-flows/enrichment-methods/bmp-bgp-monitoring-protocol) — routers push BGP updates over TCP
- [bio-rd RIS](/docs/network-performance-monitoring/network-flows/enrichment-methods/bio-rd-ripe-ris) — pull BGP data from a bio-rd-compatible RIS gRPC daemon

**Network identity (cloud IP ranges, IPAM)**
- [AWS IP Ranges](/docs/network-performance-monitoring/network-flows/enrichment-methods/aws-ip-ranges) — public AWS prefix list
- [GCP IP Ranges](/docs/network-performance-monitoring/network-flows/enrichment-methods/gcp-ip-ranges) — public GCP prefix list
- [Azure IP Ranges](/docs/network-performance-monitoring/network-flows/enrichment-methods/azure-ip-ranges) — Azure Service Tags (requires an internal mirror)
- [NetBox](/docs/network-performance-monitoring/network-flows/enrichment-methods/netbox) — open-source IPAM / DCIM
- [Generic JSON-over-HTTP IPAM](/docs/network-performance-monitoring/network-flows/enrichment-methods/generic-json-over-http-ipam) — Infoblox, BlueCat, phpIPAM, custom CMDBs

**Static and rule-based**
- [Static Metadata](/docs/network-performance-monitoring/network-flows/enrichment-methods/static-metadata) — per-exporter, per-interface, per-CIDR labels
- [Classifiers](/docs/network-performance-monitoring/network-flows/enrichment-methods/classifiers) — Akvorado-compatible expression rules

**Decapsulation**
- [Decapsulation](/docs/network-performance-monitoring/network-flows/enrichment-methods/decapsulation) — SRv6 and VXLAN inner-packet extraction

### Related concepts

- [Intel Downloader](/docs/network-performance-monitoring/network-flows/enrichment-intel-downloader) — the bundled tool that fetches and converts MMDB files for the auto-detect path.
- [Configuration](/docs/network-performance-monitoring/network-flows/configuration) — the full `enrichment` section reference.
- [Field reference](/docs/network-performance-monitoring/network-flows/field-reference) — every flow record field, what writes it, what reads it.
- [Retention and querying](/docs/network-performance-monitoring/network-flows/retention-and-tiers) — which fields survive into rollups, how queries pick a tier.
