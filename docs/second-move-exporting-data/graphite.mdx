...
the [exporting engine](/docs/second-move-exporting-data/exporting-reference) to archive your agent's
metrics to Graphite providers for long-term storage, further analysis, or correlation with data from other sources.

## Prerequisites

You have already [installed Netdata](/docs/getting-started/install-netdata) and 
Graphite.
...
We'll not worry about the [rest of the settings](/docs/second-move-exporting-data/exporting-reference#options)
 for now. Restart the Agent using `sudo systemctl restart netdata`, or the
[appropriate method](/docs/getting-started/manage-and-configure/start-stop-or-restart-the-netdata-agent) for your
...
the [exporting reference guide](/docs/second-move-exporting-data/exporting-reference).
