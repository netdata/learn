...
For a complete explanation of node states, state transitions, and when nodes move between states, see [Node States and Transitions](/docs/netdata-cloud/node-states-and-transitions).
...
For nodes that are part of streaming configurations, see [Nodes Ephemerality](/docs/netdata-parents/node-types-and-lifecycle-reference) for more advanced configuration options.
...
* [Stop the Netdata Agent](/docs/netdata-agent/maintenance/service-control) on the node
* Remove or clear any existing `claim.conf` file
* Clear related environment variables on the node

To remove Netdata from the node entirely instead, see [Uninstall Netdata](/docs/netdata-agent/maintenance/uninstall).
...
You can mark nodes as pending for decommissioning to avoid getting notifications about them. This action can also be performed in bulk, and it can be undone if needed.
...
For a complete reference on node states and transitions, see [Node States and Transitions](/docs/netdata-cloud/node-states-and-transitions).

For more advanced configuration options with streaming setups, see [Nodes Ephemerality](/docs/netdata-parents/node-types-and-lifecycle-reference).

To avoid removal issues when cloning VMs, see [VM Templates](/docs/netdata-agent/vm-templates) for proper identity cleanup.

To understand how node identity affects removal and cleanup, see [Node Identities](/docs/netdata-agent/node-identities).
