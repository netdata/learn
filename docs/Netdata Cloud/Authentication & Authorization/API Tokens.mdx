...
| `scope:grafana-plugin` | Used for the [Netdata Grafana plugin](/docs/dashboards-and-charts/grafana-plugin) to access Netdata charts | Access to chart and data endpoints |
| `scope:mcp`            | Used to connect MCP clients (Claude Desktop, Cursor, etc.) to [Netdata Cloud MCP](/docs/netdata-ai/mcp#netdata-cloud-mcp) for AI-assisted monitoring | Access to MCP server endpoints     |
