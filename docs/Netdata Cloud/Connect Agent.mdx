...
This section guides you through installing and securely connecting a new Agent to Netdata Cloud via the encrypted Agent-Cloud Link  ([ACLK](/docs/netdata-cloud/agent-cloud-link-aclk)). Connecting your Agent to your Space unlocks centralized monitoring, easier collaboration, and more.
...
- [**Nodes tab**](/docs/dashboards-and-charts/tabs/nodes): Click on the "Add nodes" button.
...
If the Agent is already running, you can either run `netdatacli reload-claiming-state` or [restart the Agent](/docs/netdata-agent/maintenance/service-control). Otherwise, the Agent connects when it starts.
...
For detailed explanations of all identity types and their relationships, see [Node Identities](/docs/netdata-agent/node-identities).
...
If you're having trouble connecting a node, this may be because the [ACLK](/docs/netdata-cloud/agent-cloud-link-aclk) cannot connect to Cloud.
...
**Solution:** Check our [installation section](/docs/netdata-agent/installation) to find the proper way of installing Netdata on your system.
...
**Solution:** We recommend you reinstall Netdata with a [static build](/docs/netdata-agent/installation/linux/static-binary-linux-packages), which uses an up-to-date version of OpenSSL with hostname validation enabled.
