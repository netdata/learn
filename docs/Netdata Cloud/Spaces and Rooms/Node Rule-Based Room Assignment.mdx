...
Set `CLUSTER_ENV` to `dev`, `acc`, or `prod` per cluster (or node pool) via your deployment tooling. For more Helm chart options, see the [Kubernetes installation guide](/docs/netdata-agent/installation/kubernetes) and the [Helm chart reference](https://github.com/netdata/helmchart#configuration).
