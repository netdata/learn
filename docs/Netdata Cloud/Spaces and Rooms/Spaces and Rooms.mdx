...
2. Assign a role to each invited user. Space-level roles determine what someone can do across the Space:
   ...
   For the full permission matrix and role availability by plan, see the [role-based access model](/docs/netdata-cloud/authentication-&-authorization/role-based-access-model).
