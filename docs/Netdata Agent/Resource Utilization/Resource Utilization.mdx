...
|:---------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **CPU**        | [1%-5% of a single core](/docs/netdata-agent/resource-utilization/cpu) with default settings; up to [5%-20% in production](/docs/netdata-agent/resource-utilization/impact-on-cloud-vms#typical-netdata-resources-usage-on-production-systems) |
| **RAM**        | [100-200 MB](/docs/netdata-agent/resource-utilization/ram) on an empty system; [250-350 MB in typical production](/docs/netdata-agent/resource-utilization/impact-on-cloud-vms#typical-netdata-resources-usage-on-production-systems)          |
| **Disk**       | [~4 GiB by default](/docs/netdata-agent/resource-utilization/disk-&-retention#default-disk-footprint) (3 GiB metrics plus metadata), configurable per tier                                                   |
| **Privileges** | Root on Linux, or Administrator on Windows, required for installation                                                                                                                                                           |

For multi-node setups that centralize metrics on a Netdata Parent, resource needs scale with the number of Children and retention — see [Parent Configuration Best Practices](/docs/netdata-parents/parent-configuration-best-practices).
...
Check [RAM Requirements](/docs/netdata-agent/resource-utilization/ram) for more information.
...
2. **Compact Storage Engine**
   ...
   For per-tier on-disk sample sizes, see [Disk Requirements & Retention](/docs/netdata-agent/resource-utilization/disk-&-retention).
