...
If you need to align Netdata with enterprise cybersecurity platforms such as Sophos, secure-access gateways, or corporate reverse proxies, see [Configure Netdata for cybersecurity platforms](/docs/netdata-agent/configuration/securing-agents/configure-netdata-for-cybersecurity-platforms).
...
Edit the `[web]` section in `netdata.conf` using the [`edit-config`](/docs/netdata-agent/configuration#edit-configuration-files) script:
...
- Agent must be [claimed to Netdata Cloud](/docs/netdata-cloud/connect-agent)
- Works with both Community (free) and Business plans

For detailed configuration options, see [Secure Your Netdata Agent with Bearer Token Protection](/docs/netdata-agent/configuration/securing-agents/bearer-token-protection).
...
You can secure your nodes by disabling local dashboard access while maintaining Cloud monitoring capabilities. This eliminates public exposure of metrics and system information while maintaining secure metrics viewing through Netdata Cloud via [ACLK](/docs/netdata-cloud/agent-cloud-link-aclk).
...
For more information, see [Observability Centralization Points](/docs/netdata-parents).
...
You can restrict access to your local dashboard while maintaining Netdata Cloud connectivity by using [access lists](/docs/netdata-agent/configuration/securing-agents/web-server-reference#access-lists).
...
- Consider [enabling SSL](/docs/netdata-agent/configuration/securing-agents/web-server-reference#examples) to encrypt local dashboard traffic (Netdata Cloud connections are always TLS-encrypted)
...
The security measures above concern **inbound** access to your Agent's local dashboard (port `19999`). Claimed nodes — including Parent Agents that connect to Netdata Cloud — also require **outbound** access to Netdata Cloud over the encrypted Agent-Cloud Link ([ACLK](/docs/netdata-cloud/agent-cloud-link-aclk)) — an outbound WebSocket Secure (WSS) connection on TCP port `443`.

IP addresses can change without notice and vary based on your geographic location due to CDN-edge servers, so **always prefer domain-based allowlisting**. For the complete list of required domains and ports (inbound and outbound), see [Required endpoints and ports](/docs/netdata-agent/configuration/securing-agents/configure-netdata-for-cybersecurity-platforms#required-endpoints-and-ports). If your Agent requires a proxy to reach the internet, see [proxy configuration](/docs/netdata-cloud/connect-agent#proxy-configuration).
