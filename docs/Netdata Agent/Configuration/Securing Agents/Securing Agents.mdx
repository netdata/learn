...
For the complete reference of all default outbound connections — including which are active by default, what each connects to, and how to disable each one — see [Outbound Network Communication](/docs/security-and-privacy-design/netdata-agent#outbound-network-communication).

For the domain allowlist and port table required for firewall configuration, see [Configure Netdata for cybersecurity platforms](/docs/netdata-agent/configuration/securing-agents/configure-netdata-for-cybersecurity-platforms#required-endpoints-and-ports).
