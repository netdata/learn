...
To understand what actions you can perform based on your role, refer to the [Role-Based Access documentation](/docs/netdata-cloud/authentication-&-authorization/role-based-access-model).
...
- In `netdata.conf`, under the `[health]` section, exclude the alert name and restart the Agent:
  ...
  ```
  [Restarting the Agent](/docs/netdata-agent/maintenance/service-control) is required because `netdatacli reload-health` reloads health configuration files but does not reload `netdata.conf`.
...
For the full manual configuration syntax, see [How to Disable or Silence Alerts](/docs/alerts-&-notifications/alert-configuration-reference#how-to-disable-or-silence-alerts).
