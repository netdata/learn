...
You can configure Netdata's **central Registry** to provide unified cross-server dashboards. Together with certain browser features, it allows Netdata to provide these dashboards. The Registry operates with [minimal data transfer](/docs/netdata-agent/registry#communication-with-the-registry), with all communication occurring directly between your web browser and the Registry.

:::info

Read more about it in the [Registry overview](/docs/netdata-agent/registry).
...
1. Modify `netdata.conf` using [`edit-config`](/docs/netdata-agent/configuration#edit-configuration-files):
...
To modify cookie settings, edit `netdata.conf` using [`edit-config`](/docs/netdata-agent/configuration#edit-configuration-files):
