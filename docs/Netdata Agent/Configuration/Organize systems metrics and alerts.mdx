...
Learn more in our [Spaces and Rooms documentation](/docs/netdata-cloud/spaces-and-rooms).
...
See [Virtual Nodes (vnodes)](/docs/netdata-agent/node-identities#virtual-nodes-vnodes) for the full reference: how to define a vnode via YAML or the dynamic configuration GUI, and how to attach it to a collector job.
...
Child node labels contain sensitive system information. Secure your streaming connections with SSL and consider using [access lists](/docs/netdata-agent/configuration/securing-agents/web-server-reference#access-lists) or [restricting API access](/docs/netdata-agent/configuration/securing-agents#alternative-methods).
...
See the [health documentation](/docs/alerts-&-notifications/alert-configuration-reference#alert-line-host-labels) for more possibilities.

### Export labels with metrics

When using [metrics exporters](/docs/exporting-metrics/exporting-reference), include host labels with your exported data:
