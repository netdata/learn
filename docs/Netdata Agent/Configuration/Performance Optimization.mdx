...
- **Machine Learning**: Trains anomaly detection models for each metric ([Anomaly Detection](https://github.com/netdata/netdata/blob/master/src/ml/README.md))
...
See [Resource Utilization](/docs/netdata agent/resource utilization/resource-utilization) for detailed Agent resource requirements.
...
:::tip
This architecture works especially well in production environments where you monitor many systems. Learn more in our [Centralization Points documentation](/docs/netdata parents/netdata-parents).
:::

## Disable unneeded collectors

Reduce resource usage by turning off [Plugins or Collectors](/docs/collecting metrics/collecting-metrics) you don't need.
...
Follow our [configuration guide](/docs/collecting metrics/collectors-configuration) to identify and disable specific collectors.
...
Learn how to adjust collection frequency in our [configuration guide](/docs/collecting metrics/collectors-configuration).
...
Configure retention settings using our [database configuration guide](/docs/netdata agent/configuration/database).

## Switch to RAM mode

For IoT devices and Child nodes in [Parent-Child setups](/docs/netdata parents/netdata-parents), switch to RAM mode to eliminate disk operations entirely. This mode:
...
Set up RAM mode following our [database configuration guide](/docs/netdata agent/configuration/database).
...
To disable ML, edit your configuration using [`edit-config`](/docs/netdata agent/configuration#edit-configuration-files):
