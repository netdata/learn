...
See [Node Ephemerality](/docs/netdata-parents/node-types-and-lifecycle-reference) for full documentation, cleanup rules, and alerting details.
...
Verify the service stopped with `Get-Service Netdata`. See [Service Control](/docs/netdata-agent/maintenance/service-control) for details.
