...
2. **Stop the Netdata Agent**
   ...
   It is recommended that you [stop the Netdata Agent](/docs/netdata-agent/maintenance/service-control) when backing up the Metrics/database files. Backing up the Agent configuration and Identity folders is straightforward as they shouldn't be changing very frequently.
...
1. **Prepare for restoration**
   ...
   Ensure that the Netdata Agent is installed and is [stopped](/docs/netdata-agent/maintenance/service-control)

   If you plan to deploy the Agent and restore a backup on top of it, then you might find it helpful to use the [`--dont-start-it`](/docs/netdata-agent/installation/linux#optional-parameters-for-kickstartsh) option upon installation.
...
3. **Start the Netdata Agent**
   ...
   [Start the Netdata Agent](/docs/netdata-agent/maintenance/service-control)
