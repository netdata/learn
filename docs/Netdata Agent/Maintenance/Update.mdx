...
To update Netdata, [download](/docs/netdata-agent/installation/windows#download-the-windows-installer-msi) the latest installer and reinstall the Agent.
...
If you installed [Netdata manually from Git](/docs/developer-and-contributor-corner/install-the-netdata-agent-from-a-git-checkout) run our automatic requirements installer, which works on many Linux distributions, to ensure your system has the dependencies necessary for new features.
...
For detailed instructions, see [Update your Netdata Docker container](/docs/netdata-agent/installation/docker#update-your-netdata-docker-container).
...
This configuration file can be edited using our [`edit-config` script](/docs/netdata-agent/configuration).
...
| **Docker (OCI)**           | Image pull + recreate  | See [Docker update instructions](/docs/netdata-agent/installation/docker#update-your-netdata-docker-container)                |
