...
This guide is community-maintained and might not always reflect the latest details (like package versions). Double-check before proceeding! Want to help? [Submit a PR!](/docs/netdata-agent/installation/freebsd)
...
The simplest approach is to use our one-line [kickstart installer](/docs/netdata-agent/installation/linux).
...
| `--prepare-offline-install-source ./netdata-offline` | Prepare offline installation source. See [Offline Install Guide](/docs/netdata-agent/installation/linux/offline-systems). |
...
Anonymous usage data is collected by default. You can learn more or opt-out [here](/docs/netdata-agent/anonymous-telemetry-events).
