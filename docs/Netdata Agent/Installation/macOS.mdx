...
Community-maintained packages _may_ receive support from Netdata, but are only a best-effort affair. Learn more about [Netdata's platform support policy](/docs/netdata-agent/versions-&-platforms).
...
To install Netdata using our automatic [kickstart](/docs/netdata-agent/installation/linux) open a new terminal and run:
...
The `kickstart.sh` script accepts additional parameters to automatically [connect](/docs/netdata-cloud/connect-agent) your node to Netdata
...
- `--claim-proxy`: Specify a proxy to use when connecting to the Cloud in the form of `http://[user:pass@]host:ip` for an HTTP(S) proxy.
  See [connecting through a proxy](/docs/netdata-cloud/connect-agent#proxy-configuration) for details.
...
If you experience issues while connecting your node, follow the steps in our [Troubleshoot](/docs/netdata-cloud/connect-agent#troubleshooting) documentation.
...
Homebrew installs Netdata but does not start the Agent automatically, so the local dashboard at `http://localhost:19999` is not available until you start it yourself. See [macOS (Homebrew)](/docs/netdata-agent/maintenance/service-control#macos-homebrew) for the `brew services` commands to start, stop, restart, and check the status of the Agent.

### Connect a Homebrew-installed Agent to Netdata Cloud

The easiest way to connect a Homebrew-installed Netdata Agent to Netdata Cloud is via the local dashboard UI, as described in [Method 1: Via UI](/docs/netdata-cloud/connect-agent#method-1-via-ui-recommended):
...
For the full list of claiming options and troubleshooting, see [Connect Agent to Cloud](/docs/netdata-cloud/connect-agent).
...
3. Use the same terminal session to install some of Netdata's prerequisites using Homebrew. If you don't want to use [Netdata Cloud](/docs/netdata-cloud), you can omit `cmake`.
...
- The number of charts displaying system metrics is limited, but you can use any of Netdata's [external plugins](/docs/developer-and-contributor-corner/external-plugins) to monitor any services you might have installed on your macOS system.
- You could also use a macOS system as the parent node in a [streaming configuration](/docs/netdata-parents/parent-child-configuration-reference).
