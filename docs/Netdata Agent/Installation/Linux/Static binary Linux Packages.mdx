...
Netdata provides pre-built static binaries for Linux systems where native packages aren't available. The [installer](/docs/netdata-agent/installation/linux) automatically uses these static builds when needed.
...
→ [Perform a cleanup](/docs/developer-and-contributor-corner/install-the-netdata-agent-from-a-git-checkout#perform-a-cleanup-in-your-netdata-repo)
