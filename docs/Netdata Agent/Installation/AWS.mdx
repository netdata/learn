...
You can install Netdata on cloud instances to monitor the apps/services running there, or use multiple instances in a [parent-child streaming](/docs/netdata-parents/parent-child-configuration-reference) configuration.

For recommended VM sizing and the resource impact of running Netdata on cloud VMs, see [Impact on Cloud VMs](/docs/netdata-agent/resource-utilization/impact-on-cloud-vms).

## Install Netdata on Amazon EKS (Kubernetes)

Amazon Elastic Kubernetes Service (Amazon EKS) is AWS's managed Kubernetes service. To monitor an EKS cluster, follow the dedicated [Kubernetes installation guide](/docs/netdata-agent/installation/kubernetes), which covers deploying Netdata on Kubernetes using Helm.
