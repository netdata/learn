...
-   `--disable-telemetry`: Opt-out of [anonymous statistics](/docs/deployment-in-production/secure-your-nodes/anonymous-telemetry-events) we use to make
    Netdata better.
...
-   `--local-files`: Used for [offline installations](/docs/moved-installation/moved-installation-methods/offline-systems). Pass four file paths: the Netdata
...
Unlike the [`kickstart.sh`](/docs/moved-installation/moved-installation-methods/one-line-installer-kickstart.sh), the `netdata-installer.sh` script does
not allow you to automatically [connect](/docs/getting-started/monitor-your-infrastructure/connect-agent-to-cloud) your node to Netdata Cloud immediately after installation.

See the [connect to cloud](/docs/getting-started/monitor-your-infrastructure/connect-agent-to-cloud) doc for details on connecting a node with a manual installation of Netdata.
