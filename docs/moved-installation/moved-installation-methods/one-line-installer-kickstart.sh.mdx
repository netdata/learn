...
The kickstart script works on all Linux distributions and macOS environments. By default, automatic nightly updates are enabled. If you are installing on macOS, make sure to check the [install documentation for macOS](/docs/moved-installation/install-on-specific-environments/macos) before continuing.


> :bulb: Tip
>
> If you are unsure whether you want nightly or stable releases, read the [installation guide](/docs/getting-started/install-netdata#nightly-vs-stable-releases).  

> If you want to turn off [automatic updates](/docs/getting-started/install-netdata#automatic-updates), use the `--no-updates` option. You can find more installation options below.
...
- `--prepare-offline-install-source`: Instead of insallling the agent, prepare a directory that can be used to install on another system without needing to download anything. See our [offline installation documentation](/docs/moved-installation/moved-installation-methods/offline-systems) for more info.
...
```bash
[ "3ccd09092f88f23a29554be689c04bfb" = "$(curl -Ss https://my-netdata.io/kickstart.sh | md5sum | cut -d ' ' -f 1)" ] && echo "OK, VALID" || echo "FAILED, INVALID"
```
