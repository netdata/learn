...
The most common issue we have seen claiming nodes in Docker is [running on older hosts with seccomp enabled](/docs/configuring/connect-agent-to-cloud#known-issues-on-older-hosts-with-seccomp-enabled).
...
> Read more about [Starting, Stopping and Restarting the Agent](/docs/maintenance/starting-and-stopping-netdata-agents).
