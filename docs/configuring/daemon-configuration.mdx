...
After `netdata.conf` has been modified, Netdata needs to be [restarted](/docs/maintenance/starting-and-stopping-netdata-agents) for
changes to apply:
...
|                 storage tiers                 |    `1`     | The number of storage tiers you want to have in your dbengine. Check the tiering mechanism in the [dbengine's reference](/docs/contributing/database-engine#tiering). You can have up to 5 tiers of data (including the _Tier 0_). This number ranges between 1 and 5.                                                                                                                                                                                                                                                                                                                                                                      |
...
|                 update every                  |    `1`     | The frequency in seconds, for data collection. For more information see the [performance guide](/docs/configuring/how-to-optimize-the-netdata-agent-s-performance). These metrics stored as _Tier 0_ data. Explore the tiering mechanism in the [dbengine's reference](/docs/contributing/database-engine#tiering).                                                                                                                                                                                                                                                                                                                                                     |
...
[Alarm notifications](/docs/alerting/notifications/netdata-agent-notifications) are configured in `health_alarm_notify.conf`.
...
|                enabled alarms                  |                       *                          | Defines which alarms to load from both user and stock directories. This is a [simple pattern](/docs/contributing/libnetdata/simple-patterns) list of alarm or template names. Can be used to disable specific alarms. For example, `enabled alarms =  !oom_kill *` will load all alarms except `oom_kill`. |
