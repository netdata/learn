...
([kickstart.sh](/docs/installation/one-line-installer-for-all-linux-systems), or a [manual installation from
Git](/docs/installation/from-a-git-checkout).
...
[reinstall Netdata](/docs/maintenance/reinstall-netdata-agents) with Cloud enabled or change the runtime setting in your
`cloud.conf` file.

If you passed `--disable-cloud` to `netdata-installer.sh` during installation, you must
[reinstall](/docs/maintenance/reinstall-netdata-agents) your Agent. Use the same method as before, but pass `--require-cloud` to
