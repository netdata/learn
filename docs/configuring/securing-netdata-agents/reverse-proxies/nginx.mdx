...
In case Netdata's web server has been [configured to use TLS](/docs/configuring/securing-netdata-agents/web-server#enabling-tls-support), it is
...
`allow connections from` accepts [Netdata simple patterns](/docs/miscellaneous/libnetdata/simple-patterns) to match against the
