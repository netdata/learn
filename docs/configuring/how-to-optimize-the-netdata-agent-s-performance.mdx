...
[Use a different metric storage database](/docs/configuring/optimizing-metrics-database/database-modes-for-parent-child-setups) | | :heavy_check_mark: | :heavy_check_mark:
...
The Netdata Agent runs with the lowest possible [process scheduling policy](/docs/configuring/netdata-daemon#netdata-process-scheduling-policy), which is `nice 19`, and uses the `idle` process scheduler.
...
To estimate and control memory consumption, you can [disable unneeded plugins or collectors](#disable-unneeded-plugins-or-collectors), [change how long Netdata stores metrics](/docs/configuring/optimizing-metrics-database/change-how-long-netdata-stores-metrics), or [use a different metric storage database](/docs/configuring/optimizing-metrics-database/database-modes-for-parent-child-setups).
...
To configure retention, you can [change how long Netdata stores metrics](/docs/configuring/optimizing-metrics-database/change-how-long-netdata-stores-metrics).
To control disk I/O [use a different metric storage database](/docs/configuring/optimizing-metrics-database/database-modes-for-parent-child-setups), avoid querying the 
...
collected data from children Netdata nodes running on the production infrastructure, using [streaming and replication](/docs/streaming/understanding-how-streaming-works).
...
See [using a different metric storage database](/docs/configuring/optimizing-metrics-database/database-modes-for-parent-child-setups).
...
dashboard, [configure the Agent](/docs/configuring/configuration) to collect metrics less often.
...
See how to [change how long Netdata stores metrics](/docs/configuring/optimizing-metrics-database/change-how-long-netdata-stores-metrics).

## Use a different metric storage database

Consider [using a different metric storage database](/docs/configuring/optimizing-metrics-database/database-modes-for-parent-child-setups) when running Netdata on IoT devices,
and for children in a parent-child set up based on [streaming and replication](/docs/streaming/understanding-how-streaming-works).
...
A dedicated web server like nginx provides more robustness than the Agent's internal [web server](/docs/contributing/web).
Nginx can handle more concurrent connections, reuse idle connections, and use fast gzip compression to reduce payloads.

For details on installing another web server as a proxy for the local Agent dashboard, see [reverse proxies](/docs/configuring/securing-netdata-agents/reverse-proxies).
