...
learn_link: "https://learn.netdata.cloud/docs/configuring/optimizing-metrics-database/database-modes-for-parent-child-setups"
---
...
1. `dbengine`, (the default) data are in database files. The [Database Engine](/docs/architecture/database-engine) works like a
   ...
   details see [here](/docs/architecture/database-engine).
...
[Parent - Child](/docs/streaming/understanding-how-streaming-works) setups at the children side and only when the
resource constraints are very strict.

So,

- On a single node setup, use `[db].mode = dbengine`.
- On a [Parent - Child](/docs/streaming/understanding-how-streaming-works) setup, use `[db].mode = dbengine` on the
...
When having a [**parent - child**](/docs/streaming/understanding-how-streaming-works) setup, the child (the
