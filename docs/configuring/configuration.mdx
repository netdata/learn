...
installation](/docs/installation#install-on-linux-with-one-line-installer), the **Netdata config
...
- `netdata.conf` is the main configuration file. This is where you'll find most configuration options. Read descriptions
  for each in the [daemon config](/docs/configuring/daemon-configuration) doc.
...
- `health.d/` is a directory that contains [health configuration files](/docs/alerting/health-configuration-reference).
- `health_alarm_notify.conf` enables and configures [alarm notifications](/docs/alerting/notifications).
- `statsd.d/` is a directory for configuring Netdata's [statsd collector](/docs/data-collection/apm/statsd).
- `stream.conf` configures [parent-child streaming](/docs/streaming/streaming-configuration-reference) between separate nodes running the Agent.
- `.environment` is a hidden file that describes the environment in which the Netdata Agent is installed, including the
  `PATH` and any installation options. Useful for [reinstalling](/docs/maintenance/reinstall-netdata-agents) or
  [uninstalling](/docs/maintenance/uninstall-netdata-agents) the Agent.
...
See [configure agent containers](/docs/installation/docker#configure-agent-containers).
...
After you make your changes, you need to [restart the Agent](/docs/maintenance/starting-and-stopping-netdata-agents) with `sudo systemctl
