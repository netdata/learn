...
overview](/docs/improving-netdata---developers/web/dashboards).
...
Netdata uses an [internal, static-threaded web server](/docs/deployment-in-production/secure-your-nodes/web-server) to host the HTML, CSS, and JavaScript
files that make up the local Agent dashboard. You don't have to configure anything to access it, although you can adjust
[your settings](/docs/deployment-in-production/secure-your-nodes/web-server#other-netdataconf-web-section-options) in the `netdata.conf` file, or run Netdata
behind an [Nginx proxy](/docs/deployment-in-production/secure-your-nodes/reverse-proxies/nginx), and so on.
...
broken down into [contexts](/docs/improving-netdata---developers/web/dashboards#contexts) and
[families](/docs/improving-netdata---developers/web/dashboards#families).
...
[Increase the metrics retention policy](/docs/deployment-in-production/metric-retention---database/change-how-long-netdata-stores-metrics) for your node to see more historical
timeframes.
...
[families](/docs/improving-netdata---developers/web/dashboards#families) from that section. Netdata automatically
...
For information on creating custom dashboards from scratch, see the [custom dashboards](/docs/visualizations/agent-dashboard/custom-dashboards) or
[Atlassian Confluence dashboards](/docs/exporting-data/atlassian-confluence-dashboards) guides.
