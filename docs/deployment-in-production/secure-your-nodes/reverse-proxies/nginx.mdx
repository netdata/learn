...
In case Netdata's web server has been [configured to use TLS](/docs/deployment-in-production/secure-your-nodes/web-server#enabling-tls-support), it is
...
`allow connections from` accepts [Netdata simple patterns](/docs/improving-netdata---developers/libnetdata/simple-patterns) to match against the
