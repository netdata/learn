...
You can disable the local dashboard (and API) but retain the encrypted Agent-Cloud link ([ACLK](/docs/deployment-in-production/agent-cloud-link-aclk)) that
...
Save and close the editor, then [restart your Agent](/docs/getting-started/manage-and-configure/start-stop-or-restart-the-netdata-agent) using `sudo systemctl
...
> See the [configuration basics doc](/docs/getting-started/manage-and-configure/configuration) for details on how to find `netdata.conf` and use
> `edit-config`.
...
[access lists](/docs/deployment-in-production/secure-your-nodes/web-server#access-lists). This method also fully retains the ability to stream metrics
...
patterns](/docs/improving-netdata---developers/libnetdata/simple-patterns).
...
See the [web server](/docs/deployment-in-production/secure-your-nodes/web-server#access-lists) docs for additional details about access lists. You can take
access lists one step further by [enabling SSL](/docs/deployment-in-production/secure-your-nodes/web-server#enabling-tls-support) to encrypt data from local
...
[lighthttpd](/docs/deployment-in-production/secure-your-nodes/reverse-proxies/lighttpd-v1.4.x), 
[caddy](/docs/deployment-in-production/secure-your-nodes/reverse-proxies/caddy), and
[H2O](/docs/deployment-in-production/secure-your-nodes/reverse-proxies/h2o).
...
The default configuration uses a public registry under registry.my-netdata.io (more information about the registry here: [mynetdata-menu-item](/docs/deployment-in-production/secure-your-nodes/registry) ). Please be aware that if you use that public registry, you submit the following information to a third party server: 

-   The url where you open the web-ui in the browser (via http request referrer)
-   The hostnames of the Netdata servers

If sending this information to the central Netdata registry violates your security policies, you can configure Netdata to [run your own registry](/docs/deployment-in-production/secure-your-nodes/registry#run-your-own-registry).
...
about the information collected, and learn how to-opt, on our [anonymous telemetry events](/docs/deployment-in-production/secure-your-nodes/anonymous-telemetry-events) page.
