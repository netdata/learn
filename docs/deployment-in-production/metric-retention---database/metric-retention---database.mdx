...
1. `dbengine`, (the default) data are in database files. The [Database Engine](/docs/deployment-in-production/metric-retention---database/database-engine) works like a
   ...
   details see [here](/docs/deployment-in-production/metric-retention---database/database-engine).
...
[Parent - Child](/docs/deployment-in-production/streaming-and-replication) setups at the children side and only when the
resource constraints are very strict.

So,

- On a single node setup, use `[db].mode = dbengine`.
- On a [Parent - Child](/docs/deployment-in-production/streaming-and-replication) setup, use `[db].mode = dbengine` on the
...
When having a [**parent - child**](/docs/deployment-in-production/streaming-and-replication) setup, the child (the
...
You can also disable [data collection plugins](/docs/data-collection) that you don't need. Disabling such plugins will also
