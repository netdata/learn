...
[`dbengine`](/docs/deployment-in-production/metric-retention---database/database-engine), to store metrics.
...
[DBENGINE memory requirements](/docs/deployment-in-production/metric-retention---database/database-engine#memory-requirements).
...
[`edit-config`](/docs/getting-started/manage-and-configure/configuration#use-edit-config-to-edit-configuration-files) 
and make your changes in the `[db]` subsection. 

Save the file and restart the Agent with `sudo systemctl restart netdata`, or
the [appropriate method](/docs/getting-started/manage-and-configure/start-stop-or-restart-the-netdata-agent) 
