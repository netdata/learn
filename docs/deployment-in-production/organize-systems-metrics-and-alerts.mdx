...
[Spaces](/docs/getting-started/monitor-your-infrastructure/spaces) are used for organization-level or infrastructure-level 
grouping of nodes and people. A node can only appear in a single space, while people can have access to multiple spaces.

The [war rooms](/docs/getting-started/monitor-your-infrastructure/war-rooms) in a space bring together nodes and people in 
collaboration areas. War rooms can also be used for fine-tuned 
[role based access control](/docs/deployment-in-production/secure-your-nodes/role-based-access-model). 
...
to violate any of the [host label naming rules](/docs/getting-started/manage-and-configure/common-configuration-changes#organize-nodes-with-host-labels).
...
> documentation](/docs/deployment-in-production/streaming-and-replication/streaming-configuration-reference#securing-streaming-communications) for details. You may also want to use
> [access lists](/docs/deployment-in-production/secure-your-nodes/web-server#access-lists) or [expose the API only to LAN/localhost
> connections](/docs/deployment-in-production/security-and-privacy-design#expose-netdata-only-in-a-private-lan).
...
documentation](/docs/alerts-and-notifications/configure-alerts#alarm-line-host-labels) for more details, and then get creative!

### Host labels in metrics exporting

If you have enabled any metrics exporting via our experimental [exporters](/docs/exporting-data/exporting-reference), any new host
...
more about exporting, read the [documentation](/docs/exporting-data/exporting-reference).
