...
To estimate and control memory consumption, you can [change how long Netdata stores metrics](/docs/deployment-in-production/metric-retention---database/change-how-long-netdata-stores-metrics), or [use a different metric storage database](/docs/deployment-in-production/metric-retention---database).
...
production system using a [streaming and replication](/docs/deployment-in-production/streaming-and-replication), [reduce the data collection frequency](#reduce-collection-frequency).
...
infrastructure](/docs/visualizations/infrastructure-overview) in real-time with Netdata Cloud, or running [Metric
Correlations](/docs/troubleshooting-and-machine-learning/metric-correlations).

The Netdata Agent runs with the lowest possible [process scheduling
policy](/docs/getting-started/manage-and-configure/netdata-daemon#netdata-process-scheduling-policy), which is `nice 19`, and uses the `idle` process scheduler.
...
A dedicated web server like Nginx provides far more robustness than the Agent's internal [web server](/docs/improving-netdata---developers/web).
...
doc](/docs/deployment-in-production/secure-your-nodes/reverse-proxies/nginx).
