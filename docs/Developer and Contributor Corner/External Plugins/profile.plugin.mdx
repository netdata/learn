...
Edit the `netdata.conf` configuration file using [`edit-config`](/docs/netdata-agent/configuration#edit-configuration-files) from the [Netdata config directory](/docs/netdata-agent/configuration#locate-your-config-directory), which is typically at `/etc/netdata`.
