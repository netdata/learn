...
visualize your nodes' metrics. When you [look at charts in Netdata Cloud](/docs/netdata-dashboards-and-visualizations/netdata-charts)
...
- **Performance**: Every query to a node's database takes only a few milliseconds to complete for responsiveness when
  ...
  like [Metric Correlations](/docs/machine-learning-and-assisted-troubleshooting/metric-correlations).
...
Read our document on changing [how long Netdata stores metrics](/docs/typical-netdata-agent-configurations/optimizing-metrics-database/change-how-long-netdata-stores-metrics) on your nodes for
details.

You can also stream between nodes using [streaming](/docs/metrics-streaming-and-replication/streaming-configuration-reference), allowing to replicate databases and create
...
TimescaleDB, and many others, see details on [integrating Netdata via exporting](/docs/exporting-data-to-other-systems).
...
To enable certain features, such as [viewing active alarms](/docs/monitor-your-infrastructure/view-active-alerts)
or [filtering by hostname/service](/docs/monitor-your-infrastructure/war-rooms), Netdata Cloud does
