...
3. _Connect nodes*_, click on **Nodes** tab. Copy the claiming script to your node and run it. See the
   [connect to Cloud doc](/docs/developer-and-contributor-corner/configuring/connect-agent-to-cloud) for details.
...
Next, pick a role for the invited user. You can read more about [which roles are available](/docs/developer-and-contributor-corner/configuring/securing-netdata-agents/role-based-access-model#what-roles-are-available) based on your [subscription plan](/docs/monitor-your-infrastructure/netdata-plans).
