...
learn_rel_path: "Contributing to Netdata/External plugins"
sidebar_position: "1208120"
---
# charts.d.plugin
...
To better understand the guidelines and the API behind our External plugins, please have a look at the [Introduction to External plugins](/docs/contributing-to-netdata/external-plugins) prior to reading this page.
...
By default, `charts.d.plugin` is not included as part of the install when using [our official native DEB/RPM packages](/docs/install-the-netdata-agent/native-linux-distribution-packages). You can install it by installing the `netdata-plugin-chartsd` package.

## Configuration

`charts.d.plugin` itself can be [configured](/docs/typical-netdata-agent-configurations/configuration#use-edit-config-to-edit-configuration-files) using the configuration file `/etc/netdata/charts.d.conf`. This file is also a BASH script.
