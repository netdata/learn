...
While Netdata's charts require no configuration and are [easy to interact with](/docs/agent/dashboards/interact-chartsx),
...
For a refresher on the anatomy of a chart, see [dashboards and charts](/docs/agent/dashboards/dashboards-chartsx).
...
dashboard](/docs/agent/dashboards/customizex#change-chart-legend-position)) is a legend of dimensions. When there are
...
Dimensions can be [hidden](/docs/agent/dashboards/interact-chartsx#show-and-hide-dimensions) to help you focus your
attention.
...
Netdata also uses contexts for [alarm configuration](/docs/monitor/configure-alarms) (the [`on`
...
dashboard. We recommend looking into [using the time & date picker](/docs/agent/dashboards/pick-timeframesx), but you can
also skip ahead to [customization](/docs/agent/dashboards/customizex).

If you feel familiar enough with navigating the dashboard, a good next step is [data
collection](/docs/collect/how-collectors-workx), which will help you see new metrics on Netdata's dashboard based on
the running services/applications.

### Further reading & related information

- [Interact with charts](/docs/agent/dashboards/interact-chartsx)
- [Pick timeframes to visualize](/docs/agent/dashboards/pick-timeframesx)
