...
Can't find a dashboard? Dashboards belong to a specific [Room](/docs/netdata-cloud/spaces-and-rooms#create-your-first-room), and each Room keeps its own separate dashboards. A dashboard that appears to be missing is often in another Room — check the **Dashboards** tab in each Room to locate it.
