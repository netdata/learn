...
- The **Actions column** allows you to:
    - Create a new [silencing rule](/docs/alerts-&-notifications/notifications/centralized-cloud-notifications/centralized-cloud-notifications-reference#alert-notification-silencing-rules).
...
Users can also silence or disable alerts at the Agent level by editing alert configuration. For example, set `to: silent` in the alert's `health.d/*.conf` file and run `sudo netdatacli reload-health` to stop notifications, or use `enabled alarms = !alert_name *` in `netdata.conf` to exclude specific alerts entirely (requires restarting the Agent). See [manual configuration](/docs/alerts-&-notifications/alert-configuration-reference) for step-by-step instructions.
...
- **[Alerts Automation](/docs/netdata-ai/alerts-automation)** — Describe what you want in plain English and let AI generate the alert configuration, including thresholds and tuning parameters
- **[Alerts Configuration Manager](/docs/alerts-&-notifications/creating-alerts-with-the-alerts-configuration-manager)** — Visual UI wizard for manual alert creation and fine-tuning
- **[Manual configuration](/docs/alerts-&-notifications/alert-configuration-reference)** — Edit `health.d/*.conf` files directly for full control

:::note

**Alerts Automation** and the **Alerts Configuration Manager** require a Netdata Cloud paid plan.
See [Netdata OSS limitations and feature availability](/docs/security-and-privacy-design/access-control-and-feature-availability) to confirm plan requirements.
Community plan users can still create and tune alerts using [manual configuration](/docs/alerts-&-notifications/alert-configuration-reference).
