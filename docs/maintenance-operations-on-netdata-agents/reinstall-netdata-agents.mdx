...
any [user configuration](/docs/typical-netdata-agent-configurations/configuration) in `netdata.conf` or other files, and will keep the same install
type that was used for the original install.

If you used any [optional
parameters](/docs/install-the-netdata-agent/one-line-installer-for-all-linux-systems#optional-parameters-to-alter-your-installation) during initial
...
best route is to [uninstall](/docs/maintenance-operations-on-netdata-agents/uninstall-netdata-agents) and then try a fresh installation using the [one-line
installer](/docs/install-the-netdata-agent/one-line-installer-for-all-linux-systems).
