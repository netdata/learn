...
Everything is the same with [badges](/docs/improving-netdata---developers/web/api/netdata-badges). In short:
...
- `OPTIONS` is a space separated list of `percentage`, `absolute`, `min2max`, `unaligned`,
     `match-ids`, `match-names`. Check the [badges](/docs/improving-netdata---developers/web/api/netdata-badges) documentation for more info.
