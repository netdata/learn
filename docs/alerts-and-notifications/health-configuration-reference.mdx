...
[infrastructure](/docs/netdata-architecture/the-role-of-netdata-cloud), you must configure each node with the same health configuration
files.
...
- The `[health]` section in `netdata.conf`. By editing the daemon's behavior, you can disable health monitoring
    ...
    [daemon configuration](/docs/typical-netdata-agent-configurations/daemon-configuration#health-section-options) for a table of
...
Navigate to your [Netdata config directory](/docs/typical-netdata-agent-configurations/configuration) and
...
[simple pattern](/docs/contributing-to-netdata/libnetdata/simple-patterns) that
...
To write a new health entity into a new file, navigate to your [Netdata config directory](/docs/typical-netdata-agent-configurations/configuration),
...
- A few lines use space-separated lists to define how the entity behaves. You can use `*` as a wildcard or prefix with
    `!` for a negative match. Order is important, too! See our [simple patterns docs](/docs/contributing-to-netdata/libnetdata/simple-patterns) for
    more examples.
...
list of [simple patterns](/docs/contributing-to-netdata/libnetdata/simple-patterns). For example,
...
list of [simple patterns](/docs/contributing-to-netdata/libnetdata/simple-patterns). For
...
The value is a space-separated list of [simple patterns](/docs/contributing-to-netdata/libnetdata/simple-patterns). For
...
The value is a space-separate list of simple patterns. See our [simple patterns docs](/docs/contributing-to-netdata/libnetdata/simple-patterns) for
some examples.
...
The full [database query API](/docs/netdata-apis/queries) is supported. In short:

- `METHOD` is one of  the available [grouping methods](/docs/netdata-apis/queries#grouping-methods) such as `average`, `min`, `max` etc.
     This is required.
...
- `OPTIONS` is a space separated list of `percentage`, `absolute`, `min2max`, `unaligned`,
     `match-ids`, `match-names`. Check the [badges](/docs/netdata-apis/netdata-badges) documentation for more info.
...
Defines the list of labels present on a host. See our [host labels guide](/docs/typical-netdata-agent-configurations/organize-systems-metrics-and-alerts) for
...
See our [simple patterns docs](/docs/contributing-to-netdata/libnetdata/simple-patterns) for more examples.
...
> If you don't know how to find the CHART_NAME, you can read about it [here](/docs/contributing-to-netdata/web#charts).
...
templates for charts belonging to a given [context](/docs/contributing-to-netdata/web#contexts). The reason is that all charts of a given
context are essentially identical, with the only difference being the [family](/docs/contributing-to-netdata/web#families) that
...
### Example 8 - [Anomaly rate](/docs/machine-learning-and-assisted-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) based CPU dimensions alarm

Warning if 5 minute rolling [anomaly rate](/docs/machine-learning-and-assisted-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) for any CPU dimension is above 5%, critical if it goes above 20%:
...
### Example 9 - [Anomaly rate](/docs/machine-learning-and-assisted-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) based CPU chart alarm

Warning if 5 minute rolling [anomaly rate](/docs/machine-learning-and-assisted-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) averaged across all CPU dimensions is above 5%, critical if it goes above 20%:
...
### Example 10 - [Anomaly rate](/docs/machine-learning-and-assisted-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) based node level alarm

Warning if 5 minute rolling [anomaly rate](/docs/machine-learning-and-assisted-troubleshooting/machine-learning-ml-powered-anomaly-detection#anomaly-rate) averaged across all ML enabled dimensions is above 5%, critical if it goes above 20%:
...
The `lookup` line will use the `anomaly_rate` dimension of the `anomaly_detection.anomaly_rate` ML chart to calculate the average [node level anomaly rate](/docs/machine-learning-and-assisted-troubleshooting/machine-learning-ml-powered-anomaly-detection#node-anomaly-rate) over the last 5 minues.
...
a [Netdata simple pattern](/docs/contributing-to-netdata/libnetdata/simple-patterns) to create
...
Use a [simple pattern](/docs/contributing-to-netdata/libnetdata/simple-patterns)! One example of a simple pattern is a single wildcard
(`*`).
...
our [simple patterns documentation](/docs/contributing-to-netdata/libnetdata/simple-patterns).
...
You can compile Netdata with [debugging](/docs/typical-netdata-agent-configurations/netdata-daemon#debugging) and then set in `netdata.conf`:
