...
-   Learn all the specifics of [using charts](/web.md#using-charts#using-charts) or the differences between [charts,
    context, and families](/web.md#charts-contexts-families#charts-contexts-families).
...
Once you save your changes, [restart Netdata](/docs/#start-stop-and-restart-netdata) to load your new configuration.

**What's next?**:

-   [Change how long Netdata stores metrics](/docs/#change-how-long-netdata-stores-metrics) by changing the `page cache size`
...
-   Run your own [registry](/registry.md#run-your-own-registry#run-your-own-registry).
...
Read our tutorial, [**Changing how long Netdata stores metrics**](/tutorials/longer-metrics-storage.md), to learn
more.

**What's next?**:

-   Learn more about the [memory requirements for the database engine](/database/engine.md#memory-requirements#memory-requirements)
...
Netdata](/docs/#start-stop-and-restart-netdata).
...
-   Configure `systemd` to expose [systemd services
    utilization](/collectors/cgroups.plugin.md#monitoring-systemd-services#monitoring-systemd-services) metrics automatically.
-   [Reconfigure individual charts](/daemon/config.md#per-chart-configuration#per-chart-configuration) in `netdata.conf`.
...
To show all your servers in that menu, you need to [register for or sign in](/netdata-cloud/signing-in.md) to
[Netdata Cloud](/netdata-cloud/) from each system. Each system will then appear in the **My nodes** menu, which
