...
and applications, our [**step-by-step tutorial**](/docs/step-by-step/step-00) might be a better fit.

> If you haven't installed Netdata yet, visit the [installation instructions](/docs/packaging/installer) for details,
...
-   Read more about the [standard Netdata dashboard](/docs/web/gui/).
-   Learn all the specifics of [using charts](/docs/web#using-charts) or the differences between [charts,
    context, and families](/docs/web#charts-contexts-families).
...
-   Move Netdata's dashboard to a [different port](https://docs.netdata.cloud/web/server/) or enable TLS/HTTPS
    encryption.
-   See all the `netdata.conf` options in our [daemon configuration documentation](/docs/daemon/config/).
-   Run your own [registry](/docs/registry#run-your-own-registry).
...
Read our tutorial, [**Changing how long Netdata stores metrics**](/docs/tutorials/longer-metrics-storage), to learn
more.

**What's next?**:

-   Learn more about the [memory requirements for the database engine](/docs/database/engine#memory-requirements)
...
documentation](/docs/collectors/plugins.d/) to find the appropriate module for your source. Those pages will contain more
...
-   Look at the [full list of data collection modules](/docs/collectors/collectors)
    to configure your sources for auto-detection and monitoring.
-   Improve the [performance](/docs/performance) of Netdata on low-memory systems.
-   Configure `systemd` to expose [systemd services
    utilization](/docs/collectors/cgroups.plugin#monitoring-systemd-services) metrics automatically.
-   [Reconfigure individual charts](/docs/daemon/config#per-chart-configuration) in `netdata.conf`.
...
-   Follow the [health quickstart](/docs/health/quickstart) to locate and edit existing health entities, and then create
    your own.
-   See all the alarm options via the [health configuration reference](/docs/health/reference).
-   Add a new notification method, like [Slack](/docs/health/notifications/slack/).
...
To show all your servers in that menu, you need to [register for or sign in](/docs/netdata-cloud/signing-in) to
...
-   Read up on how the [Netdata Cloud registry works](/docs/registry/), and what kind of data it stores and sends to your
    web browser.
-   Familiarize yourself with the [Nodes View](/docs/netdata-cloud/nodes-view)
...
-   Centralize Netdata metrics from many systems with [streaming](/docs/streaming)
-   Enable long-term archiving of Netdata metrics via [backends](/docs/backends) to time-series databases.
-   Improve security by putting Netdata behind an [Nginx proxy with SSL](/docs/running-behind-nginx).

Or, learn more about how you can contribute to [Netdata core](/docs/contributing) or our
