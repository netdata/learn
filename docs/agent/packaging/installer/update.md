...
it from the default in our [Docker installation instructions](/docs/agent/packaging/docker#run-netdata-with-the-docker-command).
...
installation instructions](/docs/agent/packaging/docker#run-netdata-with-the-docker-command) for details. For example, using the
`docker` command:
