...
databases](/docs/export/external-databases), or jump in to [enabling a connector](/docs/export/enable-connector).
...
deprecated [backends](/docs/agent/backends#configuration) system. The most important difference is that type of a
connector should be specified in a section name before a colon and an instance name after the colon. Also, you can't use
`host tags` anymore. Set your labels using the [`[host labels]`](/guides/using-host-labels) section in
`netdata.conf`.
...
1.  The exporting engine uses a number of connectors to send Netdata metrics to external time-series databases. See our
    [list of supported databases](/docs/export/external-databases#supported-databases) for information on which
    connector to enable and configure for your database of choice.

    -   [**AWS Kinesis Data Streams**](/docs/agent/exporting/aws_kinesis): Metrics are sent to the service in `JSON`
        format.
    -   [**Google Cloud Pub/Sub Service**](/docs/agent/exporting/pubsub): Metrics are sent to the service in `JSON`
        format.
    -   [**Graphite**](/docs/agent/exporting/graphite): A plaintext interface. Metrics are sent to the database server as
        ...
        Graphite](/guides/export/export-netdata-metrics-graphite).
    -   [**JSON** document databases](/docs/agent/exporting/json)
    -   [**OpenTSDB**](/docs/agent/exporting/opentsdb): Use a plaintext, HTTP, or HTTPS interfaces. Metrics are sent to
        OpenTSDB as `prefix.chart.dimension` with tag `host=hostname`.
    -   [**MongoDB**](/docs/agent/exporting/mongodb): Metrics are sent to the database in `JSON` format.
    -   [**Prometheus**](/docs/agent/exporting/prometheus): Use an existing Prometheus installation to scrape metrics
        from node using the Netdata API.
    -   [**Prometheus remote write**](/docs/agent/exporting/prometheus/remote_write). A binary snappy-compressed protocol
        buffer encoding over HTTP. Supports many [storage
        providers](https://prometheus.io/docs/operating/integrations/#remote-endpoints-and-storage).
    -   [**TimescaleDB**](/docs/agent/exporting/timescale): Use a community-built connector that takes JSON streams from a
