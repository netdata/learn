...
anymore. Set your labels using the [`[host labels]`](/guides/using-host-labels) section in `netdata.conf`.
...
1.  Supported databases and services
    ...
    -   **prometheus** is described at [prometheus page](/docs/agent/exporting/prometheus) since it pulls data from
        Netdata.
    ...
    -   **prometheus remote write** (a binary snappy-compressed protocol buffer encoding over HTTP used by
        ...
        **Splunk**, **VictoriaMetrics**, and a lot of other [storage
        ...
        protocol](/docs/agent/exporting/prometheus). Notes on using the remote write connector are
        [here](/docs/agent/exporting/prometheus/remote_write).

    -   **TimescaleDB** via [community-built connector](/docs/agent/exporting/timescale) that takes JSON streams from a Netdata
