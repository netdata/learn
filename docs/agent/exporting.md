...
The exporting engine is an update for the former [backends](agent/backends.md) which is deprecated and will be deleted
...
[backends](agent/backends.md#configuration). The most important difference is that type of a connector should be
specified in a section name before a colon and an instance name after the colon. Also, you can't use `host tags`
anymore. Set your labels using the [`[host labels]`](agent/tutorials/using-host-labels.md) section in `netdata.conf`.
...
1.  Supported databases and services
    ...
    -   **prometheus** is described at [prometheus page](agent/exporting/prometheus.md) since it pulls data from
        Netdata.
    ...
    -   **prometheus remote write** (a binary snappy-compressed protocol buffer encoding over HTTP used by
        ...
        **Splunk**, **VictoriaMetrics**, and a lot of other [storage
        ...
        protocol](agent/exporting/prometheus.md). Notes on using the remote write connector are
        [here](agent/exporting/prometheus/remote_write.md).

    -   **TimescaleDB** via [community-built connector](agent/exporting/timescale.md) that takes JSON streams from a Netdata
