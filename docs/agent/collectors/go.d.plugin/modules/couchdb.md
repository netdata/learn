...
Edit the `go.d/couchdb.conf` configuration file using `edit-config` from the your agent's [config
...
For all available options, see the CouchDB collector's [configuration
file](https://github.com/netdata/go.d.plugin/blob/master/config/go.d/couchdb.conf).

## Troubleshooting

To troubleshoot issues with the CouchDB collector, run the `go.d.plugin` with the debug option enabled.
