...
|:----:|:------:|:-:|:----------|
|[apps.plugin](agent/collectors/apps.plugin.md)|`C`|linux, freebsd|monitors the whole process tree on Linux and FreeBSD and breaks down system resource usage by **process**, **user** and **user group**.|
|[charts.d.plugin](agent/collectors/charts.d.plugin.md)|`BASH`|all|a **plugin orchestrator** for data collection modules written in `BASH` v4+.|
|[cups.plugin](agent/collectors/cups.plugin.md)|`C`|all|monitors **CUPS**|
|[fping.plugin](agent/collectors/fping.plugin.md)|`C`|all|measures network latency, jitter and packet loss between the monitored node and any number of remote network end points.|
|[ioping.plugin](agent/collectors/ioping.plugin.md)|`C`|all|measures disk latency.|
|[freeipmi.plugin](agent/collectors/freeipmi.plugin.md)|`C`|linux|collects metrics from enterprise hardware sensors, on Linux servers.|
|[nfacct.plugin](agent/collectors/nfacct.plugin.md)|`C`|linux|collects netfilter firewall, connection tracker and accounting metrics using `libmnl` and `libnetfilter_acct`.|
|[xenstat.plugin](agent/collectors/xenstat.plugin.md)|`C`|linux|collects XenServer and XCP-ng metrics using `lxenstat`.|
|[perf.plugin](agent/collectors/perf.plugin.md)|`C`|linux|collects CPU performance metrics using performance monitoring units (PMU).|
|[node.d.plugin](agent/collectors/node.d.plugin.md)|`node.js`|all|a **plugin orchestrator** for data collection modules written in `node.js`.|
|[python.d.plugin](agent/collectors/python.d.plugin.md)|`python`|all|a **plugin orchestrator** for data collection modules written in `python` v2 or v3 (both are supported).|
|[slabinfo.plugin](agent/collectors/slabinfo.plugin.md)|`C`|linux|collects kernel internal cache objects (SLAB) metrics.|
...
1.  **python**, use `python.d.plugin`, there are many examples in the [python.d
    directory](agent/collectors/python.d.plugin.md)
...
2.  **node.js**, use `node.d.plugin`, there are a few examples in the [node.d
    directory](agent/collectors/node.d.plugin.md)
...
3.  **BASH**, use `charts.d.plugin`, there are many examples in the [charts.d
    directory](agent/collectors/charts.d.plugin.md)
