...
to which it assigns all running processes. This list is [customizable](/docs/agent/collectors/apps_groups.conf) and Netdata
...
> glad to accept pull requests improving the [defaults](/docs/agent/collectors/apps_groups.conf) shipped with Netdata.
...
-   For **Applications** per [target configured](/docs/agent/collectors/apps_groups.conf).
...
The configuration file is `/etc/netdata/apps_groups.conf` (the default is [here](/docs/agent/collectors/apps_groups.conf)).
...
[`/etc/netdata/apps_groups.conf`](/docs/agent/collectors/apps_groups.conf)
(to edit it on your system run `/etc/netdata/edit-config apps_groups.conf`).
