...
list](agent/collectors/collectors.md). The only exception is the [third-party collectors](agent/collectors/collectors.md#third-party-plugins), which
...
From here on out, this quickstart guide will use the [Nginx collector](agent/collectors/go.d.plugin/modules/nginx.md) as an
...
Netdata's collector. Our [collector support list](agent/collectors/collectors.md) contains links to each collector's documentation page
so you can learn more.

For example, the [Nginx collector documentation](agent/collectors/go.d.plugin/modules/nginx.md) states that your Nginx
...
directory](agent/step-by-step/step-04.md#find-your-netdataconf-file). Launch `edit-config` with the path to the
...
For example, the [Nginx collector](agent/collectors/go.d.plugin/modules/nginx.md) has its configuration file at `go.d/nginx.conf`.
...
methods](agent/getting-started.md#start-stop-and-restart-netdata).
...
reference](agent/collectors/reference.md#troubleshoot-a-collector).

View our [collectors tutorials](agent/collectors.md#tutorials) to get specific instructions on enabling new and
popular collectors.

Finally, learn more advanced collector features, such as disabling plugins or developing a custom collector, in our
[internal plugin API](agent/collectors/reference.md#internal-plugins-api) or our [external plugin
docs](agent/collectors/plugins.d.md).
