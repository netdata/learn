...
By default, Netdata stores metrics in a custom database we call the [database engine](agent/database/engine.md), which
...
> engine's](agent/database/engine.md#memory-requirements) to ensure you don't overwhelm your system. Out of memory
> errors are no fun!
...
After you've made your changes, [restart Netdata](agent/getting-started.md#start-stop-and-restart-netdata).
...
As we said in [step 1](agent/step-by-step/step-01.md), we have only complimentary systems, not competitors! We're happy
...
[Restart](agent/getting-started.md#start-stop-and-restart-netdata) Netdata to enable the MongoDB backend. Click on the
...
documentation](agent/backends.md).
...
[Next: Set up a proxy &rarr;](agent/step-by-step/step-10.md)
