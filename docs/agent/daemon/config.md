...
the [web server access lists](/docs/agent/web/server#access-lists).
...
5. `[environment variables]` to [configure](#environment-variables-section-options) the environment variables used
   Netdata.
6. `[sqlite]` to [configure](#sqlite-section-options) the [Netdata daemon](/docs/agent/daemon) SQLite settings.
7. `[ml]` to configure settings for [machine learning](/docs/agent/ml).
8. `[health]` to [configure](#health-section-options) general settings for [health monitoring](/docs/agent/health).
9. `[web]` to [configure the web server](/docs/agent/web/server).
10. `[registry]` for the [Netdata registry](/docs/agent/registry).
11. `[global statistics]` for the [Netdata registry](/docs/agent/registry).
12. `[statsd]` for the general settings of the [stats.d.plugin](/docs/agent/collectors/statsd.plugin).
13. `[plugins]` to [configure](#plugins-section-options) which [collectors](/docs/agent/collectors) to use and PATH
    settings.
...
After `netdata.conf` has been modified, Netdata needs to be [restarted](/docs/configure/start-stop-restart) for
changes to apply:
...
|         log         |                         `/var/log/netdata`                         | The directory in which the [log files](/docs/agent/daemon#log-files) are kept.                                                                                                                                                     |
...
|      registry       |              `/opt/netdata/var/lib/netdata/registry`               | Contains the [registry](/docs/agent/registry) database and GUID that uniquely identifies each Netdata Agent                                                                                                                        |
...
|               debug                | `/var/log/netdata/debug.log`  | The filename to save debug information. This file will not be created if debugging is not enabled. You can also set it to `syslog` to send the debug messages to syslog, or `none` to disable this log. For more information check [Tracing Options](/docs/agent/daemon#debugging). |
...
the [registry documentation](/docs/agent/registry).
