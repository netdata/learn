...
This config file **is not needed by default**. Netdata works fine out of the box without it. But it does allow you to adapt the general behavior of Netdata, in great detail. You can find all these settings, with their default values, by accessing the URL `https://netdata.server.hostname:19999/netdata.conf`. For example check the configuration file of [netdata.firehol.org](http://netdata.firehol.org/netdata.conf). HTTP access to this file is limited by default to private IPs, via the [web server access lists](agent/web/server.md#access-lists).

`netdata.conf` has sections stated with `[section]`. You will see the following sections:

1.  `[global]` to [configure](#global-section-options) the [Netdata daemon](agent/daemon.md).
2.  `[web]` to [configure the web server](agent/web/server.md).
3.  `[plugins]` to [configure](#plugins-section-options) which [collectors](agent/collectors.md) to use and PATH
    settings.
4.  `[health]` to [configure](#health-section-options) general settings for [health monitoring](agent/health.md)
5.  `[registry]` for the [Netdata registry](agent/registry.md).
6.  `[backend]` to set up [streaming and replication](agent/streaming.md) options.
7.  `[statsd]` for the general settings of the [stats.d.plugin](agent/collectors/statsd.plugin.md).
...
|:-----:|:-----:|:---|---|---|
| process scheduling policy|`keep`|See [Netdata process scheduling policy](agent/daemon.md#netdata-process-scheduling-policy)|||
| OOM score|`1000`|See [OOM score](agent/#oom-score)|||
| glibc malloc arena max for plugins|`1`|See [Virtual memory](agent/daemon.md#virtual-memory).|||
| glibc malloc arena max for Netdata|`1`|See [Virtual memory](agent/daemon.md#virtual-memory).|||
| hostname|auto-detected|The hostname of the computer running Netdata.|||
| history|`3996`| Used with `memory mode = save/map/ram/alloc`, not the default `memory mode = dbengine`. This number reflects the number of entries the `netdata` daemon will by default keep in memory for each chart dimension. This setting can also be configured per chart. Check [Memory Requirements](agent/database.md) for more information. |||
| update every|`1`|The frequency in seconds, for data collection. For more information see [Performance](agent/performance.md).|||
| config directory|`/etc/netdata`|The directory configuration files are kept.|||
| stock config directory|`/usr/lib/netdata/conf.d`||||
| log directory|`/var/log/netdata`|The directory in which the [log files](agent/daemon.md#log-files) are kept.|||
...
| memory deduplication (ksm)|`yes`|When set to `yes`, Netdata will offer its in-memory round robin database to kernel same page merging (KSM) for deduplication. For more information check [Memory Deduplication - Kernel Same Page Merging - KSM](agent/database.md#ksm)|||
| TZ environment variable|`:/etc/localtime`|Where to find the timezone|||
| timezone|auto-detected|The timezone retrieved from the environment variable|||
| debug flags|`0x0000000000000000`|Bitmap of debug options to enable. For more information check [Tracing Options](agent/daemon.md#debugging).|||
| debug log|`/var/log/netdata/debug.log`|The filename to save debug information. This file will not be created if debugging is not enabled. You can also set it to `syslog` to send the debug messages to syslog, or `none` to disable this log. For more information check [Tracing Options](agent/daemon.md#debugging).|||
...
| cleanup obsolete charts after seconds|`3600`|See [monitoring ephemeral containers](agent/collectors/cgroups.plugin.md#monitoring-ephemeral-containers), also sets the timeout for cleaning up obsolete dimensions|||
...
| delete obsolete charts files|`yes`|See [monitoring ephemeral containers](agent/collectors/cgroups.plugin.md#monitoring-ephemeral-containers), also affects the deletion of files for obsolete dimensions|||
| delete orphan hosts files|`yes`|Set to `no` to disable non-responsive host removal.|||
| enable zero metrics|`no`|Set to `yes` to show charts when all their metrics are zero.|||

### [web] section options

Refer to the [web server documentation](agent/web/server.md)
...
monitoring](agent/health.md).

[Alarm notifications](agent/health/notifications.md) are configured in `health_alarm_notify.conf`.
...
To understand what this section is and how it should be configured, please refer to the [registry documentation](agent/registry.md).

### [backend]

Refer to the [streaming and replication](agent/streaming.md) documentation.
...
Most internal plugins will provide additional options. Check [Internal Plugins](agent/collectors.md) for more
information.
...
| setting | default | info |
| :-----:|:-----:|:---|
| update every | the value of `[global].update every` setting|The frequency in seconds the plugin should collect values. For more information check [Performance](agent/performance.md#performance).|
...
title, make it appear higher in Netdata's [menu](agent/web/gui.md#metrics-menus), tweak its dimensions, and much more.
...
| `type`            | Uniquely identify which [metrics menu](agent/web/gui.md#metrics-menus) on the Netdata dashboard this chart should appear under. Some examples include `system` (**System**), `disk` (**Disks**), `net` (**Network Interfaces**), and `netdata` (**Netdata Monitoring**). |
| `family`          | Change the chart's [family](agent/web.md#families) from its default. For example, you could force a disk space chart to collect metrics for family `sdb` instead of family `sda`.                                                                        |
...
| `context`         | Change the default [context](agent/web.md#contexts) of the chart. Changing this setting will affect what metrics and metrics the chart displays, and which alarms are attached to it.                                                                    |
...
| `name`            | The name of the chart that appears in the top-left corner, after the chart's title. You can also use this name when writing [health entities](agent/health/reference.md#health-entity-reference).                                                               |
