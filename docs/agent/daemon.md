...
-   You can stop Netdata by killing it with `killall netdata`. You can stop and start Netdata at any point. When
    exiting, the [database engine](agent/database/engine.md) saves metrics to `/var/cache/netdata/dbengine/` so that
...
You can send commands during runtime via [netdatacli](agent/cli.md).
