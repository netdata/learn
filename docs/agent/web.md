...
1.  The [local Agent dashboard](/docs/agent/web/gui) that comes pre-configured with every Netdata installation. You can
    ...
    JavaScript](/docs/agent/web/gui#customizing-the-local-dashboard).

2.  The [`dashboard.js` JavaScript library](#dashboardjs), which helps you
   [customize the standard dashboards](/docs/agent/web/gui#customizing-the-local-dashboard)
...
You can also view all the data Netdata collects through the [REST API v1](/docs/agent/web/api#netdata-rest-api).
...
The [custom dashboards documentation](/docs/agent/web/gui/custom) contains examples of such
custom HTML pages.
