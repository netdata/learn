...
Otherwise, [perform a cleanup](/docs/developer-and-contributor-corner/install-the-netdata-agent-from-a-git-checkout#perform-a-cleanup-in-your-netdata-repo)
...
-   `--disable-telemetry`: Opt-out of [anonymous statistics](/docs/netdata-agent/configuration/anonymous-telemetry-events) we use to make
    Netdata better.
...
-   `--local-files`: Used for [offline installations](/docs/netdata-agent/installation/linux/offline-systems). Pass four file paths: the Netdata
...
Unlike the [`kickstart.sh`](/docs/netdata-agent/installation/linux), the `netdata-installer.sh` script does
not allow you to automatically [connect](/docs/netdata-agent/configuration/connect-agent-to-cloud) your node to Netdata Cloud immediately after installation.

See the [connect to cloud](/docs/netdata-agent/configuration/connect-agent-to-cloud) doc for details on connecting a node with a manual installation of Netdata.
