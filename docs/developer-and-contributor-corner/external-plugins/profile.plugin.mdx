...
Edit the `netdata.conf` configuration file using [`edit-config`](/docs/netdata-agent/configuration#edit-netdataconf) from the [Netdata config directory](/docs/netdata-agent/configuration#the-netdata-config-directory), which is typically at `/etc/netdata`.
