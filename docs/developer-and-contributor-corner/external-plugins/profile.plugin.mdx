...
Edit the `netdata.conf` configuration file using [`edit-config`](/docs/netdata-agent/configuration#use-edit-config-to-edit-configuration-files) from the [Netdata config directory](/docs/netdata-agent/configuration#the-netdata-config-directory), which is typically at `/etc/netdata`.
