...
the [official installation method](/docs/netdata-agent/installation#install-on-linux-with-one-line-installer).
...
Netdata [config directory](/docs/netdata-agent/configuration#the-netdata-config-directory), which is typically
at `/etc/netdata`.
...
Then [restart netdata](/docs/netdata-agent/installation#maintaining-a-netdata-agent-installation) for the
...
Read [how to write a Netdata collector in Go](/docs/developer-and-contributor-corner/external-plugins/go.d.plugin/how-to-write-a-netdata-collector-in-go).
