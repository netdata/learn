...
- If you want to test the collector with the actual Netdata Agent, you need to replace the `go.d.plugin` binary that
  ...
  you [restart](/docs/netdata-agent/installation#maintaining-a-netdata-agent-installation) the Netdata Agent, it will detect and run
...
:exclamation: Netdata module produces [`charts`](/docs/developer-and-contributor-corner/external-plugins#chart), not
raw metrics.
...
- `Charts` returns the [charts](/docs/developer-and-contributor-corner/external-plugins#chart) (`*module.Charts`).
