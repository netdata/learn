...
sidebar_position: "61248000"
learn_link: "https://learn.netdata.cloud/docs/developer-and-contributor-corner/monitor-pi-hole-(and-a-raspberry-pi)-with-netdata"
---
...
collector](/docs/collecting-metrics/dns-and-dhcp-servers/pi-hole) will autodetect the new service
...
If you're completely new to Netdata, look at the [Introduction](https://github.com/netdata/netdata/blob/master/docs/getting-started/introduction.md) section for a walkthrough of all its features. For a more expedited tour, see the [get started documentation](/docs/netdata-agent/installation).

### Enable temperature sensor monitoring

You need to manually enable Netdata's built-in [temperature sensor
collector](/docs/collecting-metrics/hardware-devices-and-sensors/linux-sensors-sysfs) to start collecting metrics.

> Netdata uses a few plugins to manage its [collectors](/docs/collecting-metrics/collectors-configuration), each using a different language: Go,
...
engine](/docs/developer-and-contributor-corner/database-engine). On the Raspberry Pi used for this guide, Netdata collects 1,500 metrics every
...
and the [Database configuration documentation](/docs/netdata-agent/configuration/optimizing-metrics-database/database-modes-for-parent-child-setups) to help you determine the right
